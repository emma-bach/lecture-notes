...
\end{proof}
\section{Konstruktion der reellen Zahlen durch Dedekindschnitte}
\chapter{Topologie}
...
\end{proof}
\section{Konstruktion der reellen Zahlen durch Cauchyfolgen}
%
%
%
%
...
\end{theorem}
\begin{corollary}
	Seien $f,g$ $\mu$-messbar und $\int_X f \dmu > -\infty$. Sei $f \leq g$ $\mu$-fast überall. Dann gilt
	\begin{align*}
		\int_X f \dmu \leq \int_X g \dmu
	\end{align*}
	Insbesondere ist $\int_X g \dmu$ definiert. Die entsprechende Aussage gilt mit $\geq$, falls $\int_X f \dmu < \infty$.
\end{corollary}
\begin{proposition}
	Sei $\alpha \in \bR$. Wir betrachten
	\begin{align*}
		f : \bR^n &\to \bR^n\\
		x &\mapsto \abs{x}^{-\alpha}
	\end{align*}
	So gilt
	\begin{align*}
		\int_{\bR^n \setminus B_1(0)} f\ d\lambda^n < \infty \Leftrightarrow \alpha > n
	\end{align*}
	und
	\begin{align*}
		\int_{B_1(0)} f\ d\lambda^n < \infty \Leftrightarrow \alpha < n
	\end{align*}
\end{proposition}
\section{Konvergenzsätze}
Das Lebesqueintegral wurde insbesondere definiert, weil man die Bedingungen abschwächen wollte, bei denen man Grenzwerte und Integrale vertauschen kann, also:
\begin{align*}
	\lim_{n \to \infty} \int_X f_n \dmu = \int_X \lim_{n \to \infty} f_n \dmu
\end{align*}
Auch beim Lebesqueintegral geht dies nicht immer.
\begin{example}
	Sei 
	\begin{align*}
		f_\epsilon = \frac{1}{2\epsilon} \chi_{[-\epsilon, \epsilon]}
	\end{align*}
	So konvergiert $f_\epsilon$ fast überall gegen
	\begin{align*}
		f_0 = \begin{cases}
			\infty & x = 0\\
			0 & x \neq 0\\
	...
	Also:
	\begin{align*}
		\lim_{\epsilon \to 0} \int_X f_\epsilon \dmu = 1 > 0 = \int_X \lim_{\epsilon \to 0} f_\epsilon \dmu
	\end{align*}
\end{example}
\begin{theorem}
	\theoremname{Satz von Levi / Monotoner Konvergenzsatz:} Seien $f_n, f$ $\mu$-messbare Funktionen auf $X$ mit $f_n \uparrow f$ $\mu$-fast überall und sei $\int_X f_1 \dmu > -\infty$. Dann gilt
	\begin{align*}
		\lim_{n \to \infty} \int_x f_n \dmu = \int_x f \dmu
	\end{align*}
\end{theorem}
\begin{anmerkung}
	Ist $\mu$ vollständig, so folgt die $\mu$-Messbarkeit von $f$ bereits aus der $\mu$-Messbarkeit der $f_n$.
\end{anmerkung}
\begin{corollary}
	Für nichtnegative $\mu$-messbare Funktionen $f_n$ gilt:
	\begin{align*}
		\int_X \sum_{n = 0}^\infty f_n \dmu = \sum_{n = 0}^\infty \int_X f_n \dmu
	\end{align*}
\end{corollary}
\begin{lemma}
	\theoremname{Lemma von Fatou:} Sei $(f_k)$ eine Folge $\mu$-messbarer Funktionen auf $X$ und sei $g$ integrierbar. Falls für alle $k \in \bN$ $f_k \geq g$ $\mu$-fast überall, dann gilt
	\begin{align*}
		\int_x \liminf f_k \dmu \leq \liminf \int_X f_k \dmu
	\end{align*}
\end{lemma}
\begin{proof}
	$f_k$ sei auf $D_k \in \cA$ definiert und $\mu(X \setminus D_k) = 0$. Sei $D := \bigcap D_k$ und
	\begin{align*}
		f : D &\to \barR\\ 
			x &\mapsto \liminf f_k(x)
	\end{align*} 
	Sei nun $g_k := \inf_{j \geq k} f_j$. Diese Folge ist monoton wachsend und definiert auf $\bigcup_{j \geq k} D_j$. Nun gilt
	\begin{align*}
		\lim_{k \to \infty} g_k = f
	\end{align*}
	Es existiert $g_k \geq g \in \cL^1$, also gilt
	\begin{align*}
		\int_X \liminf_{k \to \infty} f_k \dmu &= \int_X f \dmu\\
		&= \liminf_{k \to \infty} g_k \dmu\\ 
		&\leq \liminf_{k \to \infty} \int_X f_k \dmu
	\end{align*}
\end{proof}
\begin{theorem}
	\theoremname{Satz von Lebesque / Satz über dominierte Konvergenz:} Seien $f_k$ und $f$ $\mu$-messbare Funktionen auf $X$ mit $f_k \to f$ $\mu$-fast überall. Sei $g$ eine nichtnegative integrierbare Funktion, sodass
	\begin{align*}
		\sup_{k \in \bN} \abs{f_k} \leq g
	\end{align*}
	$\mu$-fast überall. Dann ist $f$ integrierbar und
	\begin{align*}
		\int_x f \dmu = \lim_{k \to \infty} \int_x f_k \dmu.
	\end{align*}
	Es gilt sogar
	\begin{align*}
		\norm{f - f_k}_{L^1(\mu)} := \int_X \abs{f - f_k} \dmu \to 0
	\end{align*}
\end{theorem}
\begin{corollary}
	Sei $h_n$ eine Folge $\mu$-messbarer Funktionen und $g \in \cL^1$. Falls die die Reihe
	\begin{align*}
		\sum_{j = 0}^\infty h_j
	\end{align*}
	$\mu$-fast überall konvergiert und
	\begin{align*}
		\sup_{n \in \bN}\abs{\sum_{j = 0}^n} \leq g
	\end{align*}
	Dann gilt
	\begin{align*}
		\int_X \sum_{j \in \bN} h_j \dmu = \sum_{j \in \bN} \int_X h_j \dmu
	\end{align*}
\end{corollary}
\begin{corollary}
	Jede Regelfunktion $f : (I = [a,b]) \to \bR$ ist integrierbar, und der Regelintegral stimmt mit dem Lebesqueintegral überein, also:
	\begin{align*}
		\int_a^b f \ dx = \int_I f \ d \lambda^1
	\end{align*}
\end{corollary}
\begin{anmerkung}
	Dies gilt nicht für uneigentliche Integrale! Es gibt Funktionen, die ein uneigentliches Regelintegral haben, aber kein Lebesqueintegral!
\end{anmerkung}
\begin{lemma}
	\theoremname{(Stetigkeit von Parameterintegralen)}
	Sei $(X, \cA, \mu)$ ein Maßraum, $U \subset \bR^n$ eine offene Menge und erfülle $f : U \times X \to \bR$ die folgenden Bedingungen:
	\begin{enumerate}
		\item Es existiert eine Nullmenge $N \subseteq X$, sodass für alle Punkte $x \in X \setminus N$ die Funktion $f(\cdot, x)$ stetig in $U$ ist;
		\item Für alle $y \in U$ ist die Funktion $f(y, \cdot)$ $\mu$-messbar;
		\item Es existiert eine Funktion $g \in \cL^1(\mu)$, sodass für alle $y \in U$ und für $\mu$-fast alle $x \in X$ gilt
		\begin{align*}
			\abs{f(y,x)} \leq g(x)
		\end{align*}
		...
		\begin{align*}
			\phi : U &\to \bR\\
			y &\mapsto \int_X f(y,x) d\mu(x)
		\end{align*}
		stetig auf $U$.
	\end{enumerate}
\end{lemma}
\begin{theorem}
	\theoremname{(Differentiation unter dem Integralzeichen)} Sei $(X, \cA, \mu)$ ein Maßraum, $U \subset \bR^n$ eine offene Menge und erfülle $f : U \times X \to \bR$ die folgenden Bedingungen:
	\begin{enumerate}
		\item Es existiert eine Nullmenge $N \subseteq X$, sodass für alle Punkte $x \in X \setminus N$ die Funktion $f(\cdot, x)$ stetig differenzierbar in $U$ ist;
		\item Für alle $y \in U$ ist die Funktion $f(y, \cdot)$ $\mu$-messbar;
		\item Es existiert eine Funktion $g \in \cL^1(\mu)$, sodass für alle $y \in U$ und für $\mu$-fast alle $x \in X$ und $1 = 1, \hdots, n$ gilt
		\begin{align*}
			\abs{\partial_i f(y,x)} \leq g(x)
		\end{align*}
		Dann ist die Funktion
		\begin{align*}
			\phi : U &\to \bR\\
			y &\mapsto \int_X f(y,x) d\mu(x)
		\end{align*}
		auf $U$ stetig differenzierbar und es gilt für alle $y \in U$ und $i = 1, \hdots, n$
		\begin{align*}
			\partial_i \phi(y) = \int_X \partial_i f(y,x) d\mu(x)
		\end{align*}
	\end{enumerate}
\end{theorem}
\begin{theorem}
	\theoremname{(Absolutstetigkeit des Integrals)} Sei $f \in \cL^1(\mu)$. Dann gibt es zu jedem $\epsilon > 0$ ein $\delta > 0$, sodass für alle $A \in \cA$ gilt:
	\begin{align*}
		\mu(A) < \delta \implies \int_A \abs{f} d\mu < \epsilon
	\end{align*}
\end{theorem}
\section{$L^p$-Räume}
Die wohl wichtigste Konsequenz der Konvergenzsätze ist, dass die sogenannten $L^p$-Räume ein und zeigen, dass sie Banachräume (Vollständig normierte Vektorräume) sind.
\begin{definition}
	Sei $1 \leq p < \infty$. Wir bezeichnen mit $\cL^p = \cL^p(X, \cA, \mu)$ die Menge aller $\mu$-messbaren Funktionen $f$ mit
	\begin{align*}
		\int_X \abs{f}^p \dmu < \infty
	\end{align*}
\end{definition}
\begin{definition}
	Sei $f \in \cL^p$. Die $L^p$-Norm der Funktion $f$ ist die Abbildung
	\begin{align*}
		\norm{f}_p = \lr(\int_X \abs{f}^p \dmu)^\frac{1}{p}
	\end{align*}
\end{definition}
\begin{definition}
	Mit $\cL^\infty$ bezeichnen wir die Menge aller $\mu$-messbaren Funktionen $f$, fürdie eine Konstante $K \in \bR$ existiert, sodass
	\begin{align*}
		\abs{f} \leq K \quad\textnormal{$\mu$-fast überall}
	\end{align*}
	Die Größe
	\begin{align*}
		\norm{f}_\infty &:= \underset{x \in X}{\textnormal{ess sup}} \abs{f(x)}\\ &:= \inf\lr{s > 0 \mid \mu(\abs{f} > s) = 0}
	\end{align*}
	heißt $L^\infty$-Norm von $f$.
\end{definition}
\begin{anmerkung}
	"ess sup" steht für "essentielles Supremum".
\end{anmerkung}
\noindent Wir müssen nun zeigen, dass die $L^p$-Normen tatschlich Normen sind.
\begin{lemma}
	\theoremname{Young-Ungleichung} Sei $p,q \in (1, \infty)$ mit $\frac{1}{p} + \frac{1}{q} = 1$. Für nichtnegative Zahlen $a,b \geq 0$ gilt:
	\begin{align*}
		ab \leq \frac{a^p}{p} + \frac{b^q}{q}
	\end{align*}
\end{lemma}
\begin{proof}
	Aus der Konkavität der Logarithmusfunktion folgt für $a,b > 0$:
	\begin{align*}
		...
		&= \ln(ab)
	\end{align*}
	Die Logarithmusfunktion ist streng monoton wachsend, also gilt auch die Behauptung.
\end{proof}
\begin{lemma}
	\theoremname{(Hölder-Ungleichung)} Sei $f \in \cL^p$ und $g \in \cL^p$ mit $p,q \in (1, \infty)
	$ und $\frac{1}{p} + \frac{1}{q} = 1$. Dann ist $fg \in \cL^1$ und es gilt:
	\begin{align*}
		\abs{\int_x fg \dmu} \leq \int_X \abs{fg} \dmu \leq \norm{f}_p \norm{g}_q
	\end{align*}
\end{lemma}
\begin{theorem}
	Sei $1 \leq p \leq \infty$. Dann gilt:
	\begin{enumerate}
		\item $\norm{f}_p = 0 \implies f = 0$ $\mu$-fast überall
		\item Sei $f \in \cL^p$ und $\lambda \in \bR$. Dann gilt:
		\begin{align*}
			\norm{\lambda f}_p = \abs{\lambda}\norm{f}_p
		\end{align*}
		\item \theoremname{Minkowski-Ungleichung:} Sei $f,g \in \cL^p$. Dann gilt $f + g \in \cL^p$	und
		\begin{align*}
			\norm{f + g}_p \leq \norm{f}_p + \norm{g}_p
		\end{align*}
\end{enumerate}
\end{theorem}
$\norm{\cdot}_p$ ist also schon fast eine Norm - das Problem ist momentan noch, dass es Funktionen gibt, welche nicht die Nullfunktion sind, aber trotzdem Norm $0$ haben. Wir lösen dies durch eine Äquivalenzklassenkonstruktion.
\begin{definition}
	Wir definieren die Äquivalenzrelation $f \sim g \Leftrightarrow f = g$ $\mu$-fast überall. Unsere $L^p$-Räume sind nun die Mengen der Äquivalenzklassen
	\begin{align*}
		L^p = \lr{[f] \mid f \in \cL^p}
	\end{align*}
	mit den offensichtlichen Definitionen für Addition, Skalarmultiplikation und Norm.
\end{definition}
In der Praxis wird leider oft nicht zwischen $L^p$ und $\cL^p$ unterschieden :(
\begin{theorem}
	Für beliebige $E \in \cA$ und $f : E \to \barR$ sei $f_0 : X \to \barR$ die Fortsetzung von $f$ mit $\forall x \in X \setminus E : f_0(x) = 0$. Wir setzen dann
	\begin{align*}
		\cL^p(E) = \cL^p(E, \mu) := \lr{f : E \to \barR \mid f_0 \in \cL(X)}
	\end{align*}
	und dementsprechend
	\begin{align*}
		L^p(E) := \lr{[f] \mid f \in \cL^p(E)}
	\end{align*}
	Durch
	\begin{align*}
		\norm{[f]}_{p, L(E)} = \norm{[f_0]}_{p, L(X)}
	\end{align*}
	Wird eine Norm auf $L^p(E)$ definiert.
\end{theorem}
\begin{lemma}
	Sei $1 \leq p < \infty$ und 
	\begin{align*}
		f_k(x) = \sum_{j = 1}^k u_j(x)
	\end{align*}
	mit $u_j \in L^p$. Falls 
	\begin{align*}
		\sum_{j = 1}^\infty \norm{u_j(x)} < \infty,
	\end{align*}so gilt:
	\begin{enumerate}
		\item $f(x) = \lim_{k \to \infty} f_k(x)$ existiert für fast alle $x \in X$
		\item $f \in L^p$
		\item $\lim_{k \to \infty }\norm{f - f_k}_p = 0$
	\end{enumerate}
\end{lemma}
\begin{theorem}
	\theoremname{(Satz von Riesz-Fischer)} Für $1 \leq p \leq \infty$ ist $(L^p, \norm{\cdot}_p)$ ein Banachraum, also vollständig.
\end{theorem}
\begin{proof}
	Sei $(f_k)$ eine Cauchyfolge in $L^p$. Da die Folge Cauchy ist, reicht es zu zeigen, dass eine konvergente Teilfolge existiert:
	\begin{align*}
		\norm{f - f_n} \leq \norm{f - f_{n_k}} - \norm{f_{n_k} - f_n}
	\end{align*}
	Der Rest folgt, sobald ich wieder Zugang zum Skript habe :)
\end{proof}
\begin{corollary}
	Gilt $f_k \to f$ in $L^p$ $1 \leq p \leq \infty$, so existeirt eine Teilfolge $f_{k_j}$, die punktweise $\mu$-fast-überall gegen $f$ konvergiert. Im Fall $p = \infty$ ist diese Teilfolge die ganze Folge, im endlichen Fall ist im Allgemeinen eine echte Teilfolge nötig.
\end{corollary}
\begin{example}
	Seien $n,j,k \in \bN$, sodass $n = 2^k + j$ und $j \leq 2^k$. Wir betrachten eine Folge
	\begin{align*}
		f_n(x) = 
		\begin{cases}
			1 & j \cdot 2^{-k} \leq x \leq (j+1) \cdot 2^{-k}\\
			0 & \textnormal{sonst}
		\end{cases}
	\end{align*}
	Es gilt
	\begin{align*}
		\int_{\bR} f_n d\lambda^1 = 2^{-k} \leq \frac{2}{n} \to 0
	\end{align*}
	aber die Folge konvergiert nicht punktweise. Wohl aber konvergiert die Teilfolge
	\begin{align*}
		f_{n_k} := f_{2^k + \lfloor{x2^k}\rfloor}
	\end{align*}
	Denn für alle $k$ gilt $f_{n_k} = 1$.
\end{example}
\begin{theorem}
	\theoremname{(Konvergenzsatz von Vitali)} Sei $1 \leq p < \infty$, also $p$ endlich. Die Folge $f_n \in L^p$ konvergiere punktweise $\mu$-fast überall gegen eine $\mu$-messbare Funktion $f$. Dann sind die folgenden Aussagen äquivalent:
	\begin{enumerate}
		\item $f \in L^p$ und $\lim_{k \to \infty} \norm{f - f_k}_p = 0$
		\item Die Folge ist \tbf{gleichgradig integrierbar}: Mit $A \in \cA$ und
		\begin{align*}
			\lambda(A) = \limsup_{k \to \infty} \int_A \abs{f_k} \dmu
		\end{align*}
		gilt:
		\begin{enumerate}
			\item Für jedes $\epsilon \in \bR_{> 0}$ existiert ein $\delta \in \bR_{> 0}$, sodass für alle $A \in \cA$ mit $\mu(A) < \delta$ $\lambda(A) \leq \epsilon$ gilt.
			\item Für jedes $\epsilon \in \bR_{> 0}$ existiert ein $E \in \cA$, sodass $\mu(E) < \infty$ und $\lambda(X \setminus E) < \epsilon$
		\end{enumerate}
	\end{enumerate}
\end{theorem}
\begin{anmerkung}
	Bedingung (a) verhindert Situationen, in der eine Funktion auf sehr kleinen Intervallen sehr großes Maß hat, und (b) verhindert Situationen, in denen die Masse der Funktion "ins unendliche wandert".
\end{anmerkung}
\begin{definition}
	Sei $\Omega \subset \bR^n$ eine offene Menge. Der \tbf{Träger} einer Funktion $f : \Omega \to \bR$ ist die Menge
	\begin{align*}
		\spt(f) := \overline{x \in \Omega \mid f(x) \neq 0}
	\end{align*}
\end{definition}
\begin{definition}
	Der Raum der stetigen Funktionen mit kompaktem Träger in $\Omega$ wird mit
	\begin{align*}
		C_0^0(\Omega)
	\end{align*}
	bezeichnet.
\end{definition}
\begin{definition}
	Für $K \subset \bR^n$ kompakt, $x \in \bR^n$, $X \subset \bR^n$ definieren wir
	\begin{align*}
		\dist(x,K) := \inf_{y \in K} d(x,y)
	\end{align*}
	\begin{align*}
		\dist(X,K) = \inf_{x \in X} \dist(x,K)
	\end{align*}
\end{definition}
\begin{proposition}
	$\dist$ ist lipschitzstetig mit Lipschitzkonstante $1$.
\end{proposition}
\section{Satz von Fubini}
(Nachzuarbeiten: Viel Vorarbeit)
\begin{theorem}
	\theoremname{(Satz von Fubini)}
	Sei $f \in \cL^*(\mu \otimes \nu)$. Dann gilt
	\begin{align*}
		\int_{X \times Y} f ~d\mu \otimes \nu 
		&= \int_X \lr(\int_Y f ~d\nu(y)) ~d\mu(x)\\
		&= \int_Y \lr(\int_X f ~d\mu(x)) ~d\nu(y)
	\end{align*}
\end{theorem}
\begin{theorem}
	Der Satz von Fubini ist anwenderbar, wenn:
	\begin{enumerate}
		\item $f \geq 0$ und $\mu \otimes \nu$-messbar ist.
		\item $\int_X \int_Y \abs{f(x,y)} \leq \infty$ oder $\int_Y \int_X \abs{f(x,y)} \leq \infty$
	\end{enumerate}
\end{theorem}
\begin{theorem}
	Seien $(X, \cA, \mu)$ und $(Y, \cC, \nu)$ vollständige Maßräume. Sei $Q$ der Halbring der messbaren Quader in $X \times Y$ und $\lambda$ das durch die Einschränkung des Produktmaßes $\mu \otimes \nu$ auf $Q$ gegebene Prämaß. Sei $\tau$ die Carathéodory-Fortsezung von $\lambda$.
	\newpar
	Dann gilt $(X \times Y, \cA \overline{\otimes} \cC, \mu \overline{\otimes} \nu) = (X \times Y, \cM(\tau), \tau|_{\cM(\tau)})$.
\end{theorem}
\begin{corollary}
	Das Lebesquemaß $\lambda^{n+k}$ auf $\cM(\lambda^{n+k})$ ist die Vervollständigung des Produktmaßes von $\lambda^n$ und $\lambda^k$ auf ihren jeweiligen messbaren Mengen, also als Formel:
	\begin{align*}
		\lambda^{n+k} = \lambda^n \overline{\otimes} \lambda^k
	\end{align*}
\end{corollary}
\begin{theorem}
	\theoremname{(Satz von Fubini 2.0)}
	Seien $(X, \cA, \mu)$, $(Y, \cC, \nu)$ vollständige, $\sigma$-endliche Maßräume, und sei $f \in \cL^*(\mu \overline{\otimes} \nu)$. Dann ist die Funktion $f(x,\cdot)$ $\nu$-messbar für alle $x \in X$ und die Funktion $f(\cdot, y)$ ist $\mu$-messbar für alle $y \in Y$, und es gilt:
	\begin{align*}
		\int_{X \times Y} f d\mu \overline{\otimes} \nu = \int_X \lr(\int_Y f(x,y) ~d\nu(y)) ~d\mu(x) = \int_Y\lr(\int_X f(x,y) ~d\mu(x)) ~d\nu(Y)
	\end{align*}
\end{theorem}
\begin{theorem}
	\theoremname{(Partielle Integration)} Seien $f,g \in C^1(\bR^n)$ Funktionen mit $f,\partial_j f \in L^p(\bR^n)$ und $g, \partial_j g \in L^q(\bR^n)$ für ein $j \in \lr{1, \hdots, n}$ und $p,q \in [1, \infty]$ mit $\frac{1}{p} + \frac{1}{q} = 1$. Dann gilt:
	\begin{align*}
		\int_{\bR^n} (\partial_j f)g ~d\lambda^n = -\int_{\bR^n} f(\partial_j g) ~d\lambda^n
	\end{align*}
\end{theorem}
\section{Dichtefunktionen und Satz von Radon-Nikodym}
\begin{theorem}
	Seien $(X,\cA,\mu)$ ein Maßraum und $\theta$ eine nichtnegative $\mu$-messbare Funktion auf $X$. Dann ist die Funktion $\nu : \cA \to \barR$ mit
	\begin{align*}
		\nu(A) := \int_A \theta ~d\mu
	\end{align*}
	ein Maß, welches auch mit $\mu_\theta$ bezeichnet wird.
\end{theorem}
\begin{theorem}
	Es gilt außerdem:
	\begin{enumerate}
		\item Aus $\mu(A) = 0$ folgt $\mu_\theta(A) = 0$
		\item Für alle $\mu$-messbaren nichtnegativen Funktionen gilt
		\begin{align*}
			\int_X f ~d\mu_\theta = \int_X f \theta d\mu
		\end{align*}
	\end{enumerate}
\end{theorem}
\begin{definition}
	Wir nennen $\theta$ die \tbf{Dichtefunktion} oder einfach \tbf{Dichte} von $\mu_\theta$ bezüglich $\mu$.
\end{definition}
Wir können also aus einem gegebenen Maß $\mu$ ein neues Maß $\nu = \mu_\theta$ basteln, indem wir jedem Punkt $x \in X$ eine "Dichte"/"Masse" $\theta(x)$ zuweisen.
\begin{definition}
	Seien $(X, \cA, \mu)$ und $(X,\cA,\nu)$ Maßräume. Wir sagen $\nu$ ist \tbf{absolutstetig} bezüglich $\mu$, falls $\nu(A) = 0$ für alle $A \in \cA$ mit $\mu(A) = 0$. Wir notieren dies $\nu \ll \mu$.
\end{definition}
\begin{theorem}
	Seien $(X,\cA,\sigma)$ und $(X,\cA,\nu)$ endliche Maßräume, sodass $\nu(A) \leq \sigma(A)$ für alle $A \in \cA$. Dann existiert eine nichtnegative Funktion $\theta \in L^2(\sigma)$, sodass für alle $A \in \cA$
	\begin{align*}
		\nu(A) = \int_A \theta ~d\sigma.
	\end{align*}
\end{theorem}
\begin{anmerkung}
	Dieser Satz ist ein Spezialfall des sogenannten Rieszschen Darstellungssatzes für Hilberträume. Für $g \in L^2(\sigma)$ und
	\begin{align*}
		F(g) = \int_X g ~d\nu
	\end{align*}
	ist $F$ ein lineares Funktional auf $L^2$. Durch Hölder kann die Stetigkeit von $F$ gezeigt werden. $L^2$ ist ein Hilbertraum, und der Rieszsche Darstellungssatz besagt nun genau, dass ein $f \in L^2(\sigma)$ existiert, sodass für alle $g \in L^2(\sigma)$
	\begin{align*}
		F(g) = \scalar{f}{g}_{L^2(\sigma)} = \int_X fg ~d\sigma.
	\end{align*}
	Mit $g = \chi_A$ folgt unser Lemma.
\end{anmerkung}
\begin{lemma}
	Sei $(X, \cA)$ ein messbarer Raum, seien $\mu,\nu$ Maße auf $\cA$, und sei $\sigma := \mu + \nu$. Falls $f \in \cL^*(\sigma)$, dann gilt $f \in \cL(\mu)$ und $f \in \cL(\nu)$, und es gilt
	\begin{align*}
		\int_X f ~d\sigma = \int_X f ~d\mu + \int_X f ~d\nu.
	\end{align*}
\end{lemma}
\begin{theorem}
	\theoremname{(Radon-Nikodym):} Seien $(X,\cA,\mu)$ und $(X,\cA,\nu)$ endliche Maßräume und $\nu \ll \mu$. Dann existiert eine eindeutige nichtnegative Dichtefunktion $\theta \in L^1(\mu)$ von $\nu$ bezüglich $\mu$, sodass für alle $A \in \cA$
	\begin{align*}
		\nu(A) = \int_A \theta ~d\mu
	\end{align*}
\end{theorem}
\begin{anmerkung}
	Wir bezeichnen diese Dichtefunktion $\theta$ auch als die \tbf{Radon-Nikodym-Ableitung}, notiert
	\begin{align*}
		\frac{d\nu}{d\mu}
	\end{align*}
\end{anmerkung}
\begin{theorem}
	Sei $(X, \overline \cA)$ ein messbarer Raum, seien $\mu, \lambda$ endliche Maße auf $\cA$. Dann sind äquivalent:
	\begin{enumerate}
		\item $\nu \ll \mu$
		\item Es existiert eine nichtnegative Funktion $\theta \in L^1(\mu)$, sodass für alle $A \in \cA$
		\begin{align*}
			\nu(A) = \int_A \theta ~d\mu
		\end{align*}
		\item Für alle $\epsilon > 0$ existiert ein $\delta > 0$, sodass für alle $A \in \cA$
		\begin{align*}
			\mu(A) < \delta \implies \nu(A) < \epsilon
		\end{align*}
	\end{enumerate}
\end{theorem}
\begin{anmerkung}
	Durch Aussage (iii) ist die Bezeichnung der Relation $\nu \ll \mu$ als eine Form der Steigkeit gerechtfertigt.
\end{anmerkung}
Der Satz von Radon-Nikodym gilt auch noch in leicht abgeschwächter Form, wenn $\nu$ und $\mu$ nicht mehr endlich sind, sondern nur noch $\sigma$-endlich (also müssen abzählbare Vereinigungen von Mengen mit endlichem Maß endliches Maß haben):
\begin{theorem}
	Sei $(X,\cA)$ ein messbarer Raum. Seien $\mu, \nu$ $\sigma$-endliche Maße mit $\nu \ll \mu$. Dann gibt es eine nichtnegative $\mu$-messbare Funktion $\theta$, sodass für alle $A \in \cA$
	\begin{align*}
		\nu(A) = \int_A \theta ~d\mu.
	\end{align*}
	$\theta$ ist eindeutig bis auf $\mu$-fast-überall-Gleichheit.
\end{theorem}
\begin{theorem}
	Seien $(X,\cA,\mu)$ und $(X,\cA,\nu)$ Maßräume. Wir sagen, dass die Maße $\mu$ und $\nu$ \tbf{zueinander singulär} sind, in Zeichen $\mu \bot \nu$, falls es eine Menge $M \in \cA$ gibt, sodass $\mu(M) = 0$, aber auch $\nu(X \setminus M) = 0$.
\end{theorem}
\begin{proposition}
	Das Lebesquemaß $\lambda^1$ und die Einschränkung des Diracmaßes $\delta_0$ auf die Lebesque-messbaren Mengen sind zueinander singulär.
\end{proposition}
\begin{proof}
	$\lambda^1(\lr{0}) = 0$ und $\delta_0(\bR \setminus \lr{0}) = 0$.
\end{proof}
\begin{theorem}
	\theoremname{(Zerlegungssatz von Lebesque)} Sei $(X, \cA)$ ein messbarer Raum. Sei $\mu$ ein beliebiges Maß und $\nu$ ein $\sigma$-endliches Maß, beide auf $\cA$. Dann existiert eine eindeutige Zerlegung $\nu = \nu_a + \nu_s$ mit $\nu_a \ll \mu$ und $\nu_s \bot \mu$. Man nennt $\nu_a$ den absolutstetigen Teil und $\nu_s$ den singulären Teil von $\nu$ bezüglich $\mu$.
\end{theorem}
\begin{proof}
	\begin{enumerate}
		\item Sei $\nu$ endlich und $c := \sup \lr{\nu(B) : B \in \cA, \mu(B) = 0} \in [0,\infty)$. Wähle eine Folge von Mengen $B_j \in \cA$ mit $\nu(B_j) \to c$.
		Mit $M := \bigcup_j B_j$ folgt $\mu(M) = 0$. Gleichzeitig gilt
		\begin{align*}
			c \geq \nu(M) \geq \nu(B_j) \to c,
		\end{align*}
		also $\nu(M) = c$.
		\newpar
		Für jedes $A \in \cA$ definieren wir nun $\nu_a(A) := \nu(A \setminus M)$ und $\nu_s(A) := \nu(A \cap M)$. Dann folgt direkt $\nu = \nu_a + \nu_b$. Es gilt außerdem $\mu(M) = 0$ und
		\begin{align*}
			\nu_s(X \setminus M) = \nu((X \setminus M) \cap M = \nu(\emptyset) = 0
		\end{align*}
		also $\nu_s \bot \mu$.
		\newpar
		Es bleibt $\nu_a \ll \mu$ zu zeigen. Angenommen, es existiert ein
		$B' \subseteq X \setminus M$ mit $\mu(B') = 0$ und $\nu(B') > 0$. Sei $\tilde M := M \cap B'$. Dann gilt $\mu(\tilde M) = 0$ und
		\begin{align*}
			\mu(\tilde M) = \nu(M) + \nu(B') > \nu(M),
		\end{align*}
		was ein Widerspruch ist, da ja $M$ die $\mu$-Nullmenge mit größtem $\nu$-Maß ist. Also gilt für alle $\mu(B') = 0$ auch $\nu(B') = 0$, also $\nu_a(B') = 0$.
		\item Sei $\nu$ nur $\sigma$-endlich. Dann existieren paarweise disjunkte Mengen $X_k \in \cA$ mit endlichem Maß und $\bigcup_k X_k = X$. Definiere $M := \bigcup_k M_k$.
		\begin{enumerate}
			\item 
			\item $\nu_a \ll \mu$: 
			\newpar
			Sei $B' \in \cA$ mit $B' \subset X \setminus M$. Sei $\nu(B') > 0$. Dann muss ein $k \in \bN$ existieren, sodass $\nu(B' \cap X_k) > 0$. Rest wie vorher.
			\item
		\end{enumerate}
	\end{enumerate}
\end{proof}
\begin{proposition}
	Die Zerlegung ist sogar eindeutig.
\end{proposition}
\begin{proof}
	Angenommen, $\nu = \nu_a + \nu_s = \nu_a' + \nu_s'$ mit zugehöriger Zerlegung $M$ zu $\nu_a$ und $M'$ zu $\nu_a'$. Sei $A \in \cA$ und $C := A \cap (M \cup M')$ und $D := A \setminus (M \cup M')$.
	\begin{align*}
		\mu(C) \leq \mu(M) + \mu(M') = 0\\
		\implies \nu_a(c) = \nu_a'(c) = 0\\
		\implies \nu_s(c) = \nu_s'(c)
	\end{align*}
	Es gilt $D \subseteq (X \setminus M) \cap (X \setminus M')$, also $\nu_s(D) = \nu_s'(D) = 0$, also gilt auch $\nu_a(D) = \nu_a'(D)$. Insgesamt folgt $\nu(A) = \nu'(A)$, was zu zeigen war.
\end{proof}
\section{Transformationssatz}
\begin{definition}
	Eine Abbildung $\phi : U \to V$ zwischen offenen Teilmengen $U$ und $V$ des $\bR^n$ heißt $\cC^1$-Diffeomorphismus, falls sie stetig differenzierbar mit stetig differenzierbarer Umkehrfunktion ist.
\end{definition}
Erinnerung: Für $U \subseteq \bR^n$ und $V \subseteq \bR^m$ offen und $f : U \to V$ stetig differenzierbar existiert eine Lineare Abbildung $Df(x)$, welche wir als \tbf{Differential} der Abbildung bezeichnen. Die darstellende Matrix von $Df(x)$ bezüglich der Standardbasen ist die \tbf{Jacobimatrix}
\begin{align*}
	Df &= (\partial_1 f, \hdots, \partial_n f)\\
	   &=
	   \begin{pmatrix}
	   \partial_1 f_1 & \hdots & \partial_n f_1\\
	   \vdots & & \vdots\\
	   \partial_1f_m & \hdots & \partial_n f_m
	   \end{pmatrix}
\end{align*}

\subsection{Polarkoordinaten}
Im $\bR^2$ können wir jeden Punkt in \tbf{Polarkoordinaten} darstellen.
\begin{definition}
	Die Abbildung $\phi : (0,\infty) \times (0,2\pi) := U \to V := \bR^2 \setminus \lr{(x,0) : x \geq 0}$, welche einen Punkt auf seine Polarkoordinaten abbildet, ist gegeben durch:
	\begin{align*}
		\phi(r,\omega) := (r \cos \omega, r \sin \omega)^\top
	\end{align*}
	Für die Umkehrabbildung gilt:
	\begin{align*}
		\phi^{-1}(x,y) = 
		\begin{cases}
			(r, \arccos \frac{x}{r})^\top & y \geq 0\\
			(r, 2\pi - \arccos \frac{x}{r} & y < 0
		\end{cases}
	\end{align*}
\end{definition}
\begin{definition}
	\begin{align*}
		Q(x,\rho) := \lr{y \in \bR^n \mid \abs{x-y}_\infty \leq \rho}
	\end{align*}
\end{definition}
\begin{lemma}
	Sei $U \subseteq \bR^n$ offen, $x_0 \in U$, $\phi : U \to \bR^n$ eine Abbildung mit Differential $D\phi(x_0) \in GL(\bR^n)$ (also mit invertierbarem Differential). Für eine Folge von Würfeln $Q_j := Q(x_j, \rho_j) \subseteq U$ mit $\rho_j \to 0$ und $x_0 \in Q_j$ gilt dann
	\begin{align*}
		\limsup \frac{\lambda^n\lr(\phi(Q_j))}{\lambda^n(Q_j)} \leq \abs{\det D\phi(x_0)}
	\end{align*}
\end{lemma}
\begin{proof}
	Sei durch Translationen $x_0 = 0$ und $\phi(0) = 0$.
	\begin{enumerate}
		\item Angenommen, $D\phi(0) = E_n$. Dann gilt
		\begin{align*}
			0 &= \lim_{x \to 0} \frac{\norm{\phi(x) - \phi(0) + D\phi(0)x}_\infty}{\norm{x}_\infty}\\
			  &= \lim_{x \to 0} \frac{\norm{\phi(x) - x}_\infty}{\norm{x}_\infty}\\
		\end{align*}
		Sei $\epsilon > 0$, $x \in Q_j$. Dann gilt
		\begin{align*}
			\norm{x}_\infty \leq \norm{x - x_j} + \norm{x_j - 0} \leq 2 \rho_j
		\end{align*}
		(...)
	\end{enumerate}
\end{proof}
\begin{theorem}
	Seien $U, V \subset \bR^n$ offen, $\phi : U \to V$ ein $\cC^1$-Diffeomorphismus. Ist $A \subset U$ $\lambda^n$-messbar, so ist $\phi(A)$ $\lambda^n$-messbar und es gilt
	\begin{align*}
		\lambda^n(\phi(A)) = \int_A \abs{\det D\phi(x)} ~d\lambda^n(x)
	\end{align*}
	Außerdem gilt für jede $\lambda^n$-messbare Funktion $f : V \to \barR$
	\begin{align*}
		\int_V f(y) ~d\lambda^n(y) = \int_{\phi^{-1}(V)} f(\phi(x)) \cdot \abs{\det D\phi(x)} ~d\lambda^n(x)
	\end{align*}
\end{theorem}
\begin{theorem}
	Sei $\phi : U \to V$ ein $C^1$-Diffeomorphismus ($U,V \subset \bR^n$) mit zugehöriger Gramscher Matrix $g_{ij}$.
	\begin{enumerate}
		\item Für $v \in C^1(V,\bR)$ gilt mit $u = v \circ \phi$:
		\begin{align*}
			(\grad v) \circ \phi = (D \phi) \circ \grad_g u
		\end{align*}
		Wobei
		\begin{align*}
			\grad_g u = \sum_{i,j = 1}^n g^{ij} \partial_i u e_j
		\end{align*}
		\item Für $Y \in C^1(V, \bR^n)$ gilt mit $Y \circ \phi = (D\phi)X$
		\begin{align*}
			(\div Y) \circ \phi = \div_g~ X = \frac{1}{\sqrt{\det g}} \sum_{j = 1}^n \partial_j(\sqrt{\det g} X_j)
		\end{align*}
	\end{enumerate}
\end{theorem}
\begin{corollary}
	In Polarkoordinaten in $\bR^3$ gilt:
	\begin{enumerate}
		\item $\displaystyle \grad_g u = \partial_r u e_r + \frac{1}{r^2} \cdot \partial_\theta u e_\theta + \frac{1}{r^2 \sin^2 \theta} \partial_\omega u e_\omega$
		\item $\displaystyle \div_g X = \frac{1}{r^2} \cdot (\partial_r r^2 X_r) + \frac{1}{\sin \theta} \cdot \partial_\theta (\sin \theta X_\theta) + \partial_\omega X_\omega$
		\item $\displaystyle \Delta_g u = \frac{1}{r^2} \cdot \partial_r (r^2 \partial_r u) + \frac{1}{r^2} \sin \theta \cdot \partial_\theta (\sin \theta \cdot \partial_\theta u) + \frac{1}{r^2 \sin^2 \theta} \cdot \partial_\omega^2 u$
	\end{enumerate}
\end{corollary}
\begin{theorem}
	\theoremname{(Von $\phi$ induzierte Riemannsche Metrik):}
	Sei $g : U \to \bRnn$ die Gramsche Matrix eines Diffeomorphismus $\phi \in \cC^1(U,V)$. 
	\newpar
	So definiert $g$ in jedem Punkt $x \in U$ ein Skalarprodukt und eine zugehörige Euklidische Norm durch:
	\begin{align*}
		g(x)(v,w) = \scalar{D\phi(x)v}{D\phi(x)w}
	\end{align*}
	\begin{align*}
		\norm{v}_{g(x)} 
		&= \sqrt{g(x)(v,v)}\\
...
\end{theorem}
\begin{theorem}
	Außerdem gilt für die Bogenlänge einer Kurve $\gamma : [a,b] \to U$ in Koordinaten $\phi : U \to V$:
	\begin{align*}
		L(\phi \circ \gamma)
		&= \int_a^b \abs{\frac{d}{dt} \phi(\gamma(t))} ~dt\\
		&= \int_a^b \abs{D\phi(\gamma(t))\gamma'(t)} ~dt\\
		&= \int_a^b \sqrt{g(\gamma(t))(\gamma'(t), \gamma'(t))} ~dt
	\end{align*}
\end{theorem}
\section{Das Flächenmaß auf Untermannigfaltigkeiten}
\begin{definition}
	Sei $U \subset \bR^n$ offen und $f \in C^1(U, \bR^n+k)$. Wir nennen $f$ eine \tbf{Immersion}, wenn das Differential von $f$ invertierbar ist, also wenn für alle $x \in U$:
	\begin{align*}
		\rang Df(x) = n
		\qquad( \Leftrightarrow \ker Df(x) = \lr{0})
	\end{align*}
\end{definition}
\begin{definition}
	Ist $f$ eine Immersion, dann bildet $\Im Df(x)$ eine sogenannte \tbf{Untermannigfaltigkeit des $\bR^{n+k}$}. Dieser bildet einen Untervektorraum von $\bR^{n+k}$, von dem die Vektoren $\partial_i f(x)$ eine Basis bilden.
\end{definition}
\begin{definition}
	Wir definieren die Gramsche Matrix in diesem allgemeinen Fall als:
	\begin{align*}
		g(x) = Df(x)^\top Df(x) \in \Hom(\bR^{n+k}, \bR^{n+k})
	\end{align*}
	bzw.
	\begin{align*}
		g_{ij} = \scalar{\partial_i f(x)}{ \partial_j f(x)}
	\end{align*}
\end{definition}
\begin{theorem}
	$g_f$ ist genau dann strikt positiv definit, und damit auch invertierbar, wenn $f$ eine Immersion ist.
\end{theorem}
\begin{proof}
	Es gilt $\scalar{g(x)v}{v} = \abs{Df(x)v}^2$, also $\ker g(x) = \ker Df(x)$.
\end{proof}
\begin{definition}
	Sei $f \in C^1(U, \bR^{n+k})$ eine $n$-dimensionale Immersion mit Gramscher Matrix und zugehöriger Induzierter Metrik $g$, und sei $E \subset U$ $\lambda^n$-messbar. Der \tbf{$n$-dimensionale Flächeninhalt von $f$ auf $E$} ist dann definiert durch:
	\begin{align*}
		A_E(f) = \int_E \sqrt{\det g} ~d\lambda^n
	\end{align*}
\end{definition}
\begin{definition}
	Wir nennen die Funktion $\sqrt{\det g}$ auch den \tbf{Fächenintegrand} oder die \tbf{Jacobische} von $f$ und schreiben
	\begin{align*}
		Jf = \sqrt{\det g}
	\end{align*}
\end{definition}
Angenommen, $f_1 : V \to \bR^{n+k}$ und $f_2 : U \to \bR^{n+k}$ sind zwei verschiedene Parametrisierungen der gleichen Untermannigfaltigkeit $M$. Dann sind per Definition die Bilder Diffeomorph, also existiert per Definition ein $\phi \in \cC^1(U,V)$, sodass $f_2 = f_1 \circ \phi$. 
\newpar
Es folgt: $f_1^{-1}(M) = E \implies f_2^{-1}(M) = \phi(E)$. Wollen wir also zeigen, dass der Flächeninhalt der Untermannigfaltigkeit $M$ nicht von der Parametrisierung abhängt, reicht folgender Satz:
\begin{theorem}
	
	Für jeden Diffeomorphismus $\phi \in \cC^1(U,V)$, jede Immersion $f \in \cC^1(V, \bR^{n+k})$ und jede $\lambda^n$-messbare Menge $E \subset U$ gilt:
	\begin{align*}
		A_{\phi(E)}(f) = A_E(f \circ \phi)
	\end{align*}
\end{theorem}
\begin{proof}
	Es gilt:
	\begin{align*}
		g_{f \circ \phi}(x)
		&= D(f \circ \phi)^\top(x) D(f \circ \phi)(x)\\
		&= D\phi(x)^\top Df(\phi(x))^\top Df(\phi(x))D\phi(x)\\
		&=D\phi(x)^\top g_f(\phi(x)) D\phi(x)
	\end{align*}
	Es folgt
	\begin{align*}
		J(f \circ \phi) = (Jf) \circ \phi\abs{\det D\phi}
	\end{align*}
	Mit dem Transformationssatz folgt:
	\begin{align*}
		A_{\phi(E)} (f) 
		&= \int_{\phi(E) Jf ~d\lambda^n}\\
		&= \int_{E} Jf \circ \phi \abs{\det D\phi(x)} ~d\lambda^n\\
		&= \int_E J(f \circ \phi) ~d\lambda^n\\
		&= A_E(f \circ \phi)
	\end{align*}
\end{proof}
\begin{corollary}
	Die Länge $L(f)$ eine Kurve $f : (a,b) \to \bR^{1+k}$ ist gegeben durch:
	\begin{align*}
		L(f) = \int_a^b \abs{f'(t)} ~dt
	\end{align*}
\end{corollary}
\begin{corollary}
	(Inhalt einer Regulären Fläche)
\end{corollary}
\begin{lemma}
	Für $f \in C^1(U, \bR^k)$ ist die Graphenabbildung
	\begin{align*}
		\tn{Graph}(f) : \bR^n \supset U &\to \bR^{n+k}\\
		x &\mapsto (x, f(x))
	\end{align*}
	eine Immersion.
\end{lemma}
\begin{theorem}
	Sei $gr(f)$ der Graph von $f : U \to \bR^k$. Dann gilt:
	\begin{align*}
		A(\tn{Graph}(f)) = \int_U \sqrt{1 + \abs{Df(x)}^2 ~dx}
	\end{align*}
\end{theorem}
\begin{theorem}
	Sei $n \in \bN_{> 0}$, $k \in \bN_{\geq 0}$. Für $M \subset \bR^{n+k}$ sind folgende Aussagen äquivalent:
	\begin{enumerate}
		\item \theoremname{(Plättbarkeitskriterium):} 
		Zu jedem $p \in M$ gibt es eine offene Umgebung $W \subset \bR^{n+k}$ und einen $C^1$-Diffeomorphismus $\phi : W \to \phi(W) \subset \bR^{n+k}$ mit
		\begin{align*}
			\phi(M \cap W) = \lr{z \in \phi(W) \mid z_{n+1} = \hdots = z_{n+k} = 0}
		\end{align*} 
		\item \theoremname{(Niveaumengenkriterium):}
		Zu jedem $p \in M$ gibt es eine offene Umgebung $W \subset \bR^{n+k}$ und für alle $q \in W$ eine Funktion $h \in C^1(W, \bR^k)$ mit $\rang Dh(q) = k$, sodass:
		\begin{align*}
			M \cap W = \lr{q \in W \mid h(q) = 0}
		\end{align*}
		\item \theoremname{(Graphenkriterium):} 
		Zu jedem $p \in M$ gibt es eine Translation $B$ des $\bR^{n+k}$ mit $B(0) = p$, offene Mengen $U \subset \bR^n$, $V \subset \bR^k$ mit $0 \in U \times V$, und eine $C^1$-Funktion $u : U \to V$ mit $u(0) = 0$, sodass
		\begin{align*}
			M \cap B(U \times V) = B(\lr{x, u(x) \mid x \in U})
		\end{align*}
	\end{enumerate}
	Wir nennen eine Menge $M \subset \bR^{n+k}$, welche diese Bedingungen erfüllt, eine \tbf{$n-$dimensionale $C^1$-Untermannigfaltigkeit des $\bR^{n+k}$}.
\end{theorem}
\begin{definition}
	\theoremname{(Relativtopologie):}
	Jede Teilmenge $M$ eines Metrischen Raums $R$ ist, versehen mit der Metrik auf $R$, ebenfalls ein Metrischer Raum. Wir können so eine Topologie auf $M$ definieren: $V \subset M$ ist offen in $M$ bezüglich der \tbf{durch $R$ induzierten Relativtopologie auf $M$}, wenn es zu jedem $p \in V$ ein $\epsilon > 0$ gibt, sodass $M \cap B_\epsilon(p) \subset V$.
\end{definition}
\begin{lemma}
	Sei $M \subset \bR^{n+k}$. Dann gilt:
	\begin{enumerate}
		\item $V \subset M$ ist genau dann offen in $M$, wenn es eine offene Menge $W \subset \bR^{n+k}$ gibt, sodass $V = M \cap W$.
		\item Eine Menge $K \subset M$ ist genau dann kompakt in $M$, wenn $K$ kompakt in $\bR^{n+k}$ ist.
	\end{enumerate}
\end{lemma}
\begin{theorem}
	\theoremname{($\sigma$-Kompaktheit von Untermannigfaltigkeiten):}
	Für jede $n$-dimensionale $C^1$-Untermannigfaltigkeit $M$ des $\bR^{n+k}$ existieren Kompakte Mengen $K_i \subset \bR^{n+k}$, sodass
	\begin{align*}
		M = \bigcup_{i \in \bN} K_i
	\end{align*}
\end{theorem}
\begin{definition}
	Sei $M \subset \bR^{n+k}$ eine $n$-dimensionale $C^1$-Untermannigfaltigkeit. Eine \tbf{lokale Parametrisierung} von $M$, auch \tbf{Karte} von $M$ genannt, ist eine injektive $C^1$-Immersion
	\begin{align*}
		f : U \to M \subset \bR^{n+k},
	\end{align*}
	wobei $U$ offen in $\bR^n$ ist.
\end{definition}
\begin{theorem}
	Sei $M \subset \bR^{n+k}$ eine $n$-Untermannigfaltigkeit. Dann gilt:
	\begin{enumerate}
		\item Sei $f : U \to M$ eine Karte von $M$. Dann ist $f(U)$ offen in $M$ und die Umkehrfunktion ist stetig, d.h. $f : U \to f(U)$ ist ein Homeomorphismus.
		\item Für zwei verschiedene Karten $f_i : U_i \to V_i$ ist der \tbf{Parameterwechsel}
		\begin{align*}
			(f_2^{-1} \circ f_1)|_{f^{-1}(V_1 \cap V_2)}
		\end{align*}
		ein $C^1$-Diffeomorphismus.
	\end{enumerate}
\end{theorem}
\begin{theorem}
	\theoremname{(Existenz eines Abzählbaren Atlas):}
\end{theorem}
\begin{definition}
	Ein Vektor $v \in \bR^{n+k}$ heißt \tbf{Tangentialvektor} von $M \subset \bR^{n+k}$ am Punkt $p$, wenn es eine Abbildung $\gamma : (-\delta, \delta) \to M$ gibt, sodass $\gamma(0) = p$ und $\gamma'(0) = v$.
\end{definition}
\begin{corollary}
	\theoremname{(Tangentialraum):} Sei $M$ eine $n$-dimensionale $C^1$-Untermannigfaltigkeit des $\bR^{n+k}$ und $p \in M$. Dann ist die Menge aller Tangentialvektoren von $M$ ein $n$-dimensionaler Untervektorraum, der Tangentialraum $T_pM$.
\end{corollary}
\begin{theorem}
	\theoremname{(Das Oberflächenmaß):}
	Sei $M$ eine $n$-dimensionale $C^1$-Untermannigfaltigkeit des $\bR^{n+k}$, und sei $\cE$ das System aller Teilmengen $E \subset M$, für die für jede lokale Parametrisierung $f : U \to M$ gilt, dass
	\begin{align*}
		f^{-1}(E \cap f(U)) \in \cM(\lambda^n)
	\end{align*}
	Dann gibt es genau ein reguläres äußeres Maß $\omega$ auf $M$ mit folgenden Eigenschaften:
	\begin{enumerate}
		\item Das System der $\omega$-messbaren Mengen ist $\cE$. Dieses System enthält insbesondere alle Borelmengen in $M$.
		\item Ist $f : U \to M$ eine lokale Parametrisierung und $E \in \cE$ mit $E \subset f(U)$, so gilt
		\begin{align*}
			\omega(E) 
			&= A_{f^{-1}(E)}(f)\\
			&=\int_{f^{-1}(E)} Jf ~d\lambda^n\\ 
			&\lr(= \int_{f^{-1}} \sqrt{\det g_f} ~d\lambda^n)
		\end{align*}
	\end{enumerate}
	Wir nennen dieses Maß das \tbf{Flächenmaß} auf $M$.
\end{theorem}
\begin{theorem}
	Sei $M$ eine $n$-dimensionale $C^1$-Untermannigfaltigkeit des $\bR^{n+k}$ und $u : M \to \bR$ eine integrierbare Funktion bezüglich des Oberflächenmaßes $\omega$. Ist $f : U \to V$ eine lokale Parametrisierung, so gilt:
	\begin{align*}
		\int_V u ~d\omega = \int_{f^{-1}(V)} (u \circ f) \cdot  Jf ~d\lambda^n
	\end{align*}
\end{theorem}
\begin{lemma}
	\theoremname{(Transformation des Flächeninhalts):}
\end{lemma}
\begin{theorem}
	\theoremname{(Zwiebelformel):}
\end{theorem}

\clearpage
\section{Integralsatz von Gauß}
\begin{align*}
	\int_Q \div X d\lambda^2
	&= \int_Q \partial_1 X_1 + \partial_2 X_2 ~d\lambda^2\\
	&= \int_{a_2}^{b_2} \int_{a_1}^{b_1} \partial_1 X_1 ~dx_1x_2 + \int_{a_1}^{b_1} \int_{a_2}^{b_2} \partial_2 X_2 ~dx_2x_1\\
	&= \int_{a_2}^{b_2} X_1(b_1,x_2) - X_1(a_1 x_2) ~dx_2 + \int_{a_1}^{b_1} X_2(x_1, b_2) - X_2(x_1, a_2) ~dx_1\\
	&= \int_{a_2}^{b_2} \scalar{X}{e_1}(b_1,x_2) - \scalar{X}{e_1}(a_1, x_2) dx_2 + (...)\\
	&= \int_{\partial Q} \scalar{X}{\nu} ~d\omega
\end{align*}
\begin{definition}
	Eine offene Menge $\Omega \subset \bR^n$ hat einen \tbf{$C^1$-Rand}, wenn ihr Rand eine $(n-1)$-dimensionale Untermannigfaltigkeit des $\bR^n$ ist, und die Menge lokal auf einer Seite des Randes liegt: Es muss also eine $C^1$-Funktion $u : \bR^{n-1} \supset U \to I \subset \bR$ gibt, sodass nach geeigneter Umnummerierung der Koordinaten gilt:
	\begin{align*}
		\Omega \cap (U \times I) = \lr{(x,y) \in U \times I : y < u(x)}
	\end{align*}
\end{definition}
\begin{lemma}
	Für eine offene Menge $\Omega \subset \bR^n$ mit $C^1$-Rand gilt:
	\begin{align*}
		\partial \Omega \cap (U \times I) = \lr{(x,y) \in U \times I : y = u(x)}\\
		(\bR^n \setminus \ol{\Omega}) \cap (U \times I) = \lr{(x,y) \in U \times I : y > u(x)}
	\end{align*}
\end{lemma}
\begin{lemma}
	\theoremname{(Äußere Normale):}
	Sei $\Omega \subset \bR^n$ offen mit $C^1$-Rand. Dann gibt es zu $p \in \partial \Omega$ genau einen Vektor $\nu(p) \in \bR^n$ mit folgenden Eigenschaften:
	\begin{enumerate}
		\item $\nu(p) \bot T_p(\partial \Omega)$
		\item $\norm{\nu(p)} = 1$,
		\item $p + t\nu(p) \notin \Omega$ for sufficiently small $t > 0$.
	\end{enumerate}
\end{lemma}
\begin{definition}
	Für $\Omega \subset \bR^n$ offen bezeichnen wir mit $C^1(\ol{\Omega}$ den Vektorraum der Funktionen $f \in C^1(\Omega)$, für die $f$ und das Differential $Df$ stetige Fortsetzungen auf $\ol{\Omega}$ besitzen. Wir bezeichnen die Fortsetzung von $f$ wieder mit $f$. :)
\end{definition}
%
%
%
...
\begin{description}
	...
	\item[\tit{$\sigma$-endliches Maß:}] Ein Maß $\mu$ heißt $\sigma$-endlich, falls für jede Familie $M_i$ mit $\mu(M_i) < \infty$ für alle $i$ auch $\mu\lr(\bigcup_{i \in I} M_i)$ endlich ist.
