...
\section{Folgen}
\begin{definition}
	Eine Folge ist eine Abbildung $\bN \to M$ in eine beliebige Menge $M$, in der Regel in die Reellen Zahlen. Für eine Folge $a$ schreibt man in der Regel "$a_n$" statt "$a(n)$".
\end{definition}
\begin{definition}
...
\begin{proof}
	...
	\begin{align*}
		\abs{a-b} &= \abs{a - a_n - (b - a_n)}\\
		&\leq \abs{a - a_n} + \abs{b - a_n}\\
		&< 2\epsilon\\
		&=\abs{a-b}
	\end{align*}
	Also $\abs{a-b} < \abs{a-b}$, was ein Widerspruch ist.
\end{proof}
\begin{theorem}
	Jede konvergente Folge ist beschränkt.
\end{theorem}
\begin{proof}
	Sei $a_n$ eine Folge, welche gegen den Grenzwert $a$ konvergiert. So gilt per Definition von Konvergenz für ein beliebiges $\epsilon \in \bR^+$
	\begin{align*}
		\exists N \in \bN : \forall n \geq N : \abs{a_n - a} \leq \epsilon
	\end{align*}
	Es folgt:
	\begin{itemize}
		\item Alle $a_n$ mit $n \geq N$ sind höchstens $\epsilon$ von $a$ entfernt, also endlich.
		\item Die Menge $a_n$ $n < N$ ist endlich, ist also durch ihr Maximum und ihr Minimum beschränkt:
		\begin{align*}
			\min\{a_k \mid k < N\} \leq a_n \leq \max\{a_k \mid k < n\}
		\end{align*}
	\end{itemize} 
	Somit sind alle Folgenglieder beschränkt, also ist die gesamte Folge beschränkt.
\end{proof}
\begin{theorem}
	\label{theorem:increasingandboundedsequence}
	Jede begrenzte, monoton steigende Folge ist konvergent.
\end{theorem}
\begin{theorem}
