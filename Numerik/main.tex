...
\section{Singulärwertzerlegung}
	...
	Wir wollen durch geeignete Rotation sukzessiv die Außerdiagonaleinträge einer Matrix verkleinern.
	\begin{definition}
		Für $A \in \bRnn$ sei
		\begin{align*}
			\cN(A) &= \norm{A}_\cF^2 - \sum_{i = 1}^n a_{ii}^2\\
			&= \sum_{i \leq j, j \leq n, i \neq j} a_{ii}^2
		\end{align*}
	\end{definition}
	$A$ ist diagonal genau dann, wenn $\cN(A) = 0$. 
	
	Mit Gerschgorim folgt, dass zu jedem Eigenwert $\lambda$ von $A$ ein Diagonaleintrag $a_{ii}$ existiert mit
	\begin{align*}
		\abs{\lambda - a_{ii}} \leq \sqrt{n-1}\sqrt{\cN(A)}
	\end{align*}
	Man kann sogar zeigen, dass 
	\begin{align*}
		\abs{\lambda - a_{ii}} \leq \sqrt{\cN(A)}.
	\end{align*}
	\begin{definition}
		Für $c,s \in \bR$ mit $c^2 + s^2 = 1$, $1 \leq p$, $q \neq n$ definieren wir
		die \tbf{Givens-Rotation} $G_{pq} \in \cO(n)$ durch:
		\begin{align*}
			(G_{pq})_{ij} =
			\begin{cases}
				1 & i = j, (i \neq p) \vee (i \neq q)\\
		...
		\begin{itemize}
			\item $(G_{pq})_{pp} = (G_{pq})_{qq} = c$,
			\item $(G_{pq})_{pq} = -s$,
			\item $(G_{pq})_{qp} = s$.
		\end{itemize}   
	\end{definition}
	\begin{proposition}
		$G_{pq}$ rotiert einen Vektor in der durch $e_p$, $e_q$ aufgespannten Ebene um den Winkel $\alpha$ mit $c = \cos(\alpha)$, $s = \pm\sin(\alpha)$.
	\end{proposition}
	\begin{theorem}
		Ist $A \in \bRnn$ symmetrisch und $G_{pq}$ eine Givens-Rotation, so gilt für
		\begin{align*}
			B = G_{pq}^\top A G_{pq},
		\end{align*}
		dass:
		\begin{align*}
			\cN(B) = \cN(A) - 2(a_{pq}^2 - b_{pq}^2),
		\end{align*}
		wobei 
		\begin{align*}
	...
	\begin{lemma}
		...
		\begin{align*}
			c = \sqrt{\frac{1 + D}{2}}, \quad s = -\sgn(a_{pq})\sqrt{\frac{1-D}{2}},
		\end{align*} wobei
		\begin{align*}
	...
	\end{lemma}
	\begin{theorem}
		Ist $a_{pq}$ das betragsmäßig größte Außerdiagonalelement von $A$, so gilt mit der Givens-Rotation $G_{pq}$ aus dem vorherigen Lemma für
		\begin{align*}
			B = G_{pq}^\top A G_{pq}
		\end{align*}
		und 
		\begin{align*}
			\epsilon_n = \frac{2}{n(n-1)},
		\end{align*}
		dass
		\begin{align*}
			\cN(B) \leq (1 - \epsilon_n)\cN(A)
		\end{align*}
	\end{theorem}
	Wir erhalten den folgenden Algorithmus:
	\begin{theorem}
		\theoremname{(Jacobi-Verfahren)}
		Sei $A \in \bRnn$ symmetrisch. Setze $A_0 = A$ und $k = 0$.
		\begin{enumerate}
			\item Seien $p,q$ die Indizes des betragsmäßig größten Außerdiagonalelements $a_{pq}$ von $A_k$.
			\item Sei $G_{pq}$ die Givens-Rotation zu
			\begin{align*}
				c = \sqrt{\frac{1 + D}{2}}, \quad s = -\sgn(a_{pq})\sqrt{\frac{1-D}{2}},
			\end{align*}
			\begin{align*}
				D = \frac{a_{pp} - a_{qq}}{\sqrt{(a_{pp} - a_{qq})^2 + 4a_{pq}^2}},
			\end{align*}
			\item Setze $A_{k+1} = G^\top_{pq}A_kG_{pq}$.
			\item Stoppe, falls $\cN(A_{k+1}) \leq \epsilon$, sonst setze $k \gets k+1$ und gehe zu $1$.
		\end{enumerate}
	\end{theorem}
	\begin{proposition}
		\begin{enumerate}
			\item Im Allgemeinen werden $\cO(n^2 \log \frac{1}{\epsilon})$ Iterationen benötigt.
			\item Ein auf Null transponierter Eintrag kann später wieder von $0$ abweichen.
			\item Suchen des größten Außerdiagonaleintrags ist aufwändig! In der Praxis iteriert man über alle Außerdiagonaleinträge, deren Betrag über einem Schwellwert liegen ("zyklisches Jacobiverfahren").        
		\end{enumerate}
	\end{proposition}
	\chapter{Iterative Lösungsverfahren}
	Eine Klasse von interativen Verfahren zur Lösung linearer Gleichungssysteme basiert auf dem Banachschen Fixpunktsatz.
	\section{Lineare Iterationsverfahren}
	Sei nun $\Phi(x) = Mx + s$ affin mit $M \in \bRnn, s \in \bR^n$. Es gilt:
	\begin{align*}
		\norm{\Phi(x) - \Phi(y)} &= \abs{M(x-y)} \leq \norm{M}_{op} \norm{x-y},
	\end{align*}
	also ist $\Phi$ eine Kontraktion, wenn eine Operatornorm $\norm{-}_{op}$ existiert, sodass $\norm{M}_{op} < 1$.
	\begin{theorem}
		Sei $M \in \bRnn$. Dann ist der Spektralradius $\rho(M)$, also der betragsmäßig maximalen Eigenwert von $M$, das Infimum aller möglichen Operatornormen von $M$:
		\begin{align*}
			\rho(M) = \inf_{\norm{-}_{op}} \lr{\norm{M}_{op}}
		\end{align*}
	\end{theorem}
	\begin{corollary}
		Sei $\rho(M) \leq 1$. Dann ist $\Phi : x \mapsto Mx + b$ eine Kontraktion.
	\end{corollary}
	\begin{example}
		Das Richardson-Verfahren zur approximativen Lösung eines linearen Gleichungssystem $Ax = b$ ist für $\omega > 0$ definiert durch $M = E_n - \omega A$, $c = \omega b$, also:
			\begin{align*}
				x^{k+1} = M^k x^{k} + c = x^k - \omega (Ax^k - b)
			\end{align*}
			Ist $x^*$ ein Fixpunkt dieses Verfahrens, so gilt $Ax^* = b$.
			\newpar
			Falls $A$ symmetrisch und positiv definit ist, sind alle Eigenwerte von $A$ positiv, und für $\omega$ hinreichend klein ist $\rho(M) < 1$, also ist das Verfahren eine Kontraktion.
			Allerdings ist die Konvergenz extrem langsam, also wird das Verfahren in der Praxis nicht benutzt.
	\end{example}
	\section{Jacobi- und Gauß-Seidel-Verfahren}
	Für $A \in \bRnn$ betrachten wir $D,L,U \in \bRnn$, sodass $D$ die Diagonale von $A$ ist, $L$ der "untere Dreiecksteil", und $U$ der "obere Dreiecksteil". Mit $A = L + D + U$ gilt
	\begin{align*}
		Ax = b \Longleftrightarrow Lx + Dx + Ux = b
	\end{align*}
	Wir erhalten ein Iterationsverfahren, indem wir $x$ durch $x^{k+1}$ bzw. $x^k$ ersetzen. Zum Beispiel:
	\begin{align*}
		Lx^k + Dx^{k+1} + Ux^k = b \Leftrightarrow x^{k+1} &= -D^{-1}(L + U)x^k + D^{-1}b\\
		&= -D^{-1}(A - D)x^k + D^{-1}b
	\end{align*}
	Wieder löst ein Fixpunkt $x^*$ trivial das Gleichungssystem.
	\begin{definition}
		Das \tbf{Jacobi-Verfahren} ist gegeben durch:
		\begin{align*}
			M^J = -D^{-1}(A - D),\qquad c^J = D^{-1}b
		\end{align*}
	\end{definition}
	\begin{definition}
		Das \tbf{Gauß-Seidel-Verfahren} ist gegeben durch:
		\begin{align*}
			M^{GS} = -(L + D)^{-1} U,\qquad c^{GS} = (L + D)^{-1}b
		\end{align*}
	\end{definition}
	In beiden Fällen muss in jedem Schritt ein einfaches lineares Gleichungssystem gelöst werden. Beide Verfahren sind nicht durchführbar, falls auf der Diagonalen von $A$ eine Null steht, da dann $D$ und $L + D$ nicht invertierbar sind.
	\section{Diagonaldominanz \& Irreduzibilität}
	\begin{definition}
		$A \in \bRnn$ heißt \tbf{diagonaldominant}, falls für $i = 1, \hdots, n$ die Ungleichung
		\begin{align*}
			\sum_{j = 1, \hdots, n, j \neq i} \abs{a_{ij}} \leq \abs{a_{ij}}
		\end{align*}
		gilt
		\tbf{und} ein $i$ existiert, sodass die Ungleichung strikt ist. Ist die Ungleichung für alle $i$ strikt, heißt $A$ \tbf{strikt diagonaldominant}.
	\end{definition}
	\begin{corollary}
		\begin{enumerate}
			\item Ist $A$ strikt diagonaldominant, so ist keiner der Diagonaleinträge $0$, also ist die Diagonale invertierbar.
			\item Für strikt diagonaldominantes $A$ gilt $\norm{M^J}_\infty < 1$, also $\rho(M^J) < 1$, also konvergiert das Jacobi-Verfahren.
			\item Für strikt diagonaldominantes $A$ gilt außerdem $\rho(M^{GS}) \leq \rho(M^J)$.
		\end{enumerate}
	\end{corollary}
	Leider ist strikte Diagonaldominanz selten.
	\begin{definition}
		Eine Matrix $A \in \bRnn$ heißt \tbf{reduzibel}, falls nichtleere, disjunkte Indexmengen $I,J$ existieren, sodass $I \cup J = \lr{1, \hdots, n}$ und für alle $(i,j) \in I \times J$
		\begin{align*}
			a_{ij} = 0
		\end{align*}
	\end{definition}
	\begin{example}
		Eine $2 \times 2$-Matrix ist genau dann reduzibel, wenn mindestens einer der Einträge außerhalb der Diagonalen $0$ ist.
	\end{example}
	\begin{proposition}
		Für reduzible Matrizen lässt sich das lösen linearer Gleichungssysteme $Ax = b$ Zerlegen: Für $X,Y \subset \lr{1, \hdots, n}$ sei $A_{xy} = (a_{ij})_{i \in X, j \in Y}$. und $x_y = (x_k)_{k \in Y}$. Dann gilt:
		\begin{align*}
			A_{II}x_I = b_I, \qquad A_{JJ}x_J = b_J - A_{JI}x_I
		\end{align*}
	\end{proposition}
	\begin{lemma}
		Ist $M$ irreduzibel und diagonaldominant, so ist $M$ regulär und alle Diagonaleinträge sind von $0$ verschieden.
	\end{lemma}
	\section{Das Simplex-Verfahren}
	\subsection{Lineare Programmierung}
	\subsection{Der Simplex-Schritt}
	\begin{enumerate}
		\item Sei $x \in M$ eine Ecke. Falls diese Entartet ist, so können wir $I_x$ so zu einer $m$-elementigen Mengen $I$ ergänzen, dass $A_I$ regulär ist. Wir setzen dann $J = \lr{1, \hdots, n} \setminus I$.
		\item Für alle $z \in M$ folgt aus $b = Az = A_Iz_I + A_Jz_J$, dass
		\begin{align*}
			z_I = A_I^{-1}b - A_I^{-1}A_Jz_J
		\end{align*}
		Somit sind alle Komponenten bezüglich $I$ eindeutig durch die Komponenten bezüglich $J$ festgelegt.
		
		Wegen $x_J = 0$ folgt insbesondere $xQ_I = A_I^{-1}b$.
		
		Für $f(z) = c^\top z$ folgt
		\begin{align*}
			c^\top z
			&= c_I^\top z_I + c_J^\top z_J\\
			&= c_I^\top(A_I^{-1}b - A_I^{-1}A_Jz_J) + c_J^\top z_J\\
			&= c_I^\top x_I + (c_J^\top - c_I^\top A_I^{-1}A_J)z_J\\
			&= c^\top x + (c_J - A_J^\top A_I^{-\top} c_I)^\top z_J
		\end{align*}
		mit $u_j = c_J - A_J^\top A_I^{-\top}c_I$ gilt also
		\begin{align*}
			f(z) = f(x) + u_J^\top z_J
		\end{align*}
		Da $z \geq 0$ ist eine Verringerung also nur dann möglich, wenn mindestens eine Komponente von $u_j$ negativ ist. Ansonsten ist $x$ bereits eine lokale (und somit auch globale) Lösung des Problems.
	\end{enumerate}
	\chapter{Konditionszahlen und Gleitkommaarithmetik}
	Gegeben sei als "mathematische Aufgabe" die Approximation einer Funktion $\Phi(x) : X \to Y$ zwischen normierten Vektorräumen.
	\begin{definition}
		Die relative Konditionszahl $\kappa_\Phi(x)$ der Funktion $\Phi : X \to Y$ für $x \neq 0, \Phi(x) \neq 0$ ist gegeben durch $$\inf \lr{\kappa > 0 : \exists \delta > 0 : \forall \frac{\norm{h}}{\norm{x}} < \delta : \frac{\norm{\Phi(x + h) - \Phi(x)}}{\norm{\Phi(x)}} < \kappa \frac{\norm{h}}{\norm{x}}}$$
	\end{definition}
	(Also die Lipschitzkonstante!)
	\newpar
	Ist $\kappa_\Phi(x) \gg 1$, heißt $\Phi(x)$ schlecht konditioniert. Ist $K_\Phi(x)$ nicht definiert, heißt $\Phi(x)$ schlecht gestellt.
	\begin{theorem}
		Falls $\Phi$ differenzierbar ist, gilt
		\begin{align*}
			\kappa_\Phi(x) = \frac{\norm{D \Phi} \cdot \norm{x}}{\norm{\Phi(x)}}
		\end{align*}
	\end{theorem}
	\begin{example}
		\begin{enumerate}
			\item $\Phi(x) = \abs{x-1}^s + 1$ ist bei $x = 1$ für $s \geq 1$ gut konditioniert.
			\item $\Phi(x) = \sgn(x - 1)$ ist schlecht gestellt bei $x = 1$.
			\item Für $\Phi(b) = A^{-1}(b)$ gilt $\kappa_\Phi(b) = \cond(A)$.
			\item Für $\Phi(A) = A^{-1}(b)$ gilt ebenfalls $\kappa_\Phi(b) = \cond(A)$!
			\item Auslöschungseffekte werden erfasst: Für $\Phi(x,y) = x + y$ gilt $D \Phi = (1,1)$, also
			\begin{align*}
				\kappa_\Phi(x,y) = \frac{\abs{x} + \abs{y}}{\abs{x + y}}
			\end{align*}
		\end{enumerate}
	\end{example}
	\begin{definition}
		Für eine Basis $b \geq 2$, Präzision $p \geq 1$, und Exponentenschranken $e_{\min} \leq e_{\max}$ mit $b,p,e_{\min},e_{\max} \in \bZ$ ist die Menge der \tbf{Gleitkommazahlen} gegeben durch
		\begin{align*}
			G = \lr{\pm m \cdot b^{e-p} : m,e \in \bZ; 0 \leq m \leq b^p-1; e_{\min} \leq e \leq e_{\max}}.
		\end{align*}
		Eine Gleitkommazahl $g \in G$ heißt \tbf{normalisiert}, falls $m \geq b^{p-1}$ (also falls die vorderste Ziffer nicht Null ist).
		\newpar
		Gleitkommazahlen lassen sich trivial \tbf{$b$-adisch} darstellen:
		\begin{align*}
			g = \pm (d_1 b^{-1} + \hdots + d_p b^{-p}) \cdot b^e
		\end{align*}
	\end{definition}
	\begin{definition}
		Sei $g_\epsilon \in G$ die kleinste Gleitkommazahl, für die gilt $g_\epsilon > 1$. Dann ist $g_\epsilon - 1$ die sog. Maschinengenauigkeit (oder auch einfach \tbf{Rundungsfehler} $m_\epsilon$).
	\end{definition}
	\begin{definition}
		Für eine Menge an Maschinenzahlen heißt eine Abbildung
		\begin{align*}
			r : \bR \supset [-g_{\min},g_{\max}] \to G
		\end{align*}
		eine \tbf{Rundungsabbildung}, falls für jede Zahl $x \in [-g_{\max}, g_{\max}]$ gilt, dass
		\begin{align*}
			\abs{x - r(x)} = \min_{g \in G} \abs{x - g}
		\end{align*}
	\end{definition}
	\begin{lemma}
		Für jedes $x \in \bR$ mit $\abs{x} \in [-g_{\min}, g_{\max}]$ gilt
		$$\frac{\abs{x-r(x)}}{\abs{x}} \leq \frac{1}{2}m_\epsilon$$
	\end{lemma}
	\begin{definition}
		Das sogenannte Standardmodell der Gleitkommaarithmetik fordert, dass für $x,y \in \bR$, $\abs{x}, \abs{y} \leq g_{\max}$, und jede arithmetische Standardoperation $\circ \in \lr{+,-,\cdot,\frac{\cdot}{\cdot}}$ mit $\abs{x \circ y} < g_{\max}$ und der jeweiligen numerischen Realisierung $\circ_G$ gilt, dass ein $\delta \in \bR$ mit $\delta < \frac{m_\epsilon}{2}$ existiert, sodass
		\begin{align*}
			r(x) \circ_G r(y) = (x \circ y)(1 + \delta)
		\end{align*}
	\end{definition}
	(...)
