\documentclass{report}

% custom margins
\usepackage[a4paper,margin=1.5in]{geometry}
\renewcommand{\baselinestretch}{1.2}
%\AddToHook{cmd/section/before}{\clearpage}

% emma's long list of custom macros and universally used packages
\include{../macros-and-packages.tex}

% colored box behind proofs
\tcolorboxenvironment{proof}{
	colback=white,
	boxrule=0pt,
	frame hidden,
	borderline west={1pt}{0pt}{black},
	before skip=0.75cm,
	after skip=0.75cm,
	sharp corners,
	breakable,
	enhanced,
}

\renewcommand*\contentsname{Inhalt}
\renewcommand*\proofname{Beweis}

\pagestyle{fancy} %allows headers

\lhead{Emma Bach}
\rhead{\today}

\begin{document}

\include{title}
\tableofcontents
\thispagestyle{fancy}

\chapter{Elementare Zahlentheorie}
\section{Primzahlen}
\begin{definition}
	$H \subseteq \bZ$ heißt Untergruppe von $\bZ$, wenn 
	\begin{align*}
		n \in H \implies -n \in H,
	\end{align*} und 
	\begin{align*}
		m, n \in H \implies m + n \in H
	\end{align*}
\end{definition}
\begin{theorem}
	Es gibt eine Bijektion 
	\begin{align*}
		\bN &\leftrightarrow \{\textnormal{Untergruppen von ($\bZ, + $)}\}\\
		n &\mapsto n\bZ
	\end{align*}
\end{theorem}
\begin{proof}
	Sei $H \subseteq \bZ$ eine Untergruppe. Entweder $H = \{0\} = 0\bZ$, oder $H \neq \{0\}$.
	\newpar
	Sei also $H \neq \{0\}$. Dann gibt es ein kleinstes Element $m$ der Menge $\{n \in H \mid n > 0\}$.
	Aus den Untergruppenaxiomen folgt $m\bZ \subseteq H$. Gleichzeitig kann kein Element $n \notin m \bZ$ in $H$ enthalten sein, denn sonst wäre auch $r = n \mod m \neq 0$ in $H$ enthalten. Dann hätten wir aber $r < m$, was ein Widerspruch ist.
	\newpar
	Als Umkehrfunktion wählen wir das kleinste positive Element von $H$.
\end{proof}
\begin{definition}
	Eine \tbf{Primzahl} ist eine natürliche Zahl $p \in \bN_{\geq 2}$, die nicht als Produkt zweier Zahlen $a,b < p$ geschrieben werden kann.
\end{definition}
\begin{theorem}
	Es gibt unendlich viele Primzahlen.
\end{theorem}
\begin{proof}
	Angenommen, es gäbe endlich viele Primzahlen. Sei also $p_1, \hdots, p_r$ eine vollständige Liste aller Primzahlen. Dann wäre aber
	\begin{align*}
		q = 1 + \prod_{i = 1}^{r} p_i
	\end{align*}
	durch keine Primzahl teilbar, also selbst eine Primzahl. Widerspruch!
\end{proof}
\begin{theorem}
	Jede Zahl $n \in \bN$ kann als Produkt von Primzahlen geschrieben werden:
	\begin{align*}
		n = p_1 \cdot \hdots \cdot p_r \quad (r \geq 0)
	\end{align*}
\end{theorem}
\begin{proof}
	Der Fall $n = 1$ gilt per Konvention durch das leere Produkt.
	\newpar
	Sei $n \geq 2$ gegeben. Es gilt entweder:
	\begin{itemize}
		\item $n$ ist eine Primzahl.
		\item $n$ ist von der Gestalt $n = a \cdot b$, mit $a,b < n$.
	\end{itemize}
	Der Satz folgt durch Induktion über die entstehende Baumstruktur - nach Induktionsannahme haben $a$ und $b$ eine Primfaktorzerlegung. Also hat auch $n$ eine Primfaktorzerlegung. 
\end{proof}
\begin{definition}
	Der \tbf{größte gemeinsame Teiler} von $a,b \in \bZ$ mit $a \neq 0$ oder $b \neq 0$ ist die Zahl:
	\begin{align*}
		\ggT(a,b) = \max\{d \in \bN : d \mid a \wedge d \mid b\}
	\end{align*}
\end{definition}
\begin{theorem}
	\theoremname{Über den größten gemeinsamen Teiler}: Seien $a,b \in \bZ$. So gibt es $r,s \in \bN$ mit 
	\begin{align*}
		\ggT(a,b) = ra + sb
	\end{align*}
	Gegeben $d \mid a$ und $d \mid b$ gilt außerdem $d \mid \ggT(a,b)$.
\end{theorem}
\begin{proof}
	Die Menge
	\begin{align*}
		H := \{ra + sb \mid r,s \in \bZ\} = a\bZ + b\bZ
	\end{align*}
	bildet eine Untergruppe von $Z$, ist also eine Gruppe der Form $m \bZ$ mit $m > 0$. Da $a \in m\bZ$ und $b \in m\bZ$ ist $m$ ein gemeinsamer Teiler. Da $m$ ein Element in $H$ ist existiert außerdem per Definition eine Darstellung $m = r'a + s'b$. Es gilt also 
	\begin{align*}
		(d \mid a) \wedge (d \mid b) \implies d \mid r'a + s'b \implies d \mid m.
	\end{align*}
	\newpar
	Aus $(d \mid a) \wedge (d \mid b) \implies d \mid m$ folgt nun, dass jeder Teiler von $a$ und $b$ $m$ teilt, also kann es keinen Teiler von $a$ und $b$ geben, welcher größer als $m$ ist.
\end{proof}
\noindent Die Existenz der Darstellung $\ggT(a,b) = ra + sb$ ist auch als das \tbf{Lemma von Bézout} oder die \tbf{Bézoutsche Identät} bekannt.
\begin{lemma}
	\theoremname{Lemma von Euklid:} Sei $p$ eine Primzahl und $a,b \in \bZ$.
	Dann gilt:
	\begin{align*}
		p \mid ab \implies (p \mid a) \vee (p \mid b)
	\end{align*}
\end{lemma}
\begin{proof}
	Es reicht zu Zeigen:
	\begin{align*}
		(p \nmid a) \wedge (p \mid ab) \implies p \mid b
	\end{align*}
	Aus $p \nmid a$ folgt $\ggT(p,a) = 1$. Nach dem Lemma von Bézout können wir also $1$ darstellen als:
	\begin{align*}
		1 = rp + sa
	\end{align*}
	also:
	\begin{align*}
		b = rpb + sab
	\end{align*}
	Es gilt trivial $p \mid prb$, außerdem gilt per Annahme $p \mid ab$. Es folgt $p \mid rpb + sab = b$.
\end{proof}
\begin{theorem}
	\theoremname{Eindeutigkeit der Primfaktorzerlegung im Ring $\bZ$:}
	Sei $n \in \bN_{\geq 1}$ und
	\begin{align*}
		n = \prod_{i = 1}^r p_i = \prod_{i = 1}^s q_i
	\end{align*}
	wobei alle $q_i$ und $r_i$ Primzahlen sind. So gilt $r = s$ und es gilt eine Permutation $\sigma \in S_r$ mit $p_i = q_{\sigma(i)}$.
\end{theorem}
\noindent Äquivalente Formulierungen:
\begin{itemize}
	\item Falls die $p_i$ und $q_i$ Aufsteigend oder Absteigend sortiert sind, gilt $\forall i : p_i = q_i$
	\item Es existiert eine Bijektion zwischen endlichen Multimengen von Primzahlen und $\bN_{\geq 1}$.
\end{itemize} 
\begin{proof}
	Per Induktion folgt aus dem Lemma von Euklid schnell:
	\begin{align*}
		p_1 \mid p_1 \implies \bigvee_{i = 1}^s p_1 \mid q_i
	\end{align*}
	Also existiert ein $q_i$ mit $p_1 = q_i$. Teilen wir nun beide Seiten durch $p_1$, folgt die Aussage durch die Induktionsannahme.
\end{proof}
\begin{definition}
	Euklidischer Algorithmus
\end{definition}
\chapter{Gruppen}
\begin{definition}
	\phantom{}
	\begin{enumerate}
		\item Eine Menge $M$ mit Verknüpfung $\top : M \times M \to M$ heißt \tbf{Magma}.
		\item Ein Magma mit neutralem Element heißt \tbf{unitäres Magma}.
		\item Ein unitäres Magma mit assoziativer Verknüpfung heißt \tbf{Monoid}.
		\item Ein Monoid mit inversen Elementen heißt \tbf{Gruppe}.
		\item Eine Gruppe mit kommutativer Gruppe heißt \tbf{kommutative Gruppe} oder \tbf{abelsche Gruppe}.
	\end{enumerate}
\end{definition}
\begin{example}
	\phantom{}
	\begin{enumerate}
		\item $(\bZ, -)$ ist ein nichtunitäres Magma.
		\item ?
		\item $(\bN, +)$ und $(\bN, \cdot)$ sind Monoide.
		\item $(\bZ, +)$ ist eine abelsche Gruppe.
	\end{enumerate}
\end{example}
\begin{theorem}
	Das neutrale Element einer Gruppe ist eindeutig.
\end{theorem}
\begin{itemize}
	\item Es gibt keine Gruppe mit $0$ Elementen, da die Leere Menge kein neutrales Element hat.
	\item Es gibt genau eine Gruppe mit einem Element, die Gruppe $\{e\}$.
\end{itemize}
\section{Erste Versuche der Klassifikation}
\begin{theorem}
	Es gibt genau eine Gruppe mit zwei Elementen, nämlich die Gruppe $\{e,a\}$, mit Verknüpfungstabelle:
	\begin{align*}
		\begin{array}{c|c|c}
			&e&a\\\hline
			e&e&a\\\hline
			e&a&e\\
		\end{array}
	\end{align*}
\end{theorem}
\begin{proof}
	Durch die Definition von $e$ sind drei Teile der Tabelle schon eindeutig festgelegt. $a \cdot a = e$, da sonst $a$ kein Inverses hätte.
\end{proof}
\begin{theorem}
	Gilt für ein Gruppenelement $a \in G$ $a^2 = a$, so gilt $a = e$.
\end{theorem}
\begin{proof}
	\begin{align*}
		a = ae = aaa^{-1} = aa^{-1} = e
	\end{align*}
\end{proof}
\begin{theorem}
	Aus $a \cdot x = a \cdot y$ folgt $x = y$.
\end{theorem}
\begin{proof}
	\begin{align*}
		ax &= ay\\
		a^{-1}ax &= a^{-1}ay\\
		ex &= ey\\
		x &= y
	\end{align*}
\end{proof}
Es folgt, dass in einer Gruppentabelle in jeder Spalte und jeder Zeile kein Element doppelt vorkommen kann.
\begin{corollary}
	Es gibt nur eine Gruppe mit drei Elementen, nämlich:
	\begin{align*}
		\begin{array}{c|c|c|c}
			&e&a&b\\\hline
			e&e&a&b\\\hline
			a&a&b&e\\\hline
			b&b&e&a\\
		\end{array}
	\end{align*}
\end{corollary}
\begin{theorem}
	Es gibt zwei Gruppen mit vier Elementen, nämlich:
	\begin{align*}
		\begin{array}{c|c|c|c|c}
			 &e&a&b&c\\\hline
			e&e&a&b&c\\\hline
			a&a&e&c&b\\\hline
			b&b&c&a&e\\\hline
			c&c&b&e&a
		\end{array} &\simeq (\bZ / 2\bZ) \times (\bZ / 2\bZ)\\
		\begin{array}{c|c|c|c|c}
			&e&a&b&c\\\hline
			e&e&a&b&c\\\hline
			a&a&b&c&e\\\hline
			b&b&c&e&a\\\hline
			c&c&e&a&b
		\end{array} &\simeq \bZ / 4\bZ
	\end{align*}
\end{theorem}
\begin{definition}
	Ein Gruppenhomomorphismus ist eine Abbildung
	\begin{align*}
		\phi : (G, \top) \to (H, \bot)
	\end{align*}
	mit
	\begin{align*}
		\phi(a \top b) = \phi(a) \bot \phi(b)
	\end{align*}
	Wir schreiben die Menge der Gruppenhomomorphismen von $G$ nach $H$ als $\Grp(G,H)$.
\end{definition}
\begin{theorem}
	$\Grp((\bZ, +), G) \simeq G$
\end{theorem}
\begin{proof}
	Ordne jedem Gruppenelement $g$ die Abbildung $\phi(n) : g \to g^n$ zu.
\end{proof}
\begin{theorem}
	Sind $G$ und $H$ Gruppen, so gilt $\Mag(G,H) = \Mon(G,H) = \Grp(G,H)$.
\end{theorem}
\begin{definition}
	Eine Teilmenge $H \subseteq G$ einer Gruppe $G$ heißt \tbf{Untergruppe}, wenn sie so mit einer Gruppenstruktur versehen werden kann, dass die Einbettungsabbildung $i : H \to G$ ein Gruppenhomomorphismus ist.
\end{definition}
\noindent Dies ist eine sehr allgemeine Definition, die analog für Untervektorräume, Untermagmas etc. funktioniert. In der Praxis verwendet man meistens folgendes Kriterium:
\begin{theorem}
	$H$ ist genau dann eine Untergruppe, wenn $e \in H$ und wenn für jedes $a,b \in H$ auch $ab \in H$ und $a^{-1}, b^{-1} \in H$.
\end{theorem}
\begin{theorem}
	\theoremname{Satz von Lagrange:} Sei $G$ eine endliche Gruppe und $H$ eine Untergruppe. So ist die Zahl der Elemente von $H$ ein Teiler der Zahl der Elemente von $G$.
\end{theorem}
\noindent Der Beweis folgt nach einigen weitern Definitionen und Sätzen.
\begin{definition}
	Sei $G$ eine Gruppe mit Untergruppe $H$. Wir definieren:
	\begin{align*}
		gH &= \{gh \mid h \in H\} \subseteq G\\
		G / H &= \{gH \mid g \in G\} \subseteq \cP(G)
	\end{align*}
\end{definition}
\begin{lemma}
	Sei $h \in H$. Dann gilt $hH = H$.
\end{lemma}
\begin{lemma}
	Je zwei Nebenklassen $gH$ und $g'H$ sind entweder gleich oder disjunkt.
\end{lemma}
\noindent Aus diesen beiden Lemmas folgt $\abs{G} = \abs{G / H} \cdot \abs{H}$, was den Satz von Lagrange impliziert.
\begin{theorem}
	Der Schnitt beliebig vieler Untergruppen bildet eine Untergruppe.
\end{theorem}
\begin{definition}
	Sei $G$ eine Gruppe und $T \subseteq G$ eine Teilmenge. Die \tbf{durch $T$ erzeugte Untergruppe} $\angles{T}$ ist die kleinste Untergruppe, welche $T$ enthält.
\end{definition}
\noindent $\angles{T}$ besteht aus allen Elementen von $G$, welche durch beliebig häufige Anwendung von Inversionen und Gruppenoperationen entstehen kann, wobei wir die "leere Gruppenoperation" als $e$ definieren (für den Fall $\angles{\emptyset} = \{e\}$.)
\begin{theorem}
	Sei $\varphi : G \to H$ ein Gruppenhomomorphismus. Sei $A$ eine Untergruppe von $G$ und $B$ eine Untergruppe von $H$. So ist $\phi(A)$ eine Untergruppe von $H$ und $\phi^{-1}(B)$ eine Untergruppe von $G$.
\end{theorem}
\begin{theorem}
	\theoremname{Universelle Eigenschaft surjektiver Gruppenhomomorphismen:} Sei $\phi : G \surj H$ ein surjektiver Gruppenhomomorphismus. Sei $\psi : G \to K$ ein Gruppenhomomorphismus, sodass $\ker \psi \supseteq \ker \phi$. So existiert genau ein Gruppenhomomorphismus $\ol{\psi}$, sodass:
	\begin{align*}
		\psi = \ol{\psi} \circ \phi
	\end{align*}
	Also sodass das folgende Diagramm kommutiert:
	\[\begin{tikzcd}
		G & H \\
		&\\
		{} & K
		\arrow["\phi", two heads, from=1-1, to=1-2]
		\arrow["\psi"', from=1-1, to=3-2]
		\arrow["{\exists!\overline{\psi}}", dashed, from=1-2, to=3-2]
	\end{tikzcd}\]
\end{theorem}
\begin{proof}
	Sei $\phi(x) = \phi(y)$, also $\phi^{-1}(\phi(x)) = \phi^{-1}(\phi(y))$. Dann gilt 
	\begin{align*}
		\phi(xy^{-1}) = \phi(1) = 1,
	\end{align*} 
	also $xy^{-1} \in \ker \phi$, also $y \in x \cdot \ker \phi$. 
	Somit gilt \begin{align*}
		\phi^{-1}(\phi(g)) = g \cdot \ker \phi.
	\end{align*} 
	Es gilt außerdem
	\begin{align*}
		\psi(g \cdot \ker \phi) = \psi(g) \cdot \psi(\ker(\phi)) = \phi(g),
	\end{align*}
 	also ist $\psi$ konstant auf den Fasern (also den Urbildern einzelner Elemente) von $\phi$. Somit existiert ein $\ol{\psi}$ Aufgrund der "universellen Eigenschaft surjektiver Funktionen", welche noch nicht eingeführt wurde :), folgt die Existenz eines $\ol{\psi}$ mit den gefragten Eigenschaften.
 	\newpar
 	Zu zeigen ist noch, dass $\ol{\psi}$ ein Gruppenhomomorphismus ist.\\
 	Wähle hierfür $g \in G, g' \in G$ und $\phi(g) = h$ und $\phi(g') = h'$. Dann gilt
 	\begin{align*}
 		\ol{\psi}(hh') = \ol{\psi}(\phi(gg')) = \psi(gg') = \psi(g) \cdot \psi(g') = \ol{\psi}(h) \cdot \ol{\psi}(h')
 	\end{align*}
\end{proof}
% https://q.uiver.app/#q=WzAsNCxbMSwwLCJHIl0sWzAsMiwiSCJdLFsxLDIsIksiXSxbMiwyLCJIIl0sWzAsMSwiXFxwaGkiXSxbMCwyLCJcXHBzaSIsMl0sWzAsMywiXFxwaGkiLDJdLFsxLDIsIlxcZXhpc3RzIVxcb3ZlcmxpbmV7XFxwc2l9Il0sWzIsMywiXFxleGlzdHMhXFxvdmVybGluZXtcXHBoaX0iXSxbMSwzLCJcXGlkX0giLDEseyJjdXJ2ZSI6Mn1dXQ
\[\begin{tikzcd}
	& G \\
	\\
	H & K & H
	\arrow["\phi", from=1-2, to=3-1]
	\arrow["\psi"', from=1-2, to=3-2]
	\arrow["\phi"', from=1-2, to=3-3]
	\arrow["{\exists!\overline{\psi}}", dashed, from=3-1, to=3-2]
	\arrow["{\exists!\overline{\phi}}", dashed, from=3-2, to=3-3]
\end{tikzcd}\]
Die "Moral" dieses Satzes: Ein surjektiver Gruppenhomomorphismus $\phi : G \surj H$ wird durch $(G, \ker \phi)$ "im wesentlichen eindeutig" festgelegt.
\newpar
Die Frage ist nun, ob jede Untergruppe einer Gruppe $G$ als Kern eines surjektiven Gruppenhomomorphismus festgelegt ist. Die Antwort auf diese Frage: Nein! Die einzigen Untergruppen, welche als solche Kerne auftreten, sind sogenannte \tbf{Normalteiler}.
\begin{definition}
	Sei $G$ eine Gruppe. Eine Untergruppe $N \subseteq G$ heißt \tbf{Normalteiler}, falls
	\begin{align*}
		\forall g \in G : gN = Ng
	\end{align*}
\end{definition}
\begin{example}
	Wir betrachten die Gruppe $Q \subset GL(2,\bR)$ aller linearen Abbildungen, welche ein Quadrat auf sich selbst abbilden. Prof. Soergel nennt diese Gruppe auch gerne die "Bierdeckelgruppe $B$". Die Gruppe hat acht Elemente:
	\begin{itemize}
		\item Die Rotationen $d_0$, $d_1$, $d_2$ und $d_3$ um Vielfache von $90$°,
		\item die Spiegelungen $s_{x}$ und $s_{y}$ an den Koordinatenachsen,
		\item und die Spiegelungen $s_+$ und $s_-$ an den Diagonalachsen.
	\end{itemize}
	Diese Gruppe ist nicht kommutativ! Zum Beispiel ist $d_1s_+ = s_y$, aber $s_+ d_1 = s_x$ (Wir notieren die Elemente als Abbildungen, also wird links zuerst $s_+$ und dann $d_1$ angewandt.
	\newpar
	Wir betrachten nun die Untergruppe $H = \angles{s_+}$. So gilt $d_1H = \{s_y, d_1\}$, aber $Hd_1 = \{s_x, d_1\}$
\end{example}
\begin{theorem}
	Der Kern eines Gruppenhomomorphismus ist immer ein Normalteiler.
\end{theorem}
\begin{proof}
	Sei $\phi : G \to H$. So ist $(\ker \phi) x = \phi^{-1}(\phi(x)) = x (\ker \phi)$.
\end{proof}
\begin{theorem}
	Für jeden Normalteiler $N \subseteq G$ gibt es einen surjektiven Gruppenhomomorphismus $\phi : G \surj H$ mit $\ker \phi = N$.
\end{theorem}
\begin{proof}
	Wir wählen $H := G / N = \{xN : x \in G\}$. Für je zwei Teilmengen $A, B \subseteq G$ definieren wir $AB = \{ab : a \in A, b \in B\}$ und erhalten eine assoziative Verknüpfung auf $\cP(G)$. Für einen Normalteiler $N \subseteq G$ ist $G / N$ unter dieser Verknüpfung stabil (geschlossen), denn durch Abuse of Notation folgt $xNyN = xyNN = xyN$. Unter dieser Verknüpfung wird $G/N$ eine Gruppe mit neutralem Element $1N = N$ und $x^{-1}N = (xN)^{-1}$.
	\newpar
	Dadurch wird dann $\phi : G \to G/N$, $x \to xN$ ein Gruppenhomomorphismus mit $\ker \phi = N$.
\end{proof}
\begin{example}
	Die Menge $D \subsetneq Q$ der Drehungen eines Quadrats ist ein Normalteiler der vollen Symmetriegruppe $Q$.
\end{example}
\begin{proof}
	Sei $s$ eine Beliebige Spiegelung. So gilt $Ds = \{s_+, s_-, s_x, s_y\} = sD$. Einzelne Spiegelungen kommutieren also nicht mit einzelnen Gruppenelementen, es ist jedoch trotzdem eine Art "Kommutativität mit der Gruppe als Ganzes" vorhanden.
\end{proof}
\noindent Die Menge der Drehungen ist desweiteren gegeben als der Kern der Determinante, welche ein Gruppenhomomorphismus in die Gruppe mit zwei Elementen ist (genauer in die Gruppe $\{\pm 1, \cdot\}$).
\begin{theorem}
	Sei $\phi : G \to H$ ein Gruppenhomomorphismus. So induziert $\phi$ einen Isomorphismus $G / \ker \phi \to \im \phi$.
\end{theorem}
\noindent Sei $G$ eine Gruppe mit $\abs{G} = 5$. Gemäß Lagrange hat $G$ nur die triviale Gruppe und sich selbst als Untergruppe. Sei $g \in G \setminus \{1\}$. So haben wir einen Gruppenhomomorphismus
\begin{align*}
	\phi : (\bZ, +) &\to G\\
	         n &\mapsto g^n
\end{align*}
Wir wissen nun, das das Bild von $\phi$ eine Untergruppe von $G$ bildet. Da $g \neq 1$ handelt es sich nicht um die triviale Gruppe, also ist $G = \im \phi$.
\newpar
$\ker \phi \subsetneq \bZ$ ist eine Untergruppe, also existiert genau ein $n \in \bN$, sodass $\ker \phi = \bZ n$. Da $G$ $5$ Elemente hat, muss $n = 5$. Die einzige Gruppe mit $5$ Elementen ist also $\bZ / 5\bZ$.
\begin{corollary}
	Ist $p$ eine Primzahl, so ist jede Gruppe mit $p$ Elementen isomorph zu $\bZ/ p\bZ$.
\end{corollary}
\begin{lemma}
	Sei $\pi : G \to H$ ein surjektiver Gruppenhomomorphismus und $N \subseteq G$ ein Normalteiler. So ist $\pi(N)$ ein Normalteiler von $H$.
\end{lemma}
\begin{theorem}
	\theoremname{Noetherscher Isomorphiesatz:} Sei $G$ eine Gruppe und seien $H$ und $K$ Normalteiler. So ist $H / K$ ein Normalteiler von $G / K$ und die Komposition kanonischer Abbildungen $G \surj G / K \surj (G / K) / (H / K)$ induziert einen Isomorphismus $G / H \to (G / K) / (H / K)$
\end{theorem}
\begin{definition}
	Sei $G$ eine Gruppe und $g \in G$. Wir definieren die Ordnung eines Elements als:
	\begin{align*}
		\ord(g) = \inf\{n \in \bN_{\geq 1} \mid g^n = e\}
	\end{align*}
	Erinnerung: Das Infimum der leeren Menge ist $\infty$, also gilt $\ord(g) = \infty$ falls kein solches $n$ existiert.
\end{definition}
\begin{proposition}
	Sei $G$ eine Gruppe und $g \in G$.
	\begin{enumerate}
		\item $\ord(g) = \abs{\angles{g}}$
		\item $\angles{g}$ ist isomorph zu $\bZ / (\ord(g))\bZ$, falls die Ordnung endlich ist, und zu $\bZ$, falls die Ordnung unendlich ist.
		\item Falls $\ord(g) < \infty$, so gilt $g^n = e \Leftrightarrow \ord(g) \mid n$
	\end{enumerate}
\end{proposition}
\begin{definition}
	Sei $G$ eine Gruppe. So heißt $\abs{G}$ die \tbf{Ordnung} der Gruppe.
\end{definition}
\begin{theorem}
	Sei $g \in G$ ein Element einer endlichen Gruppe. So ist $\ord(g)$ ein Teiler von $\abs{G}$.
\end{theorem}
\begin{proof}
	$\ord(g) = \abs{\angles{g}}$. Nach Lagrange teilt die Größe der Untergruppe $\angles{g}$ die Größe der Gruppe $G$.
\end{proof}
\noindent Im Allgemeinen gibt es aber nicht für jeden Teiler ein Element mit der jeweiligen Ordnung. Zum Beispiel fehlt bei jeder nicht zyklischen Gruppe bereits die Ordnung selbst. Wohl aber gilt:
\begin{theorem}
	\theoremname{Cauchy:} Sei $p$ eine Primzahl mit $p \mid \abs{G}$. So gibt es ein $g \in G$ mit $\ord(g) = p$.
\end{theorem}
\noindent Ein Beweis folgt aber erst später.
\begin{corollary}
	Sei $g \in G$ ein Element einer endlichen Gruppe. So gilt 
	\begin{align*}
		g^\abs{G} = e.
	\end{align*}
\end{corollary}
\begin{proof}
	$g^\abs{G} = g^{r \cdot \abs{\ord(g)}} = e^r = e.$
\end{proof}
\begin{theorem}
	\theoremname{Kleiner Satz von Fermat:} Sei $p$ eine Primzahl und $a \in \bZ$. So gilt
	\begin{align*}
		a^p \equiv a \mod p
	\end{align*}
\end{theorem}
\begin{proof}
	Wir betrachten $\bZ/p\bZ$. Falls $\alpha \equiv 0 \mod p$ ist die Aussage trivial. Da außerdem die Multiplikative Gruppe die Ordnung $p-1$ hat gilt im Fall $\alpha \not\equiv 0$ ebenfalls trivial $\alpha^{p-1} \equiv 1 \mod p$, also $\alpha^p \equiv \alpha \mod p$.
\end{proof}
\noindent Alternativ kann der Beweis auch auf "Allgemeinbildungsniveau" geführt werden:
\begin{proof}
	(Ohne Gruppentheorie): Sei $a \in \{0, \hdots, p-1\}$. So sind die Zahlen $a, 2a, 3a, \hdots, (p-1)a$ paarweise verschieden, also ist nach Schubfachprinzip ein Element jeder Restklasse $\mod p$ enthalten. Also gilt:
	\begin{align*}
		\prod_{k = 1}^{p-1} ka &\equiv \prod_{k = 1}^{p-1} k \mod p\\
		\implies (p-1)!a^{p-1} &\equiv (p-1)! \mod p.
	\end{align*}
	Allerdings ist $(p-1)!$ teilerfremd zu $p$, also können wir durch $(p-1)!$ teilen und erhalten $a^{p-1} \equiv 1 \mod p$.
\end{proof}
\begin{theorem}
	\label{thrm:sunziprereq}
	Seien $a,b \in \bN_{\geq 1}$ teilerfremd. So induziert
	\begin{align*}
		\phi : \bZ &\to \bZ / a\bZ \times \bZ / b\bZ\\
		n &\mapsto (n + a\bZ, n + b\bZ)
	\end{align*}
	einen Isomorphismus:
	\begin{align*}
		\bZ / ab\bZ &\overset{\sim}{\rightarrow} \bZ / a\bZ \times \bZ / b\bZ\\
	\end{align*}
\end{theorem}
\begin{proof}
	Nach Bézout gibt es $x,y \in \bZ$ mit
	\begin{align*}
		xa + yb = 1.
	\end{align*}
	Nun gilt:
	\begin{align*}
		n &= xy = 1 - yb \overset{\phi}{\mapsto} (0,1)\\
		m &= yb = 1 - xy \overset{\phi}{\mapsto} (1,0)
	\end{align*}
	Diese Tupel bilden eine Basis des Bildraums, also ist $\phi$ surjektiv:
	\begin{align*}
		\phi(\alpha n + \beta m) &= \phi(\alpha n) + \phi(\beta m)\\
								 &= \alpha\phi(n) + \beta\phi(m)\\
								 &= (\beta, \alpha)
	\end{align*}
	Da beide Seiten gleich viele Elemente haben handelt es sich sogar um einen Isomorphismus.
\end{proof}
\begin{example}
	Das Kongruenzensystem
	\begin{align*}
		n &\equiv 3 \mod 17\\
		n &\equiv 5 \mod 9\\
		n &\equiv 1 \mod 12
	\end{align*}
	hat keine Lösung, denn aus $n \equiv 5 \mod 9$ folgt $n \equiv 2 \mod 3$, also $n \not \equiv 1 \mod 12$. Ersetzen wir jedoch $12$ durch $13$, so sind die Teiler teilerfremd und der chinesische Restsatz garantiert eine Lösung.
\end{example}
\begin{theorem}
	\theoremname{Chinesischer Restsatz / Satz von Sunzi / Satz von Aryabhata:} Satz \ref{thrm:sunziprereq} funktioniert auch für mehr als zwei Zahlen - sind $q_1, \hdots, q_r \in \bN_{\geq 1}$ teilerfremd, so ist
	\begin{align*}
		\bZ \to \bZ / q_1\bZ \times \hdots \times \bZ / q_r \bZ
	\end{align*}
	surjektiv und induziert eines Isomorphismus
	\begin{align*}
		\bZ / q_1 \hdots q_r \bZ \iso \bZ / q_1\bZ \times \hdots \times \bZ / q_r\bZ
	\end{align*}
\end{theorem}
\noindent Genauer kann man isomorph jedem Element $n$ das Tupel $(n \mod q_1, \hdots, n \mod q_r)$ zuordnen - so kommt man zurück zur "klassischen" Formulierung des Chinesischen Restsatzes.
\begin{corollary}
	Die Gruppe $\bZ / q_1\bZ \times \hdots \times \bZ / q_r\bZ$ wird durch das Element $(1, \hdots, 1)$ erzeugt und ist somit zyklisch.
\end{corollary}
\begin{lemma}
	\theoremname{Zentrales Lemma der RSA-Verschlüsselung:} Seien $p$ und $q$ prim und $st \equiv 1 \mod (p-1)(q-1)$. So ist $(a^t)^s \equiv \mod a \mod pq$.
\end{lemma}
\begin{proof}
	Wir wissen $a^x = a \mod p$ falls $x \equiv 1 \mod (p-1)$. 
	\newpar
	Also $a^x = a \mod pq$, wenn $x \equiv 1 \mod (p-1)$ und $x \equiv 1 \mod (q-1)$, also insbesondere $a^x = a \mod p$ wenn $x \equiv 1 \mod (p-1)(q-1)$.
\end{proof}
\noindent In der RSA-Verschlüsselung werden dann letztendlich $pq$ und $t$ veröffentlicht, $a^t$ als Nachricht zurückgeschickt, und dann durch Exponentiation mit $s$ die Nachricht entschlüsselt.
\begin{definition}
	Sei $A$ eine abelsche Gruppe und $p$ eine Primzahl. Wir definieren:
	\begin{align*}
		A(p) = \{a \in A \mid \exists n \in \bN : \ord(a) = p^n\}
	\end{align*}
\end{definition}
\begin{theorem}
	Sei $(A, +)$ eine abelsche Gruppe. Für jede Primzahl $p$ ist $A(p) \subseteq A$ eine Untergruppe.
\end{theorem}
\begin{proof}
	Seien $x,y \in A(p)$. So existieren $r,s \in \bN_{\geq 0}$, sodass $p^rx = p^sy = 0$. Da $A$ abelsch ist folgt
	\begin{align*}
		p^{r+s}(x+y) = p^rx + p^sy = 0
	\end{align*}
	Also $x + y \in A(p)$.
	\newpar
	Es gilt außerdem $p0 = 0$, also $0 \in A(p)$, und $p^r(-x) = -(p^rx) = -0 = 0$, also $-x \in A(p)$.
\end{proof}
\begin{theorem}
	Sei $(A, +)$ eine abelsche Gruppe und seien $p_1, \hdots, p_r$ paarweise verschiedene Primzahlen. So liefert die Gruppenverknüpfung eine Injektion
	\begin{align*}
		A(p_1) \times \hdots \times A(p_r) \inj A
	\end{align*}
	mit Bild alle Elemente endlicher Ordnung, in deren Ordnung nur die Primfaktoren $p_1, \hdots, p_r$ vorkommen.
\end{theorem}
\begin{proof}
	Sei sonst $(a_1, \hdots, a_r) \neq (0, \hdots, 0)$ ein Tupel mit $a_i \in A(p_i)$ und 
	\begin{align*}
		a_1 + \hdots + a_r = 0.
	\end{align*} Sei OBdA $a_1 \neq 0$. So gilt:
	\begin{align*}
		-a_1 = a_2 + \hdots + a_r
	\end{align*}
	Für hinreichend großes $n$ gilt nun
	\begin{align*}
		-(p_2\hdots p_r)^na_1 = (p_2\hdots p_r)^n(a_2 + \hdots + a_r) = 0
	\end{align*}
	Allerdings gilt
	\begin{align*}
		(p_2\hdots p_r)^na_1 \neq 0
	\end{align*}
	Da die Primzahlen $p_2, \hdots, p_r$ alle die Ordnung von $a_1$ nicht Teilen. Widerspruch!
	\newpar
	Sei nun $a \in A$, $\ord(a) = p_1^{\alpha_1}\hdots p_r^{\alpha_r}$. So gilt:
	\begin{align*}
		\angles{a} &\cong  \bZ / \ord(a)\bZ\\
				   &= \bZ / p_1^{\alpha_1}\hdots p_r^{\alpha_r}\bZ\\
				   &\cong \bZ / p_1^{\alpha_1}\bZ \times \hdots \times \bZ / p_r^{\alpha_r}\bZ
	\end{align*}
\end{proof}
\noindent Dieser Satz ist analog zur Hauptraumzerlegung eines Vektorraums - wir zerlegen $A$ in eine direkte Summe aus Untergruppen. Es gibt in der kommutativen Algebra einen Satz über Module über Ringe, welcher sowohl diesen Satz als auch die Hauptraumzerlegung gemeinsam verallgemeinert.
\begin{corollary}
	Sei $E$ eine endliche abelsche Gruppe. Seien $p_1, \hdots, p_r$ die Primfaktoren der Kardinalität $\abs{E}$ von $E$. So gilt:
	\begin{enumerate}
		\item Die Verknüpfung liefert eine Bijektion
		\begin{align*}
			E(p_1) \times \hdots \times E(p_r) \iso E
		\end{align*}
		\item Für alle $i$ ist $\abs{E(p_i)}$ eine Potenz von $p_i$. 
	\end{enumerate}
\end{corollary}
\begin{proof}
	\phantom{}
	\begin{enumerate}
		\item Folgt direkt aus dem vorherigen Satz, da die Elemente endlicher Ordnung, in deren Ordnung nur die Primfaktoren $p_1, \hdots, p_r$ vorkommen, nun die ganze Gruppe ausmachen.
		\item Folgt aus Induktion über $\abs{E_p}$. Der Fall $\abs{E_p} = 1$ ist klar:
		\begin{align*}
			\abs{E_p} = 1 = p^0
		\end{align*}
		Für $\abs{E_p} > 1$ existiert ein Element $x \neq 0$. Es gilt $\abs{\angles{x}} = p^r$ nach Annahme. Dann folgt
		\begin{align*}
			\abs{E(p) / \angles{x}} = p^s
		\end{align*}
		nach Induktionsannahme, da der Quotient kleiner ist. Somit folgt nach Lagrange
		\begin{align*}
			\abs{E_p} = \abs{E(p) / \angles{x}} \cdot \abs{\angles{x}} = p^s \cdot p^r
		\end{align*}
	\end{enumerate}
\end{proof}
\begin{proposition}
	Sei $E$ eine endliche abelsche Gruppe, sodass die Ordnung jedes $x \in E$ eine Potenz von $p$ prim ist. So ist $\abs{E}$ ebenfalls eine $p$-Potenz.
\end{proposition}
\begin{proposition}
	Es existiert eine Bijektion zwischen der Menge der endlichen abelschen Gruppen bis auf Isomorphismus und den endlichen Multimengen echter Primpotenzen. Genauer hat jede endliche abelsche Gruppe die Form
	\begin{align*}
		\bZ / p_1\bZ \times \hdots \times \bZ / p_r \bZ
	\end{align*}
	Für $p_1, \hdots, p_r$ prim.
\end{proposition}
\noindent Zum Beispiel ist jede abelsche Gruppe der Ordnung $8$ isomorph zu $\bZ / 8\bZ$, $\bZ / 4\bZ \times \bZ / 2\bZ$, oder $\bZ / 2\bZ \times \bZ / 2\bZ \times \bZ / 2\bZ$.
\begin{theorem}
	Sei $K$ ein Körper und $K^\times$ dessen multiplikative Gruppe. So ist jede endliche Untergruppe von $K^\times$ zyklisch.
\end{theorem}
\section{Gruppenoperationen und Kongruenzklassen}
\begin{definition}
	Sei $G$ eine Gruppe und $X$ eine Menge. Eine Operation von $G$ auf $X$ ist eine Abbildung
	\begin{align*}
		G \times G &\to X\\
		(g,x) &\mapsto gx
	\end{align*}
	Sodass: 
	\begin{enumerate}
		\item $\forall g,h \in G : (gh)x = g(hx)$
		\item $1x = x$
	\end{enumerate}
\end{definition}
\begin{example}
	Sei $X$ eine Menge und
	\begin{align*}
		\Ens^\times(X) := \{f : X \to X \mid f \text{ bijektiv}\}
	\end{align*}
	So ist $\Ens^\times$ eine Gruppe, welche durch Anwendung der jeweiligen Funktion auf kanonische Weise auf $X$ operiert:
	\begin{align*}
		\Ens^\times(X) \times X &\to X\\
		(f, x) &\mapsto f(x)
	\end{align*}
\end{example}
\begin{example}
	$\cS_n$ operiert durch Anwendung der jeweiligen Permutation auf $\{1, \hdots, n\}$.
\end{example}
\begin{example}
	Analog operiert $\GL(n,\bR)$ auf $\bR^n$.
\end{example}
\begin{definition}
	Sei $G$ eine Gruppe, welche auf eine Menge $X$ operiert. So nennen wir für $x \in X$ die Menge
	\begin{align*}
		Gx = \{gx \mid g \in G\}
	\end{align*}
	die \tbf{Bahn} von $x$.
\end{definition}
\begin{example}
	Wir lassen die Gruppe $\SO(2)$ der zweidimensionalen Rotationsmatrizen auf $\bR^2$ operieren. So ist die Bahn eines Vektors $\vv$ der Kreis mit Radius $\norm{\vv}$.
\end{example}
\begin{example}
	Sei $H$ eine Untergruppe einer Gruppe $G$. So operiert $H$ durch die Gruppenoperation auf $G$ und die Bahnen $Hg$ sind genau die Nebenklassen.
\end{example}
\begin{proposition}
	Die Bahnen der Operation einer Gruppe auf einer Menge sind paarweise disjunkt.
\end{proposition}
\begin{proposition}
	Die Bahnen der Operation eines Monoids auf einer Menge sind nicht umbedingt paarweise disjunkt.
\end{proposition}
\begin{definition}
	Sei $G$ eine Gruppe, welche auf einer Menge $X$ operiert. Sei $x \in X$. Wir nennen die Menge
	\begin{align*}
		G_x := \{g \in G \mid gx = x\}
	\end{align*}
	die \tbf{Standgruppe} oder \tbf{Isotropiegruppe} von $x$.
\end{definition}
\begin{lemma}
	Die Abbildung $G \to X : g \to gx$ induziert eine Bijektion
	\begin{align*}
		G / G_x \iso Gx
	\end{align*} 
\end{lemma}
\begin{proofsketch}
	Für jede $G_x$-Linksnebenklasse $L \subset G$ besteht die Menge $Lx$ nur aus einem Punkt. Falls $L = gG_x$ haben wir genauer $Lx = gG_xx = \{gx\}$. Wir können nun unsere Bijektion definieren, indem wir jeder Linksnebenklasse $L \in G/G_x$ das einzige Element von $Lx$ zuordnen. Sie ist trivial surjektiv, gleichzeitig ist sie injektiv, denn aus $gG_x x = hG_x x$ folgt $gx = hx$, also $h^{-1}g \in G_x$, also $gG_x = hG_x$.
\end{proofsketch}
\begin{corollary}
	\theoremname{Bahnformel:}
	\begin{align*}
		\abs{G} = \abs{Gx} \cdot \abs{G_x} = \abs{G / G_x} \cdot \abs{G_x}
	\end{align*}
\end{corollary}
	\noindent Frage: Was sind die endlichen Untergruppen der dreidimensionalen Drehgruppe $\SO_3$? 
	\newpar
	Vorüberlegung: Jede Gruppe $G$ mit $x \in G$ operiert auf sich selber durch $g.x = gxg^{-1}$, auch notiert als $(\textnormal{int} g)(x)$ für "interior", da dies einen sogenannten \tbf{inneren Automorphismus} bildet. Es gilt nämlich:
	\begin{align*}
		\textnormal{int} : G &\to \Aut(G)\\
		g &\mapsto (x \mapsto gxg^{-1})
	\end{align*}
	
	\noindent Wir nennen diese Operation die "\textbf{Operation durch Konjugation}".
	\newpar
	Dabei gilt:
	\begin{align*}
		(hg).x &= hgx(hg)^{-1}\\
			   &= hgxg^{-1}h^{-1}\\
			   &= h(gxg^{-1})h^{-1}\\ 
			   &= h.(g.x)
	\end{align*}
	und außerdem:
	\begin{align*}
		g.(xy) = gxyg^{-1} = gxg^{-1}gyg^{-1} = (g.x)(g.y)
	\end{align*}
	Intuitiv sind Drehungen, welche zueinander konjugiert werden können, "quasi die gleichen" - die Symmetriegruppe, welche den Raum an einer Achse um $180$ Grad dreht, ist die gleiche, welche den Raum an einer anderen Achse um $180$ Grad dreht. Es ist also sinnvoll, die Frage einzuschränken auf "Was sind die endlichen Untergruppen von $\SO_3$ \tit{bis auf Konjugation}? Die Antwort ist nun:
	\begin{theorem}
		Die endlichen Untergruppen der Drehgruppe $\SO_3$ bis auf Konjugation sind genau:
		\begin{enumerate}
			\item Die zyklischen Gruppen $C_k$, für $k \geq 1$
			\item Die \tbf{Diedergruppen} $D_k$, für $k \geq 2$ (die Verallgemeinerungen unserer Quadratgruppe)
			\item Die Symmetriegruppen des Würfels (= die Symmetriegruppe des Oktaeders), des Tetraeders, und des Ikosaeders (= die Symmetriegruppe des Dodekaeders)
		\end{enumerate}
	\end{theorem}
	\begin{proof}
		\begin{proposition}
			$g \in \SO_3 \setminus \id$ hält genau eine Gerade punktweise fest. Diese sticht an zwei Punkten durch die Einheitssphäre $S^2$.
		\end{proposition}
		\noindent Wir nennen diese Punkte die Pole $P \subset S^2$ von $g$. Sei also $G \subset \SO(3)$ eine endliche Untergruppe. Wir definieren
		\begin{align*}
			M \in \{(g,p) \mid g \in G \setminus \id, p \in P\}
		\end{align*}
		Als die Menge der Pole der Drehachsen der Gruppe. Es gilt
		\begin{align*}
			\abs{M} &= 2(\abs{G} - 1)\\
			\abs{M} &= \sum_{p \in P} \left(\abs{G_p} - 1\right)\\
		\end{align*}
		Die Gruppe operiert außerdem auf der Menge der Pole ($G \searrow P$). Wir können die Menge der Pole somit Zerlegen in ihre $G$-Bahnen, also $P \to P_1 \sqcup \hdots \sqcup P_r$. Die Ordnung der Pole einer Bahn ist konstant, wir definieren also die \tbf{Polordnung} der Pole der $i$-ten Bahn als
		\begin{align*}
			\forall p \in P_i : n_i := \abs{G_p}
		\end{align*}
		Und erhalten
		\begin{align*}
			\sum_{p \in P} \left(\abs{G_p} - 1\right) &= \sum_{i = 1}^r \abs{P_i} \cdot (n_i - 1)\\
		\end{align*}
		Aus der Bahnformel erhalten wir
		\begin{align*}
			\abs{P_i} \cdot n_i = \abs{G}
		\end{align*}
		Also gilt:
		\begin{align*}
			2(\abs{G} - 1)  &=\sum_{i = 1}^r \abs{P_i} \cdot (n_i - 1)\\
			\implies 2 - \frac{2}{\abs{G}} &=\sum_{i = 1}^r 1 - \frac{1}{n_i}\\ 
		\end{align*}
		\begin{description}
			\item[Fall $r = 0$:] Es gibt keine Pole, also ist die Gruppe trivial. Die triviale Gruppe kann auch betrachtet werden als die zyklische Gruppe $C_1$ mit nur einem Element.
			\item[Fall $r = 1$:] Dann gilt:
			\begin{align*}
				2 - \frac{2}{\abs{G}} &= 1 - \frac{1}{n_1}
			\end{align*}
			Die Gruppe muss mindestens $2$ Elemente haben, also ist der linke Term größer als $1$ und der rechte Term kleiner als $1$, was ein Widerspruch ist.
			\item[Fall $r = 2$:] Dann gilt:
			\begin{align*}
				2 - \frac{2}{\abs{G}} &= 2 - \frac{1}{n_1} - \frac{1}{n_2}
			\end{align*}
			Es gilt $n_i \leq \abs{G}$, also ist die Gleichung genau dann erfüllt, wenn $n_1 = n_2 = \abs{G}$. Wir haben also eine Gruppe mit zwei Polen, deren Ordnung die Ordnung der gesamten Gruppe ist. Die ganze Gruppe hat also nur eine Symmetrieachse, die Gruppe muss dann eine zyklische Gruppe sein.
			\item[Fall $r = 3$:] Dann gilt:
			\begin{align*}
				2 - \frac{2}{\abs{G}} &= 3 - \frac{1}{n_1} - \frac{1}{n_2} - \frac{1}{n_3}\\
			\implies
				 1 + \frac{2}{\abs{G}} &= \frac{1}{n_1} + \frac{1}{n_2} + \frac{1}{n_3}
			\end{align*}
			Sei OBdA $n_1 \leq n_2 \leq n_3$. Behauptung: Dann muss $n_1 = 2$ sein.
			\begin{description}
				\item[Fall $3a$: $n_2 = 2$:] Dann gilt $2n_3 = \abs{G}$. Nach Bahnformel folgt $\abs{P_3} = 2$. Bei einer Polbahn mit zwei Elementen müssen sich die Pole gegenüberliegen. Wir erhalten die Diedergruppe $D_{n_3}$ - $n_3$ entspricht der zentralen Drehsymmetrie und $n_1$ und $n_2$ entsprechen den seitlichen Symmetrieachsen durch die Ecken und Kanten.
				\item[Fall $3a$: $n_2 > 2$:]
				\begin{align*}
					\frac{1}{2} + \frac{1}{n_2} + \frac{1}{n_3} > 1 + \frac{2}{\abs{G}}
				\end{align*}
				Also $n_2 = 3$ und $n_3 = 3$ oder $n_3 = 4$ oder $n_3 = 5$. Der erste Fall liefert den Tetraeder, der zweite Fall den Würfel und der dritte Fall den Ikosaeder.
			\end{description}
			\item[Fall $r \geq 4$:] Die Gleichungen sind nicht mehr lösbar.
		\end{description}
	\end{proof}
	\begin{definition}
		Eine Gruppe $G \neq 1$ heißt \tbf{einfach}, falls ihre einzigen Normalteiler die triviale Gruppe $1$ und die Gruppe $G$ selbst sind.
	\end{definition}
	\begin{proposition}
		Jede Gruppe mit Primzahlordnung ist einfach (also $\bZ/ p\bZ$ für $p$ prim). Diese Gruppen sind die einzigen abelschen einfachen Gruppen.
	\end{proposition}
	\begin{proof}
		Satz von Lagrange.
	\end{proof}
	\begin{proposition}
		Für fast alle Körper $k$ ist $GL(n,k) / k^\times$ einfach.
	\end{proposition}
	\begin{theorem}
		Die Ikosaedergruppe $I$ ist einfach.
	\end{theorem}
	\begin{proof}
		Wir haben:
		\begin{itemize}
			\item Ein Element der Ordnung $1$ (die Identität)
			\item $15$ Elemente der Ordnung $2$ (Drehungen um Kantenmitten, da wir $30$ Kanten haben und je zwei gegenüberliegende Kanten die selben Drehungen haben)
			\item $20$ Elemente der Ordnung $3$ (Drehungen um Flächenmitten, da wir $20$ Flächen haben)
			\item Keine Elemente der Ordnung $4$
			\item $24$ Elemente der Ordnung $5$ (Drehungen an Ecken, da wir $12$ Ecken haben, an jeder Ecke vier Drehungen, aber je zwei gegenüberliegende Ecken die gleichen Drehungen haben)
		\end{itemize}
		Wir wollen die Elemente nun in Konjugationsklassen zerlegen. Die Elemente der Ordnung $2$ und der Ordnung $3$ bilden jeweils eine Konjugationsklasse. Die Drehungen um Ecken können jedoch nicht alle in der selben Konjugationsklasse liegen, da $24$ kein Teiler von $60$ ist. Die Drehungen um Ecken zerfallen also in zwei Konjugationsklassen - es handelt sich um die Drehungen um $2\pi/5$ ($72$°, "Eine Ecke weiter"), und die Drehungen um $4\pi/5$ ($144°$, "zwei Ecken weiter"). Größere Drehungen existieren nicht, da eine Drehung drei Ecken weiter einer Drehung um zwei Ecken in die andere Richtung entspricht und eine Drehung vier Ecken weiter einer Drehung um eine Ecke in die andere Richtung entspricht.
		\newpar
		Sei also nun $N$ ein Normalteiler von $I$. So muss $\abs{N}$ ein Teiler von $\abs{I}$ sein, also $1,2,3,4,5,6,10,12,15,20,30,60$.
		\begin{proposition}
			Sei $N$ ein Normalteiler einer Gruppe $G$. So ist $N$ eine Vereinigung von Konjugationsklassen von $G$.
		\end{proposition}
		Aus der Proposition folgt, dass $\abs{I}$ außerdem eine Summe von $1,20,15,12,12$ sein muss, wobei die $1$ in der Sumume enthalten sein muss (da $N$ sonst nicht das neutrale Element enthalten würde). Die einzigen Möglichkeiten sind also $\abs{N} = 1 = 1$ und $\abs{N} = 60 = 1 + 20 + 15 + 12 + 12$. Also sind die einzigen Normalteiler von $I$ die triviale Gruppe und $I$ selbst.
	\end{proof}
	\begin{proposition}
		Die \tbf{alternierenden Gruppen}
		\begin{align*}
			A_r := \ker(\sgn : \cS_r \to \{\pm 1\})
		\end{align*} sind für $r \geq 5$ einfach. (Wobei der Kern genau den Permutationen mit $\sgn(\sigma) = 1$ entspricht). Insbesondere ist $A_5$ isomorph zur Ikosaedergruppe, und diese ist die kleinste nichtabelsche einfache Gruppe.
	\end{proposition}
	\begin{proof}
		Ein Ikosaeder hat die selbe Symmetriegruppe wie ein Dodekaeder, da die beiden Körper dual zueinander sind (also können die Ecken des einen den Flächenmitten des anderen zugeordnet werden und umgekehrt).
		\begin{proposition}
			Ein Dodekaeder enthält fünf eingeschriebene Würfel.
		\end{proposition}
		Wir erhalten einen Gruppenhomomorphismus $\phi : I \to \cS_5$, welcher beschreibt, wie eine Symmetrie des Ikosaeders/Dodekaeders die eingeschriebenen Würfel permutiert. So ist $\sgn \circ \phi$ ein Homomorphismus $I \to \{\pm 1\}$. Da $I$ einfach ist und $\abs{I} \geq 2$ ist $\ker(\sgn \circ \phi) = I$. 
		\newpar
		$\ker(\phi)$ ist ein von $I$ verschiedener Normalteiler, also ist $\ker(\phi) = 1$, also ist $\phi$ injektiv. Es gilt $\abs{I} = 60$ und $\abs{\cS_5} = 120 \implies \abs{A_5} = 60$, also ist $\phi$ auch surjektiv, also bijektiv, also ein Isomorphismus.
	\end{proof}
	\section{Kompositionsreihen}
	\begin{definition}
		Eine \tbf{Kompositionsreihe} einer Gruppe $G$ ist eine Folge von Untergruppen
		\begin{align*}
			1 = G_0 \subset G_1 \subset \hdots \subset G_{r-1} \subset G_r = G
		\end{align*}
		sodass jedes $G_i$ Normalteiler von $G_{i+1}$ ist und alle \tbf{Subquotienten} $G_i / G_{i-1}$ einfach sind.
	\end{definition}
	\begin{theorem}
		\theoremname{Jordan-Hölder:} Je zwei Kompositionsreihen einer endlichen Gruppe $G$ haben die selbe Länge und bis auf Reihenfolge die selben Subquotienten.
	\end{theorem}
	\noindent Dieser Satz gibt uns eine "Primfaktorzerlegung" von Gruppen, in der die "Primfaktoren" die Subquotienten sind.
	\begin{example}
		Sei $G = \bZ / 20\bZ$. Eine mögliche Kompositionsreihe ist nun:
		\begin{align*}
			\bZ/20\bZ \supset 2\bZ / 20\bZ \supset 10\bZ / 20\bZ \supset 0
		\end{align*}
		Die Subquotienten hier sind dann $\bZ/2\bZ$, $\bZ/5\bZ$ und $\bZ/2\bZ$.
	\end{example}
	\begin{proposition}
		Es existiert eine universelle Eigenschaft surjektiver linearer Abbildungen, welche exakt analog zur universellen Eigenschaft surjektiver Gruppenhomomorphismen funktioniert. 
	\end{proposition}
	\noindent Eine lineare Abbildung ist also im Wesentlichen eindeutig durch ihren Kern bestimmt - alle surjektiven linearen Abbildungen mit dem selben Kern können durch Verkettung mit einem eindeutigen Isomorphismus ineinander überführt werden.
	\begin{proposition}
		Für jeden Untervektorraum $U \subset V$ gibt es eine surjektive lineare Abbildung $s : V \surj W$ mit $\ker s = U$. Dies ist genau die Abbildung $V \surj V / U$.
	\end{proposition}
	\begin{proposition}
		Für Vektorräume gilt $\dim V = \dim U + \dim V / U$. Diese Formel kann als Analogon zum Satz von Lagrange gesehen werden.
	\end{proposition}
	\begin{example}
		Betrachte einen $3$-dimensionalen Vektorraum $V$ über $\bF_7$. Eine mögliche Kompositionsreihe ist
		\begin{align*}
			V \supset W \supset U \supset \{0\}
		\end{align*}
		mit $\dim W = 2$ und $\dim U = 1$. Die Subquotienten $V/W$, $W/U$ und $U/\{0\}$ sind jeweils eindimensionale Untervektorräume von $V$, also isomorph zur additiven Gruppe von $\bF_7$.
	\end{example}
	\begin{proof}[Beweis (Jordan-Hölder):]
		Per Induktion über $\abs{G}$. Ist $\abs{G} = 1$, ist $G$ die triviale Gruppe und somit insbesondere bereits alleine die vollständige Kompositionsreihe und die Aussage gilt trivial.
		\newpar
		Seien nun $G \supset M \supset \hdots$ und $G \supset N \supset \hdots$ Kompositionsreihen. $M$ und $N$ müssen beide Ordnungen kleiner $G$ haben, falls also $M = N$ ist gilt die Aussage per Induktion. Ist $N \neq M$, existiert ein surjektiver Homomorphismus $N \surj G / M$ mit Kern $N \cap M$. Somit geht $N / N \cap M$ isomorph auf $G / M$. 
		\newpar
		Wir wissen also, dass der Subquotient $G / M$ isomorph zu $N / N \cap M$ ist. Analog ist der Subquotient $G / N$ isomorph zu $M / M \cap N$. Also sind
		$G \supset M \supset M \cap N \supset \hdots$ und $G \supset N \supset M \cap N \supset \hdots$ valide Kompositionsreihen, welche per Induktion äquivalent zu den ursprünglichen Kompositionsreihen sind. Somit folgt auch die gesuchte Aussage per Induktion.
	\end{proof}
	\section{$p$-Gruppen und Sylowsätze}
	\begin{definition}
		Sei $p$ eine Primzahl. Eine endliche Gruppe $G$ heißt \tbf{$p$-Gruppe}, wenn
		\begin{align*}
			\abs{G} = p^n.
		\end{align*}
	\end{definition}
	\begin{definition}
		Sei $G$ eine Gruppe. Das \tbf{Zentrum} $Z$ einer Gruppe ist die Menge der Elemente, welche mit allen anderen Gruppenelementen kommutieren:
		\begin{align*}
			Z(G) = \{z \in G \mid \forall g \in G : zg = gz\}
		\end{align*}
	\end{definition}
	\begin{theorem}
		Sei $G$ eine nichttriviale $p$-Gruppe. So ist auch das Zentrum von $G$ nichttrivial.
	\end{theorem}
	\begin{proof}
		Sei $\abs{G} = p^r$. Wir zerlegen $G$ in ihre Konjugationsklassen $C_1, \hdots, C_s$. $\abs{C_i}$ teilt $\abs{G} = p^r$, also haben insbesondere alle Konjugationsklassen eine $p$-Potenz als Ordnung.
		\newpar
		Ist $\abs{C_i} = 1$, gilt $C_i = \{z\}$ mit $z \in Z(G)$. Also:
		\begin{align*}
			\abs{G}  \equiv \abs{Z(G)} \mod p
		\end{align*}
		Ist $\abs{G} \neq 1$, folgt $\abs{Z(G)} \neq 1$.
	\end{proof}
	\begin{theorem}
		Sei $p$ prim und $\abs{G} = p^2$. So ist $G$ abelsch und isomorph zu $\bZ / p^2\bZ$ oder $\bZ / p\bZ \times \bZ / p\bZ$.
	\end{theorem}
	\begin{proof}
		Wir wissen $Z(G) \neq 1$. Wäre $Z(G) \neq G$, so wäre $\abs{Z(G)} = p$ und für $x \in G \setminus Z(G)$ wäre
		\begin{align*}
			\angles{Z(G), x} = G
		\end{align*}
		Aber diese erzeugte Gruppe ist abelsch! Widerspruch!!!!!!
	\end{proof}
	\begin{definition}
		Sei $G$ eine endliche Gruppe, $p$ eine Primzahl, und $p^r$ die größte $p$-Potenz, die $\abs{G}$ teilt. Eine Untergruppe $S \subset G$ mit $\abs{S} = p^r$ heißt \tbf{$p$-Sylow} von $G$
	\end{definition}
	\begin{theorem}
		In jeder $p$-Gruppe $G \neq 1$ gibt es ein Element $g \in G$ der Ordnung $p$.
	\end{theorem}
	\begin{theorem}
		Jede $p$-Gruppe ist nilpotent.
	\end{theorem}
	\begin{theorem}
		\theoremname{Sylow-Sätze:}
		Sei $G$ eine endliche Gruppe und $p$ prim, sodass $p^r$ $G$ maximal teilt.. So gilt:
		\begin{enumerate}
			\item Es gibt $p$-Sylows in $G$.
			\item Je zwei $p$-Sylows in $G$ sind zueinander konjugiert.
			\item Jede Untergruppe $H \subset G$, deren Ordnung eine $p$-Potenz ist, liegt in einer $p$-Sylow.
			\item Die Zahl der $p$-Sylows von $G$ ist kongruent zu $1$ modulo $p$ und teilt $\abs{G}/p^r$
		\end{enumerate}
	\end{theorem}
	\begin{example}
		In der Würfelgruppe $W$ ist $\abs{W} = 24$. Ein $2$-Sylow ist also eine Untergruppe mit Ordnung $2^3 = 8$. Davon gibt es drei, und zwar die Untergruppen, die Isomorph zur Diedergruppe $D_4$ sind.
	\end{example}
	\begin{proof}
		[Beweis (Erster Sylowsatz):]
		Seien $C_1, \hdots, C_r$ die Konjugationsklassen von $G$. Wir können diese weiter Zerlegen in $C_1, \hdots, C_s$ mit $s < r$ und $Z(G)$. Wir wählen ein $g_i \in C_i$ und erhalten (warum?)
		\begin{align*}
			\abs{G} = \abs{G / Z_G(g_1)} + \hdots + \abs{G / Z_G(g_s)} + \abs{Z(G)}
		\end{align*}
		Wobei $Z_G(x) = \{g \in G : gxg^{-1} = x\}$.
		\newpar
		Wir führen nun einen Beweis durch Induktion durch. Gibt es ein $i$, sodass $p^r$ $\abs{Z_G(g_i)}$ teilt, dann folgt die Aussage direkt per Induktion.
		\newpar
		Andernfalls teilt $p$ $\abs{G / Z_G(g_i)}$ für alle $i$, also teilt $p$ auch $\abs{Z(G)}$, also existiert ein $z \in Z(G)$ mit $\ord(z) = p$. 
		\newpar
		Wir wissen $\abs{G} = k p^r$ mit $k < p$, also gilt $\abs{G / \angles{z}} = k p^{r-1}$. Insbesondere existiert eine Untergruppe $\bar S \subset G / \angles{z}$ mit $\abs{\bar S} = p^{r-1}$, falls $k = 1$ ist dies die Gruppe $G / \angles{z}$ selbst. Wir definieren nun $S := \pi^{-1}(\bar S) \subset G$ - also ist $S$ die Gruppe, die durch die Quotientenabbildung auf $\bar S$ abgebildet wird - und es folgt $\abs{S} = p^r$.
	\end{proof}
	\begin{lemma}
		Sei $G$ eine Gruppe. Sei
		\begin{align*}
			\cS= \{Q \subset G : \abs{Q} = p^r\}
		\end{align*}
		die Menge der $p$-Sylows von $G$. Ist $Q \in \cS$ schreiben wir $g.Q = gQg^{-1}$ oder ist $x \in\cS$ schreiben wir einfach $gx = gxg^{-1}$.
		\newpar
		Ist $H \subset G$ eine $p$-Gruppe und $x = Q\in \cS$ ein Fixpunkt von $H$ in der Menge $\cS$ der $p$-Sylows, so gilt $H \subset Q$.
	\end{lemma}
	\begin{proof}
		$\forall h \in H : hx = x$ bedeuted $\forall h \in H : hQ = Qh$. Daraus folgt, dass $HQ$ eine Untergruppe von $G$ ist.
		\newpar
		Also gilt gemäß Bahnformel $\abs{HQ} = \abs{QH / H} \cdot \abs{H}$. Wir wissen $\abs{H} = p^r$, da $QH/H$ eine $Q$-Bahn ist, gilt außerdem $\abs{QH/H} = p^s$. Also ist die Ordnung von $HQ$ ebenfalls eine $p$-Potenz. Da die $p$-Potenz, die die Ordnung von $Q$ teilt, bereits maximal ist, folgt $\abs{HQ} = \abs{Q}$, also $HQ = Q$.
	\end{proof}
	\begin{proof}[Beweis (Zweiter Sylowsatz, Dritter Sylowsatz):]
		Sei $P = x \in \cS$ eine beliebige $p$-Sylow. Es gilt $P \subset G_P$, also gemäß Bahnformel $\abs{GP} = \abs{G} / \abs{G_P}$, also teilt $p$ nicht die Ordnung von $GP$. Ist jetzt also $H \subset G$ eine $p$-Gruppe, können wir $GP$ in $H$-Bahnen $B_1, \hdots, B_r$ zerlegen. Da $H$ eine $p$-Gruppe ist, ist für alle $i$ die Ordnung $\abs{B_i}$ eine $p$-Potenz. Da $p$ aber nicht $\abs{GP}$ teilt, muss es $B_i$ mit $\abs{B} = 1$ geben. Also existiert ein $y \in GP \subset \cS$, welche Fixpunkte von $H$ sind. Also existiert ein $Q$ der Gestalt $gPg^{-1}$, welches ein Fixpunkt von $H$ ist, sodass $H \subset Q$.
	\end{proof}
	\begin{proof}[Beweis (Vierter Sylowsatz):]
		Ganz $\cS$ ist eine $G$-Bahn $GP$ mit $P \in \cS$ beliebig. Da es nur einen Fixpunkt gibt, und da alle Bahnen $p$-Potenzordnung haben, gilt $\abs{\cS} \equiv 1 \mod p$. Es gilt außerdem $\abs{GP} = \abs{G} / \abs{G_p} \mid \abs{G} / \abs{P} = \abs{G} / p^r$.
	\end{proof}
	\begin{corollary}
		Sei $G$ eine endliche Gruppe und $p$ eine Primzahl, die die Gruppenordnung teilt. So gibt es ein Element von $G$ der Ordnung $p$.
	\end{corollary}
	\begin{proof}
		Gemäß der Sylowsätze existiert ein $p$-Sylow und darin ein Element der Ordnung $p$.
	\end{proof}
	\begin{corollary}
		Jede Gruppe mit $6$ Elementen ist isomorph zu $\cS_3$ oder $\bZ / 6 \bZ$.
	\end{corollary}
	\begin{proof}
		Gemäß Sylowsätzen existiert ein $a \in G$ mit $\abs{\angles{a}} = 2$ und ein $b \in G$ mit $\abs{\angles{b}} = 3$. Also gilt
		\begin{align*}
			\angles{a} \times \angles{b} \inj G,
		\end{align*}
		also
		\begin{align*}
			G = \{1,b,b^2,a,ab,ab^2\}
		\end{align*}
		Es folgt, dass entweder $ba = ab$ oder $ba = ab^2$. Wählen wir eine dieser beiden Möglichkeiten ist bereits die gesamte Gruppe festgelegt. Der erste Fall ergibt $\bZ / 6\bZ$, der zweite Fall ergibt $\cS_3$.
	\end{proof}
	\chapter{Ringe}
	\section{Grundbegriffe}
	\begin{example}
		Die Abbildung $\bZ \to \bZ$, welche jedes Element auf $0$ abbildet, ist kein Ringhomomorphismus, da das Einselement auf das Nullelement abgebildet wird. Wohl aber ist für einen beliebigen Ring $R$ die Abbildung $R \to 0$ in den Nullring ein Ringhomomorphismus.
	\end{example}
	\begin{lemma}
		Es gibt keinen Ringhomomorphismus $\phi : \bQ \to \bZ$.
	\end{lemma}
	\begin{proof}
		Es müsste gelten $\phi(1) = 1$, also $\phi(2) = 2$, und $\phi(2) \cdot \phi(\frac{1}{2}) = 2 \cdot \phi(\frac{1}{2})= 1$. Allerdings gibt es kein solches Element in $\bZ$.
	\end{proof}
	\begin{theorem}
		Für jeden Ring $R$ gibt es genau einen Ringhomomorphismus $\bZ \to R$.
	\end{theorem}
	\begin{proof}
		Analog wie für einen Gruppenhomomorphismus aus einer zyklischen Gruppe:
		\newpar
		Es gibt genau einen Gruppenhomomorphismus $\phi :(\bZ,+) \to (R,+)$ mit $1_\bZ \to 1_R$. Dieser bildet auch einen Ringhomomorphismus, denn für natürliches $a,b$ gilt:
		\begin{align*}
			\phi(a \cdot b) 
			&= \sum_{i = 1}^{ab} 1_R\\
			&= \sum_{i = 1}^a 1_R \cdot \sum_{i = 1}^b 1_R &\textnormal{(Induktiv per Distributivgesetz)}\\
			&= \phi\lr(\sum_{i = 1}^a 1) \cdot \phi\lr(\sum_{i = 1}^b 1)\\
			&= \phi(a) \cdot \phi(b)
		\end{align*}
		Und für negative $a,b \in \bZ$ folgt das Selbe, wenn man eine Summe bis zu einer negativen Zahl als Summe der Inversen definiert.
	\end{proof}
	\begin{theorem}
		Sei $R$ ein Ring. So ist $\ker(Z \to R) = n\bZ$ für ein $n \in \bN$.  Wir nennen dieses $n$ die \tbf{Charakteristik} von $R$.
	\end{theorem}
	\begin{proof}
		Jeder Ringhomomorphismus bildet einen Gruppenhomomorphismus. Der Kern muss also eine Untergruppe von $\bZ$ sein, also die Form $n\bZ$ haben.
	\end{proof}
	\begin{theorem}
		Die Charakteristik von $R$ ist das kleinste $m \geq 1$ mit $\displaystyle \sum_{i = 1}^m 1_R = 0_R$, falls es ein solches $m$ gibt, ansonsten $0$.
	\end{theorem}
	\begin{example}
	\end{example}
	\begin{theorem}
		Die Charakteristik eines Körpers ist prim.
	\end{theorem}
	\begin{proof}
		Hätten wir einen Körper $K$ der Charakteristik $m = ab$, so wäre 
		\begin{align*}
			a \cdot 1_K + b \cdot 1_K = m \cdot 1_K = 0,
		\end{align*} (wobei $a \cdot 1_K = \sum_{i = 1}^a 1_k$), also wäre der Körper nicht nullteilerfrei. 
	\end{proof}
	\begin{theorem}
		Sei $R$ ein Kring (kommutativer Ring) der Charakteristik $p$, mit $p$ prim. So ist der \tbf{Frobeniushomomorphismus}
		\begin{align*}
			Fr : R &\to R\\
				 a &\mapsto a^p := \prod_{i = 1}^p a
		\end{align*}
		ein Ringhomomorphismus.
	\end{theorem}
	\begin{proof}
		Es gilt:
		\begin{itemize}
			\item $\phi(1) = 1)$
			\item $\phi(ab) = \phi(a)\phi(b)$
			\item 
			$\displaystyle
				\phi(a + b) 
				= (a+b)^p
				= \sum_{k = 0}^p \binom{p}{k}a^kb^{p-k}
			$
			\newpar
			In der letzten Summe haben alle Terme außer dem ersten und dem letzten nun durch den Binomialkoeffizienten einen Faktor $p$ und sind im Ring somit $0$. Also
			\begin{align*}
				\sum_{k = 0}^p \binom{p}{k}a^kb^{p-k} 
				&= a^p + b^p\\
				&= \phi(a) + \phi(b)
			\end{align*}
		\end{itemize}
	\end{proof}
	\begin{theorem}
		\theoremname{Universelle Eigenschaft surjektiver Ringhomomorphismen:} Sei $s : R \surj Q$ ein surjektiver Ringhomomorphismus und $\phi : R \to S$ ein beliebiger Ringhomomorphismus. So existiert genau dann ein Ringhomomorphismus $\bar\phi$ mit $\bar\phi \circ s = \phi$, wenn $\ker \phi \supseteq \ker s$. Falls $\bar\phi$ existiert, ist es eindeutig.
	\end{theorem}
	\begin{proof}
		Es ist bereits bekannt, dass ein eindeutiger Gruppenhomomorphismus $\bar\phi$ zwischen den additiven Gruppen existiert. Zu zeigen ist, dass $\bar\phi$ ein Ringhomomorphismus ist. Dies ist jedoch simpel - es gilt:
		\begin{itemize}
			\item $\bar\phi(1_Q) = ( \bar\phi \circ s)(1_R) = \phi(1_Q) = 1_S$
			\item Da $s$ surjektive ist, reicht für die Multiplikativität $\bar\phi(s(a) \cdot s(b)) = \bar\phi(s(a)) \cdot \bar\phi(s(b))$.
			\begin{align*}
				\bar\phi(s(a) \cdot s(b))
				&= \bar\phi(s(ab)\\
				&= \phi(ab)\\
				&= \phi(a) \cdot \phi(b)\\
				&= \bar\phi(s(a)) \cdot \bar\phi(s(b))
			\end{align*}
		\end{itemize}
	\end{proof}
	\noindent
	Wie im Fall für Gruppen existiert also genau ein Ringisomorphismus zwischen den Bildern zweier Ringhomomorphismen mit gleichem Kern. Wir fragen uns nun: Gegeben einen Ring $R$ - welche Untergruppen von $(R,+)$ sind Kerne von Ringhomomorphismen? Die Antwort sind \tbf{Ideale}.
	\begin{definition}
		Eine Untergruppe $I$ der Additiven Gruppe $(R,+)$ eines Rings $R$ heißt \tbf{Ideal}, falls
		$RI \subset I$ und $IR \subset I$ (also wenn das Produkt eines Ideals mit einem beliebigen Ringelement immer im Ideal liegt)
	\end{definition}
	\begin{definition}
		Ein zyklisches Ideal heißt \tbf{Hauptideal}.
	\end{definition}
	\begin{lemma}
		Sei $\phi : R \to S$ ein Ringhomomorphismus. So ist $\ker \phi \subset R$ ein Ideal.
	\end{lemma}
	\begin{proof}
		$\phi(x) = 0 \implies \phi(ax) = \phi(xa) = 0$
	\end{proof}
	\begin{theorem}
		Gegeben einen Ring $R$ mit Ideal $I$ gibt es einen surjektiven Ringhomomorphismus $s : R \surj Q$ mit Kern $I$.
	\end{theorem}
	\begin{proof}
		Betrachte die Gruppe $(R / I, +)$. Wir behaupten, dass es darauf genau eine Multiplikation gibt, die verträglich ist mit $\phi : R \to R / I$. Zu zeigen ist nur, dass die Abbildung $s \circ \cdot_R$ konstant auf den Fasern von $s \times s : R \times R \to R/I \times R/I$ ist. Diese Fasern haben genau die Form $(a + I) \times (b + I)$. Es gilt:
		\begin{align*}
			(a+I) \circ (b+I) &= ab + aI + Ib + II\\
			\implies s(ab + aI + Ib + II) &= ab + I
		\end{align*}
		(Da $aI + Ib + II \subset I$.)
	\end{proof}
	\begin{theorem}
		$n \in \bN$ lässt beim Teilen durch $9$ den selben Rest wie seine Quersumme und beim Teilen durch $11$ den selben Rest wie seine alternierende Quersumme.
	\end{theorem}
	\begin{proof}
		$10^n \equiv 1^n \mod 9$ und $10^n \equiv -1^n \mod 11$
	\end{proof}
	\begin{definition}
		Sei $R$ ein Kring.
		\begin{enumerate}
			\item Seien $a,b \in R$. Wir sagen, $a$ \tbf{teilt} $b$, falls ein $c \in R$ existiert, sodass $ac = b$.
			\item Ein Element $a \in R$ heißt \tbf{kürzbar}, wenn die Multiplikation $a \cdot : R \to R$ injektiv ist, also wenn $ax = ay \implies x = y$.
			\item $R$ heißt \tbf{Integritätskring}, wenn genau ein nicht kürzbare Element in $R$ existiert (Dieses ist dann zwingend die $0_R$).
		\end{enumerate}
	\end{definition}
	\begin{example}
		\phantom{}
		\begin{itemize}
			\item Alle $a \in \bZ \setminus 0$ sind kürzbar.
			\item $\bZ / 12\bZ$ ist kein Integritätsring, da $3 \cdot 4 = 3 \cdot 8 = 3 \cdot 0$. Im Allgemeinen ist $\bZ / m\bZ$ genau dann ein Integritätsring, wenn $m$ prim ist.
			\item Jeder Körper ist ein Integritätsring.
			\item Jeder Teilring eines Körpers ist ein Intergritätsring.
		\end{itemize}
	\end{example}
	\begin{corollary}
		Die Ringmultiplikation ist genau dann injektiv, wenn ihr Kern trivial ist, also wenn der Ring Nullteilerfrei ist.
	\end{corollary}
	\begin{lemma}
		Jeder endliche kommutative Integritätsring $R$ ist ein Körper.
	\end{lemma}
	\begin{proof}
		Sei $a \in R \setminus 0$ kürzbar. Dann ist die Multiplikation mit $a$ injektiv. Da $R$ endlich ist, ist die Multiplikation auch bijektiv, also existiert ein multiplikatives Inverses. Jeder kommutative Ring mit multiplikativen Inversen ist per Definition ein Körper.
	\end{proof}
	\begin{corollary}
		Die Charakteristik eines Körpers ist $0$ oder eine Primzahl.
	\end{corollary}
	\begin{proof}
		$\bZ / (\char K) \bZ \inj K$ ist injektiv. Dies ist nur möglich, wenn $(\char K)$ $0$ oder prim ist.
	\end{proof}
	\begin{definition}
		Sei $R$ ein kommutativer Ring. $a \in R$ heißt \tbf{Einheit}, wenn es ein multiplikatives Inverses gibt. Einheiten sind also quasi die "Einsteiler". Wir schreiben die Menge der Einheiten als $R^\times$ (da die Einheiten die größte multiplikativen Gruppe des Rings bilden).
	\end{definition}
	\begin{example}
		\phantom{}
		\begin{itemize}
			\item $\bZ^\times = \{-1,1\}$
			\item $\bQ^\times = \bQ \setminus 0$
			\item $0^\times = 0$
			\item $\bR[X]^\times = \bR^\times$
		\end{itemize}
	\end{example}
	\noindent In der Physik hat man einen eindimensionalen $\bR$-Vektorraum $V$ der Länge, der Zeit, etc. Die Einheiten sind genau die Basen dieses Vektorraums, also bildet die Multiplikation eine Bijektion $\bR \iso V$. Analog ist die Multiplikation mit einer Einheit eine Bijektion $R \iso R$.
	\begin{corollary}
		In jedem endlichen kommutativen Ring sind die Einheiten genau die kürzbaren Elemente.
	\end{corollary}
	\begin{example}
		\phantom{}
		\begin{itemize}
			\item $(\bZ / 12\bZ)^\times = \{1,5,7,11\}$
			\item $(\bZ / m\bZ)^\times = \{a \mid \ggT(a,m) = 1\}$
		\end{itemize}
	\end{example}
	\begin{definition}
		\theoremname{Diffie-Hellman-Verschlüsselung:}
		\begin{enumerate}
			\item Alice wählt eine beliebige Gruppe $G$ und eine natürliche Zahl $a \in \bN$. Sie wählt ein Element $g \in G$ und veröffentlicht $G, g^a$ und $g$.
			\item Daraufhin wählt Bob ein $b \in \bN$ und sendet $g^b$.
			\item Alice rechnet nun $(g^b)^a$ und Bob rechnet $(g^a)^b$. Dies ist der gemeinsame geheime Schlüssel.
		\end{enumerate}
		Die Sicherheit des Algorithmus beruht darauf, dass der diskrete Logarithmus (also das berechnen von $a$ aus $g^a$) im Allgemeinen sehr ineffizient ist. Die häufigste Wahl ist $G = (\bZ / p \bZ)^\times$ für eine sehr große Primzahl $p$, in besonders sicherheitskritischen Fällen wird als Gruppe oft eine elliptische Kurve über einem endlichen Körper genommen.
	\end{definition}
	\section{Polynomringe}
	\begin{definition}
		Sei $R$ ein Ring und $R[X]$ der Polynomring. Formell kann $R[X]$ auch definiert werden als die Menge aller Abbildungen $\bN \to R$, die höchstens an endlich vielen Stellen $\neq 0$ sind (diese Stellen entsprechen dann den Koeffizienten.) Wir unterscheiden zwischen \tit{Polynomen} und \tit{Polynomfunktionen} - Im Allgemeinen ist die kanonische Abbildung $\bF_p[X] \inj \Ens(\bF_p, \bF_p)$ nämlich nicht injektiv!
	\end{definition}
	\begin{lemma}
		Im Fall eines endlichen Körpers $\bF_p$ ist die kanonische Abbildung $\bF_p[X] \surj \Ens(\bF_p, \bF_p)$ surjektiv. Der Kern dieser Abbildung ist das Ideal, welches von $(x^p - x)$ erzeugt wird.
	\end{lemma}
	\begin{proposition}
		Sei $R$ ein kommutativer Ring und $c \in R$. So gibt es genau einen Ringhomomorphismus
		\begin{align*}
			E_c : R[X] \to R,
		\end{align*}
		sodass $E_c(X) = c$ und $E_c \circ \imath = \id$. (Wobei $\imath$ die kanonische Einbettung $R \to R[X]$ ist, welche ein Element als konstantes Polynom auffasst. Professor Soergel schreibt diese auch als $\textnormal{can}$.)
		\newpar
		Wir nennen $E_c$ den \tbf{Einsetzungshomomorphismus} und schreiben abkürzend 
		\begin{align*}
			P(c) := E_c(P)
		\end{align*}
	\end{proposition}
	\begin{proposition}
		Sei $\phi : R \to S$ ein Ringhomomorphismus und $c \in S$ (nicht $c \in R$!) ein Element, welches für alle $r \in R$ mit $\phi(r)$ bezüglich der Multiplikation kommutiert. So gibt es genau einen Ringhomomorphismus 
		\begin{align*}
			E_{c, \phi} : R[X] \to S
		\end{align*} mit $E_{c, \phi}[X] = c$ und $E_{c, \phi} \circ \imath = \phi$.
	\end{proposition}
	\noindent Diese Proposition verallgemeinert die Vorherige, indem sie uns zum Beispiel erlaubt, Matrizen oder komplexe Zahlen in reelle Polynome einzusetzen.
	\begin{proposition}
		Jeder Ringhomomorphismus $\phi : R \to S$ liefert einen Ringhomomorphismus $\phi[X] : R[X] \to S[X]$.
	\end{proposition}
	\begin{definition}
		Sei $R$ ein kommutativer Ring und $P \in R[X]$. Ein Element $\lambda \in R$ heißt \tbf{Nullstelle von $P$}, falls $P(\lambda) = 0$.
	\end{definition}
	\begin{definition}
		Der \tbf{Grad} eines Polynoms ist definiert als:
		\begin{align*}
			\deg(P) = 
			\begin{cases}
				n & \textnormal{Falls $P = a_nx^n + \hdots + a_0x^0, a_n \neq 0$}\\
				-\infty & \textnormal{Falls $P = 0$.}
			\end{cases}
		\end{align*}
	\end{definition}
	\begin{corollary}
		Sei $R$ ein Integritätsring und $P, Q \in R[X]$. Dann gilt
		\begin{align*}
			\deg(PQ) = \deg(P) + \deg(Q)
		\end{align*}
	\end{corollary}
	\noindent Unser Ziel ist es, folgenden Satz zu beweisen:
	\begin{theorem}
		Sei $R$ ein kommutativen Integritätsring und $P \in R[X] \setminus 0$. Dann hat $P$ höchstens $\deg(P)$ Nullstellen.
	\end{theorem}
	\begin{proofsketch}
		Die kommutativen Integritätsringe sind genau die Ringe, über denen die gewohnte Zerlegung in Linearfaktoren funktioniert.
	\end{proofsketch}
	\begin{counterexample}
		Sei $R = \Ens(M, \bR)$. Dann ist $R$ ein Ring, aber kein Integritätsring. So hat das Polynom $X^2 - X$ unendlich viele Nullstellen, nämlich alle Abbildungen $f : M \to \{0,1\}$.
	\end{counterexample}
	\begin{counterexample}
		Ebenso gilt der Satz nicht für nichtkommutative Ringe. Es existieren zum Beispiel Zahlreiche Matrixringe, welche idempotente Matrizen enthalten, also Matrizen $M$ mit $M^n = I$. Somit sind alle skalaren Vielfachen dieser Matrizen Nullstellen von $X^n$.
	\end{counterexample}
	\begin{theorem}
		Sei $k$ ein kommutativer Ring und $P,Q \in k[X]$, wobei $Q$ normiert ist (also Leitkoeffizient $1$ hat.) Dann existieren eindeutige $A,B \in k[X]$, sodass
		\begin{align*}
			P = A \cdot Q + B
		\end{align*}
		Wobei $\deg B \leq (\deg Q) - 1$
	\end{theorem}
	\begin{example}
		Der Quotientenring $\bR[X] / \angles{X^2 + 1}$ ist isomorph zu den Komplexen Zahlen (Da $X^2 + 1 = 0 \implies X^2 = -1 \implies i := X$
	\end{example}
	\begin{theorem}
		Sei $P \in \bR[X]$ ein Polynom vom Grad $2$ ohne reelle Nullstellen, und sei $\alpha \in \bC$ mit $P(\alpha) = 0$. Dann gilt $\bR[X]/\angles{P} \iso \bC$.
	\end{theorem}
	\begin{theorem}
		Sei $R$ ein Ring, $\fa_1, \hdots, \fa_r \subset R$ Ideale mit $i \neq j \implies \fa_i + \fa_j = R$. So ist die durch Restklassen gegebene Abbildung $\phi$ eine Surjektion $R \surj R/\fa_1 \times \hdots \times R/\fa_j$.
	\end{theorem}
	\begin{proof}
		Die Abbildung ist klar ein Ringhomomorphismus. Es reicht also zu zeigen, dass alle "Einheitsvektoren" $(0, \hdots, 0, 1, 0, \hdots, 0)$ im Bild liegen, da dann der Rest durch Linearkombination folgt.
		\newpar
		OBdA reicht $(1, 0, \hdots, 0) \in \im(\phi)$. Es gilt:
		\begin{align*}
			&\fa_1 + \fa_i = R\\ \implies &\exists a_i \in \fa_1, b_i \in \fa_i : a_i + b_i = 1\\
			\implies &1 - a_i = b_i\\
			\implies &\phi_1(1 - a_i) = [1 - a_i]_{\fa_1} = 1,\\ &\phi_i(1 - a_i) = [b_i]_{\fa_i} = 0\\
			\implies &\phi\lr(\prod_{i \geq 2}(1 - a_i)) = (1, \hdots, 0)
		\end{align*}
	\end{proof}
	\begin{corollary}
		\theoremname{Polynominterpolation:}
		Gegeben $x_1, \hdots, x_r \in \bR$ paarweise verschieden und $y_1, \hdots, y_r \in \bR$ beliebig gibt es ein Polynom $P \in \bR[T]$, sodass $\forall i : P(x_i) = y_i$.
	\end{corollary}
	\begin{proof}
		% https://q.uiver.app/#q=WzAsMyxbMCwwLCJcXGJSW1RdIl0sWzIsMCwiXFxiUiJdLFsxLDIsIlxcYlJbVF0gLyBcXGFuZ2xlc3tUIC0geH0iXSxbMCwxLCJFX3giXSxbMCwyLCIiLDIseyJzdHlsZSI6eyJoZWFkIjp7Im5hbWUiOiJlcGkifX19XSxbMiwxLCJcXHNpbSIsMix7InN0eWxlIjp7ImJvZHkiOnsibmFtZSI6ImRhc2hlZCJ9fX1dXQ==
		\[\begin{tikzcd}
			{\bR[T]} && \bR \\
			\\
			& {\bR[T] / \angles{T - x}}
			\arrow["{E_x}", from=1-1, to=1-3]
			\arrow[two heads, from=1-1, to=3-2]
			\arrow["\sim"', dashed, from=3-2, to=1-3]
		\end{tikzcd}\]
		The rest should be entirely obvious and is left as an exercise to the reader. (Maybe coming later :)
	\end{proof}
	Führt man eine Polynominterpolation durch, findet man die "Basisvektoren", indem man pro $x_i$ für alle $x_j$ die Geraden durch $(x_i, y_0)$ und $(x_j, 0)$ miteinander multipliziert. Diese werden dann aufsummiert. Letztendlich führt man also die selbe Rechnung durch wie in unserem abstrakteren Beweis über Ideale.
	\begin{theorem}
		Es gilt zusätzlich $\ker \phi = \bigcap_{i = 1}^r \fa_i$. Ist $R$ kommutativ, ist dies außerdem äquivalent zu $\angles{\fa_1 \fa_2 \hdots \fa_r}$.
	\end{theorem}
	\begin{proof}
		Seien $\fa_1, \fa_2 \subset R$ und $R$ kommutativ mit $\fa_1 + \fa_2 = R$. Wir zeigen $\fa_1 \cap \fa_2 = \angles{\fa_1 \fa_2}$. Die Richtung $\fa_1 \cap \fa_2 \supseteq \angles{\fa_1 \fa_2}$ ist klar.
		\newpar
		Es existieren $a_1 \in \fa_1$, $a_2 \in \fa_2$, sodass $a_1 + a_2 = 1$. Dann gilt insbesondere 
		\begin{align*}
			\forall x \in \fa_1 \cap \fa_2 :\\
			 a_1 x + a_2 x = x
		\end{align*}
		Also $\fa_1 \cap \fa_2 \subseteq \angles{\fa_1 \fa_2}$, also auch $\fa_1 \cap \fa_2 = \angles{\fa_1 \fa_2}$. Der Rest folgt durch Induktion.
	\end{proof}
	\begin{corollary}
		\begin{align*}
			\bR[T] / \angles{X^2 - 1} \cong \bR[T] / \angles{X + 1} \times \bR[T] / \angles{X - 1} \cong \bR \times \bR
		\end{align*}
	\end{corollary}
	\section{Irreduzible Elemente und Faktorielle Ringe}
	Wir wollen nun eine "Primfaktorzerlegung" für beliebige Ringe definieren:
	\begin{definition}
		Sei $R$ ein kommutativer Ring. Ein Element $a \in R$ heißt \tbf{irreduzibel in $R$}, falls es weder eine Einheit ist noch sich als Produkt von zwei Nichteinheiten darstellen lässt, also:
		\begin{align*}
			a = bc \implies b \in R^\times \vee c \in R^\times
		\end{align*}
	\end{definition}
	\begin{definition}
		Ein kommutativer Ring $R$ heißt \tbf{faktoriell}, wenn
		\begin{enumerate}
			\item $R$ ein Integritätsring ist,
			\item Jedes $a \in R \setminus (R^\times \cup \{0\})$ ist Produkt von irreduziblen Elementen $a = p_1 p_2 \hdots p_r$
			\item Ist $p_1p_2 \hdots p_r = q_1 q_2 \hdots q_s$, so ist $s = r$ und es gibt eine Permutation $\sigma \in \cS_r$ und ein $u_i \in R^\times$, sodass für alle $i$
			\begin{align*}
				p_i = u_i q_{\sigma(i)}
			\end{align*}
		\end{enumerate}
	\end{definition}
	\begin{anmerkung}
		Körper besitzen zwar keine irreduziblen Elemente, sind aber trotzdem faktoriell, da jedes Element kürzbar ist und außerdem jedes Element eine Einheit ist, also $R \setminus (R^\times \cup \{0\}) = \emptyset$.
	\end{anmerkung}
	\begin{proposition}
		Der Ring $C(\bR, \bR)$ der stetigen Funktionen $\bR \to \bR$ ist nicht faktoriell, zum Beispiel ist die Funktion $\abs{x}$ kein Produkt von irreduziblen Funktionen, da wir die Funktion als ein beliebig langes Produkt von Wurzeln von sich selbst schreiben können.
	\end{proposition}
	\begin{proposition}
		$R = \bZ[\sqrt{-5}] = \{a + b\sqrt{5} \mid a,b \in \bZ\}$ ist nicht faktoriell.
	\end{proposition}
	\begin{proof}
		Es gilt $6 = 2 \cdot 3 = (1 + \sqrt{-5})(1 - \sqrt{-5})$. Alle dieser Faktoren sind irreduzibel.
	\end{proof}
	\begin{definition}
		Ein kommutativer Integritätsring $R$ heißt \tbf{euklidisch}, falls man "mit Rest teilen kann", also "der Euklidische Algorithmus funktioniert". Formell bedeutet das, dass es eine Abbildung 
		\begin{align*}
			\sigma : R \setminus \{0\} \to \bN
		\end{align*} gibt, sodass für alle $a,b \in R$ mit $b \neq 0$ $q,r \in R$ existieren, sodass
		\begin{align*}
			a = b \cdot q + r
		\end{align*}
		und $r = 0$ oder $\sigma(b) > \sigma(r)$.
	\end{definition}
	\begin{anmerkung}
		Die Abbildung $\sigma$ kann durch eine Abbildung in eine beliebige wohlgeordnete Menge ersetzt werden.
	\end{anmerkung}
	\begin{example}\phantom{}
		\begin{enumerate}
			\item $R = \bZ$, $\sigma(n) = \abs{n}$.
			\item $R = K[X]$ (für einen beliebigen Körper $K$), $\sigma(P) = \deg(P)$.
			\item $R = \bZ[i]$, $\sigma(a + bi) = a^2 + b^2$. (Ausführlicher später).
		\end{enumerate}
	\end{example}
	\begin{theorem}
		In einem euklidischen Ring $R$ ist jedes Ideal $I$ ein Hauptideal.
	\end{theorem}
	\begin{proof}
		Falls $I = \angles{0}$, so sind wir fertig. Falls $I \neq \angles{0}$ gibt es ein $b \in I \setminus 0$ mit kleinstmöglichem $\sigma(b)$. So gilt $I = \angles{b}$, denn falls $a \in I \setminus Rb$, so wäre $a = b \cdot q + r$, mit $r \neq 0$, $\sigma(r) < \sigma(b)$ und $r = a - bq \in I$. Widerspruch!!!!!!
	\end{proof}
	\begin{example}
		Sei $K$ ein Körper und $A \in \Mat(n \times n, K)$. So ist
		\begin{align*}
			I = \{P \in K[X] \mid P(A) = 0\}
		\end{align*}
		ein Ideal, welches vom Minimalpolynom von $A$ erzeugt wird.
	\end{example}
	\begin{definition}
		Ein Integritätsring $R$, welcher kein Körper ist und in dem jedes Ideal ein Hauptideal ist, heißt \tbf{Hauptidealring}.
	\end{definition}
	\begin{anmerkung}
		Körper werden aus den Hauptidealringen ausgeschlossen, da man später in der kommutativen Algebra gerne hätte, dass die faktoriellen Ringe der \tit{Krull-Dimension $1$} die Hauptidealringe sind und die der Dimension $0$ die Körper.
	\end{anmerkung}
	\begin{theorem}
		Jeder Hauptidealring ist faktoriell.
	\end{theorem}
	\begin{proof}\phantom{}
		\begin{enumerate}
			\item Zeige: Jedes $r \in R \setminus (R^\times \cap \{0\})$ ist ein Produkt $r = p_1 \hdots p_r$, $r \geq 1$, mit $p_i$ irreduzibel.
			\newpar
			Sei $r \in R$ ein Gegenbeispiel. Dann gilt:
			\begin{itemize}
				\item $r \notin R^\times$.
				\item $r \neq 0$.
				\item $r$ ist nicht irreduzibel.
			\end{itemize}
			Also $r = r_1 \cdot r'$, mit $r_1, r' \notin R^\times$. Also ist entweder $r_1$ oder $r_2$ auch ein Gegenbeispiel. Ohne Beschränkung der Allgemeinheit sei dies $r_1$. Wir wissen nun, dass $\angles{r_1} \supsetneq \angles{r}$ sein muss, da $r'$ keine Einheit ist und somit $r_1$ kein Vielfaches von $r$ sein kann. Wir faktorisieren weiter und finden eine Kette
			\begin{align*}
				\angles{r} \subsetneq \angles{r_1} \subsetneq \hdots
			\end{align*}
			Da diese Ideale jeweils ineinander liegen ist diese Vereinigung ein Ideal, also insbesondere ein Hauptideal. Somit existiert ein Element $k \in R$, welches dieses Ideal erzeugt. Insbesondere existiert ein $i$, sodass $k \in \angles{r_i}$. Dann muss aber
			\begin{align*}
				\angles{r_i} = \angles{r_{i +1}} = \hdots
			\end{align*}
			gelten.
			Widerspruch!
			\item Zeige: Die Faktorisierung ist eindeutig. Wir zeigen allgemeiner: Ist $p$ irreduzibel mit $p \mid ab$, so folgt $p \mid a$ oder $p \mid b$.
			Äquivalent dazu ist: $p \mid ab \wedge \neg(p \mid a) \implies p \mid b$.
			\newpar
			Wir betrachten nun das Ideal $\angles{p,a}$. Dieses muss ein Hauptideal $\angles{c}$ sein. Dann gilt $c \mid p$ und $c \mid a$. Da $p$ irreduzibel ist gilt $c \in R^\times$ oder $c \in R^\times p$. Da $p$ $a$ nicht teilt, kann $c$ kein Vielfaches von $p$ sein. Also ist $c$ eine Einheit. Das von $p$ und $a$ erzeugte Ideal ist also $R$. Es folgt
			\begin{align*}
				xp + ya = 1,
			\end{align*}
			also
			\begin{align*}
				xpb + yab = b.
			\end{align*}
			Nun gilt trivial $p \mid xpb$. Wir wissen außerdem $p \mid ab$, also $p \mid yab$. Es folgt $p \mid b$.
			\newpar
			Die Eindeutigkeit der Faktorisierung folgt nun genau wie im Fall der ganzen Zahlen.
		\end{enumerate}
	\end{proof}
	\begin{corollary}
		Für jeden Körper $K$ existiert eine Bijektion, gegeben durch die Multiplikation, zwischen Multimengen von irreduziblen $K[X]$-Polynomen und normierten $K[X]$-Polynomen.
	\end{corollary}
	\begin{proposition}
		Über algebraisch abgeschlossenen Körpern haben alle irreduziblen Polynome den Grad $1$.
	\end{proposition}
	\begin{definition}
		Ein Ideal eines Rings heißt \tbf{echtes Ideal}, wenn es nicht der ganze Ring ist. Es heißt außerdem \tbf{maximales Ideal}, falls es keine echte Teilmenge eines echten Ideals ist.
	\end{definition}
	\begin{proposition}
		Ein Ideal ist genau dann maximal, wenn es ein von einem irreduziblen Element erzeugtes Hauptideal ist.
	\end{proposition}
	\begin{proposition}
		Ein Ring ist genau dann ein Körper, wenn das einzige maximale Ideal das Nullideal ist.
	\end{proposition}
	\begin{theorem}
		Sei $R$ ein Hauptidealring und $a \in R \setminus \{0\}$. So ist $R / \angles{a}$ genau dann ein Körper, wenn $\angles{a}$ maximal ist.
	\end{theorem}
	\section{Gaußprimzahlen und Summen zweier Quadrate}
	\begin{lemma}
		Der Ring $\bZ[i]$ der Gauß'schen Zahlen ist euklidisch und faktoriell.
	\end{lemma}
	\begin{definition}
		Wir nennen die irreduziblen Elemente von $\bZ[i]$ \tbf{Gaußprimzahlen}. Außerdem nennen wir die Einheiten von $\bZ[i]$ \tbf{Gaußeinheiten}.
	\end{definition}
	\begin{proposition}
		Es gibt genau vier Gaußeinheiten, nämlich
		\begin{align*}
			\bZ[i]^\times = \lr{1,-1,i,-i}
		\end{align*}
	\end{proposition}
	\begin{lemma}\phantom{}
		\begin{enumerate}
			\item Jede Gaußprimzahl $\pi \in \bZ[i]$ teilt genau eine Primzahl $p \in \bN$.
			\item Jede Primzahl $p \in \bN$ ist entweder Gaußprim oder zerfällt in das Produkt einer Gaußprimzahl mit ihrer Konjugierten.
		\end{enumerate}
	\end{lemma}
	\begin{proof}\phantom{}
		\begin{enumerate}
			\item Sei $\pi \in \bZ[i]$ gaußprim. So ist $\pi$ nicht Null und keine Gaußeinheit. Somit gilt $\pi \bar\pi > 1$. Aus $\pi \mid \pi \bar\pi$ folgt $\pi \mid p$ für mindestens einen Primteiler $p$ von $\pi \bar \pi$, also teilt $\pi$ mindestens eine Primzahl.
			Angenommen, es gilt $\pi \mid p$ und $\pi \mid q$ für Primzahlen $p$ und $q$. Dann folgt sofort $\pi \bar\pi \mid p^2$ und $\pi \bar \pi \mid q^2$, also $p = q$, da $\pi \bar \pi$ keine Einheit ist.
			\item Gegeben eine Primzahl $p$ würde jede Zerlegung $p = \alpha \beta \gamma$ in Nichtgaußeinheiten in $\bZ[i]$ eine Zerlegung in Nichteinheiten $p^2 = (\alpha\bar\alpha)(\beta\bar\beta)(\gamma\bar\gamma)$, woraus folgen würde, dass $p$ nicht Prim ist. Folglich ist jede Primzahl das Produkt von höchstens zwei Gaußprimzahlen $\alpha$ und $\beta$. Dann folgt sofort $p^2 = (\alpha \bar \alpha)(\beta \bar \beta)$, also $p = \alpha \bar \alpha = \beta \bar \beta$, also $\beta = \bar\alpha$.
		\end{enumerate}
		
	\end{proof}
	\begin{proposition}
		\label{prop:Gaußprim}
		Es gilt:
		\begin{itemize}
			\item $2 = (1 + i)(1 - i)$.
			\item $3$ ist Gaußprim.
			\item $5 = (1 + 2i)(1 - 2i)$.
			\item $7$ ist Gaußprim.
			\item $11$ ist Gaußprim.
			\item $13 = (2 + 3i)(2 - 3i)$
		\end{itemize}
	\end{proposition}
	\begin{anmerkung}
		Die $2$ spielt hier eine seltsame Sonderrolle, da $(1 + i) = i(1 - i)$, also $2 = i(1 - i)^2$. Die $2$ hat also bis auf Einheiten nur einen Gaußprimfaktor - alle anderen Primzahlen haben selbst bis auf Einheiten zwei Gaußprimfaktoren.
	\end{anmerkung}
	\begin{theorem}
		Für eine Primzahl $p \in \bN$ ist gleichbedeutend:
		\begin{enumerate}
			\item $p$ ist nicht gaußprim;
			\item $p$ ist eine Summe von zwei Quadraten;
			\item $p \equiv 1 \mod 4$ oder $p \equiv 2 \mod 4$;
			\item Das Polynom $X^2 + 1$ ist nicht irreduzibel in $\bF_p[X]$;
			\item $(-1)$ ist ein Quadrat in $\bF_p$.
		\end{enumerate}
	\end{theorem}
	\begin{proof}\phantom{}
		\begin{description}
			\item[$1 \Longleftrightarrow 2$:] Sei $p$ nicht gaußprim. So gilt $p = \pi \bar \pi$. Für $\pi = x + iy$ folgt dann $p = x^2 + y^2$. Umgekehrt folgt auch aus $p = x^2 + y^2$ direkt $p = (x + iy)(x - iy)$, also ist $p$ nicht Gaußprim.
			\item[$2 \implies 3$:] Die Quadrate in $\bZ / 4 \bZ$ sind $0$ und $1$. Somit muss eine Summe von zwei Quadraten modulo $4$ kongruent zu $0,1$ oder $2$ sein. Ist die Summe $0$, so ist $p$ ein Vielfaches von $4$, also nicht prim.
			\item[$1 \Longleftrightarrow 4$:] Wir betrachten folgendes Diagramm:
			% https://q.uiver.app/#q=
			\[\begin{tikzcd}[column sep = small]
				&& {\bZ[X]} \\
				\\
				{\bF_p[X]} &&&& {\bZ[i]} \\
				\\
				&& { \bF_p[X]\ / \angles{X^2 + 1} = \bZ[i]\ / \angles{p}}
				\arrow["{\mod \angles{p}}", two heads, from=1-3, to=3-1]
				\arrow["{\mod \angles{X^2 + 1}}"', two heads, from=1-3, to=3-5]
				\arrow["{\mod \angles{X^2 +1}}", two heads, from=3-1, to=5-3]
				\arrow["{\mod \angles{p}}"', two heads, from=3-5, to=5-3]
			\end{tikzcd}\]
			dieses kommutiert gemäß universeller Eigenschaft der Quotientenabbildung.
			\newpar
			Somit ist $\angles{X^2 + 1}$ genau dann nicht irreduzibel, wenn $\bZ[i] / \angles{p}$ kein Körper ist, also wenn $p$ nicht irreduzibel ist.
			\item[$3 \implies 5$:] Sei $p \equiv 1 \mod 4$. So gilt $p - 1 = 4m$, also
			\begin{align*}
				\abs{\bF_p^\times} = p - 1 = 4m,
			\end{align*}
			also:
			\begin{align*}
				\abs{\bF_p^\times} \cong (\bZ / 4m, +)
			\end{align*}
			somit existiert ein Element $x \in \bF_p^\times$ der Ordnung $4$. Somit hat $x^2$ die Ordnung $2$. Da in $(\bZ / 4m, +)$ nur ein solches Element existieren kann, kann auch in $\bF_p^\times$ nur ein solches Element der Ordnung $2$ existieren. Wir wissen bereits, dass $-1$ in $\bF_p^\times$ die Ordnung $2$ hat, also folgt $x^2 = -1$.
			\item[$5 \implies 4$:] trivial.
		\end{description}
	\end{proof}
	\begin{corollary}
		Wir erhalten direkt einen formellen Beweis von Proposition \ref{prop:Gaußprim}.
	\end{corollary}
	\begin{corollary}
		Eine positive natürliche Zahl $n \in \bN_1$ lässt sich genau dann als Summe $n = x^2 + y^2$ zweier Quadratzahlen schreiben, wenn in der Primfaktorzerlegung $n = p_1^{r_1}p_2^{r_2}\hdots p_t^{r_t}$ die Primfaktoren, welche Kongruent zu $3$ modulo $4$ sind, alle in geraden Potenzen auftreten.
	\end{corollary}
	\section{Bruchkörper, Bewertungen und Primfaktorzerlegung in Polynomringen}
	\begin{definition}
		Sei $R$ ein Integritätsbereich. So definieren wir eine Äquivalenzrelation $\sim$ auf $R \times R^\times$ durch $(n,d) \sim (m,b)$, falls $nb = md$. So definieren wir den \tbf{Bruchkörper} $\Frac(R)$ als die Menge der Äquivalenzklassen von $\sim$, wobei wir die Äquivalenzklasse von $(n,d)$ als $\frac{n}{d}$ schreiben.
	\end{definition}
	\begin{anmerkung}
		Wir schreiben für den Bruchkörper eines Polynomrings $k[X]$ über einem Körper $k$ auch
		\begin{align*}
			\Frac\ k[X] := k(X)
		\end{align*}
	\end{anmerkung}
	\begin{proposition}
		Der Bruchkörper $\Frac(R)$ bildet einen Körper, wenn wir:
		\begin{enumerate}
			\item Summen definieren durch
			\begin{align*}
				\frac{n}{d} + \frac{m}{b} = \frac{nb + md}{db}
			\end{align*}
			\item Produkte definieren durch
			\begin{align*}
				\frac{n}{d} \cdot \frac{m}{b} = \frac{nm}{db}
			\end{align*}
		\end{enumerate}
	\end{proposition}
	\begin{anmerkung}
		Der Bruchkörper $\Frac(R)$ kann auch durch eine universelle Eigenschaft definiert werden - Sei $h : R \to F$ ein injektiver Ringhomomorphismus aus $R$ in einen Körper $F$. Dann gibt es genau einen Ringhomomorphismus $g : \Frac(R) \to F$, sodass
		\begin{align*}
			g|_R = h.
		\end{align*}
	\end{anmerkung}
	\begin{definition}
		Sei $R$ ein faktorieller Ring und $p \in R$ irreduzibel. Die \tbf{$p$-adische Bewertung} (oder einfach \tbf{$p$-Bewertung}) ist die Abbildung
		\begin{align*}
			v_p : \Frac(R) &\to \bZ \cup \{\infty\}
		\end{align*}
		mit
		\begin{align*}
			p^n \frac{a}{b} &\mapsto n
		\end{align*}
		falls $a,b \in R \setminus 0$ und teilerfremd zu $p$ (und $v_p(0) = \infty$).
	\end{definition}
	\begin{proposition}
		$v_p (xy) = v_p(x) + v_p(y)$
	\end{proposition}
	\begin{anmerkung}
		Die Abbildung $d_p(x,y) = v_p(x - y)$ ist eine Metrik. Vervollständigen wir $\bQ$ gemäß dieser Metrik erhalten wir die sogenannten \tbf{$p$-adischen Zahlen}, welche große Relevanz insbesondere in der algebraischen Zahlentheorie haben.
	\end{anmerkung}
	\begin{example}
		Sei $R = k[X]$ über einem Körper $k$. Dann ist $v_{(x - \lambda)}(P)$ genau die Vielfachheit der Nullstelle $\lambda$ in $P$.
	\end{example}
	\begin{proposition}
		Sei $a \in \Frac(R)$. Dann ist $a \in R$ genau dann, wenn für alle irreduziblen Elemente $p \in R$ $v_p(a) \geq 0$ gilt.
	\end{proposition}
	\begin{proposition}
		Sei $a \in \Frac(R)$ und $v_p(a) = 0$ für alle $p$. Dann ist $a \in R^\times$.
	\end{proposition}
	\begin{definition}
		Sei $R$ ein faktorieller Ring und $p \in R$ irreduzibel. Wir definieren die \tbf{Minimalbewertung von $P$ bei $p$} als die Abbildung:
		\begin{align*}
			w_p : (\Frac(R))[X] &\to \bZ \sqcup \lr{\infty}\\
		\sum_{i = 0}^n a_i X^i &\mapsto \min\lr{v_p(a_i) \mid 1 \leq i \leq n}
		\end{align*}
	\end{definition}
	\begin{lemma}
		\theoremname{Lemma von Gauss:} Sei $R$ ein faktorieller Ring, $p \in R$ irreduzibel, und $A,B \in (\Frac R)[X]$. So gilt
		\begin{align*}
			w_p(AB) = w_p(A) + w_p(B)
		\end{align*}
	\end{lemma}
	\begin{proof}
		Sei fürs erste $w_p(A) = w_p(B) = 0$ und $A,B \in R[X]$. Betrachten wir die Äquivalenzklassen in $(R\ / \angles{p})[X]$, so gilt $[A] \neq [0]$ und $[B] \neq [0]$ (da mindestens ein Koeffizient existiert, der kein Vielfaches einer $p$-Potenz ist, sonst wäre ja $w_p$ nicht $0$.) Es folgt $[A][B] = [AB] \neq 0$. Also existiert in $[AB]$ ebenfalls ein Term, der kein Vielfaches einer $p$-Potenz ist, also gilt $w_p(AB) = 0$.
		\newpar
		Der allgemeine Fall folgt nun durch vorheriges Multiplizieren der Polynome mit $p$-Potenzen, um sie in die gefragte Form zu bringen.
	\end{proof}
	\begin{definition}
		Sei $P \in (\Frac(P))[X]$. Wir nennen $P$ \tbf{primitiv}, oder genauer \tbf{$R$-primitiv}, wenn für alle irreduziblen Elemente $p$ gilt $w_p(P) = 0$, also wenn es kein $p$ gibt, welches alle Koeffizienten teilt.
	\end{definition}
	\begin{theorem}
		Ist $R$ faktoriell, so auch $R[X]$, und die irreduziblen Elemente sind genau:
		\begin{enumerate}
			\item Die irreduziblen Elemente von $R$,
			\item Die irreduziblen Polynome aus $(\Frac(R))[X]$, die primitiv sind.
		\end{enumerate}
	\end{theorem}
	\begin{application}
		\label{app:kreisteilungspolynome}
		Die Primfaktorzerlegung von $X^n - 1$ in $\bQ[X]$, bzw. über $\bZ[X]$. Über den komplexen Zahlen sind die Nullstellen genau die $n$-ten Einheitswurzeln $\zeta$. Die sogenannten Kreisteilungspolynome sind dann definiert als:
		\begin{align*}
			\Phi_d(x) = \prod_{\ord \zeta = d}(X - \zeta)
		\end{align*}
		Man kann nun induktiv zeigen, dass diese sogar in $\bZ[X]$ liegen, und aus unserem Satz (und viel Extraarbeit) folgt, dass sie sogar irreduzibel sind.
	\end{application}
	\begin{theorem}
		Sei $R$ faktoriell. So ist $R[X]$ ebenfalls faktoriell, und die irreduziblen Elemente sind genau:
		\begin{enumerate}
			\item Die Irreduziblen von $R$,
			\item und die primitiven Polynome von $R[X]$, die irreduzibel in $(\Frac R)[X]$ sind.
		\end{enumerate}
	\end{theorem}
	\begin{corollary}
		Sei $R$ faktoriell und $P \in R[X] \setminus R$ (also $P$ nicht konstant), sodass $P$ in $(\Frac R)[X]$ nicht irreduzibel. Dann gibt es auch $A, B \in R[X]$ von positivem Grad mit
		\begin{align*}
			P = A \cdot B
		\end{align*}
	\end{corollary}
	\begin{proofsketch}
		"Durch Widerspruch" :)
	\end{proofsketch}
	\begin{corollary}
		Sei $R$ faktoriell, so ist auch $R[X_1, \hdots, X_n]$ faktoriell.
	\end{corollary}
	\begin{proofsketch}
		Per Induktion.
	\end{proofsketch}
	\begin{theorem}
		Sei $k$ ein Körper, $f,g \in k[X,Y] \setminus \lr{0}$ teilerfremd. So ist
		\begin{align*}
			\abs{\lr{(x,y) \in k^2 \mid f(x,y) = g(x,y) = 0}} < \infty
		\end{align*}
	\end{theorem}
	\begin{proof}
		Da $f$ und $g$ teilerfremd sind gibt es in $(\Frac\ k[X])[Y]$ eine Darstellung 
		\begin{align*}
			1 = pf + qg.
		\end{align*}
		mit $p,q \in (\Frac\ k[X])[Y]$. Dann können wir an die Gleichung das Produkt $h \in k[X] \setminus 0$ der Nenner von $p$ und $q$ multiplizeren und erhalten eine Darstellung
		\begin{align*}
			h = \tilde p \cdot f + \tilde q \cdot g
		\end{align*}
		mit $\tilde p, \tilde q \in k[X,Y]$. Somit muss jede gemeinsame Nullstelle von $f$ und $g$ eine Nullstelle von $h$ sein. Da $h \in k[X] \setminus 0$ ist, gibt es davon nur endlich viele.
	\end{proof}
	\section{Kreisteilungspolynome}
	Wir wollen darauf hinarbeiten, das Polynom $X^n - 1$ in den Rationalen Zahlen zu faktorisieren. Dafür betrachten wir zuerst Faktorisierungen in den komplexen Polynomen - In Anmerkung \ref{app:kreisteilungspolynome} haben wir das sogenannte \tbf{$d$-te Kreisteilungspolynom} oder \tbf{$d$-te zyklotomische Polynom} definiert als:
	\begin{align*}
		\Phi_d(x) = \prod_{\ord \zeta_i = d}(X - \zeta_i)
	\end{align*}
	Wobei $\zeta_i$ die $n$-ten Einheitswurzeln, also die komplexen Zahlen mit $\zeta_i^n = 1$, sind, und $\ord{\zeta_i}$ im gruppentheoretischen Sinne gemeint ist, also ist $m = \ord{\zeta_i}$ die kleinste Zahl, sodass $\zeta_i^m = 1$.
	\begin{proposition}
		\label{thm:cyclotomicprod}
		Es gilt
		\begin{align*}
			X^n - 1 = \prod_{d \mid n} \Phi_d(X)
		\end{align*}
	\end{proposition}
	\begin{proposition}
		Es gilt:
		\begin{enumerate}
			\item $\Phi_1(X) = X - 1$
			\item $\Phi_2(X) = X + 1$
			\item $\Phi_3(X) \cdot \Phi_1(X) = X^3 - 1$, also $\Phi_3(X) = X^2 + X + 1$.
			\item $\Phi_4(X) = (X + i)(X - i) = X^2 + 1$.
		\end{enumerate}
	\end{proposition}
	\begin{theorem}
		Die Kreisteilungspolynome sind in $\bZ[X]$ enthalten und normiert.
	\end{theorem}
	\begin{proof}
		Per Induktion über Proposition \ref{thm:cyclotomicprod}.
	\end{proof}
	Wir wollen letzendlich zeigen, dass alle Kreisteilungspolynome irreduzibel in $\bQ[X]$ und $\bZ[X]$ sind. Dafür brauchen wir aber noch einige Hilfssätze.
	\begin{theorem}
		\theoremname{(Eisenstein-Kriterium)} Sei 
		\begin{align*}
			P = \sum_{i = 1}^n a_i X^i \in \bZ[X].
		\end{align*} Sei $p$ eine Primzahl, sodass
		\begin{enumerate}
			\item $p \nmid a_n$,
			\item $p \mid a_i$ für alle $i < n$,
			\item und $p^2 \nmid a_0$.
		\end{enumerate}    
		Dann ist $P$ irreduzibel in $\bQ[X]$.
	\end{theorem}
	\begin{proof}
		Sei $P$ nicht irreduzibel in $\bQ[X]$. So gibt es nicht konstante $A,B \in \bZ[X]$ mit $\deg{A} = r$, $\deg{B} = s$, $n = r+s$, sodass
		\begin{align*}
			P = A \cdot B
		\end{align*}
		Es folgt
		\begin{align*}
			\ol P = \ol A \cdot \ol B
		\end{align*}
		im Quotientenring $(\bZ / p\bZ)[X]$.  Nach Annahme ist
		\begin{align*}
			\ol P = \ol a_n X^n
		\end{align*}
		Also folgt $\ol A = n X^r$ und $\ol B = m X^s$, mit $n,m \neq 0$ (und alle anderen Koeffizienten verschwinden). Es folgt (?) $p^2 \mid a_0$, was ein Widerspruch ist.
	\end{proof}
	Wenden wir nun das Eisensteinkriterium auf $\Phi_p (Y + 1)$ an, erhalten wir modulo $p$:
	\begin{align*}
		\ol \Phi_p (Y + 1) &= \frac{\ol{(Y + 1)^p} - 1}{Y + 1 - 1}\\
						   &= \frac{\ol{Y^p + 1^p - 1}}{Y}\\
						   &= \frac{Y^p}{Y}\\
						   &= Y^{p-1} \in \bZ / p\bZ
	\end{align*}
	\begin{theorem}
		Sei $p$ eine Primzahl. Dann ist $\Phi_p(X)$ irreduzibel in $\bZ[X]$.
	\end{theorem}
\section{Symmetrische Polynome}
	\begin{definition}
		Sei $k$ ein Ring. Ein Polynom $P \in k[X_1, \hdots, X_n]$ heißt symmetrisch, wenn es gleich jeder Permutation seiner Variablen ist, also $\sigma(P) = P$ für jedes $\sigma : X_i \to X_{\sigma(i)}$
	\end{definition}
	\begin{anmerkung}
		Abstrakter: Die symmetrischen Polynome sind die Polynome, die unter der Wirkung der symmetrischen Gruppe auf dem Ring $k[X_1, \hdots, X_n]$ Invariant sind.
	\end{anmerkung}
	\begin{proposition}
		Die Invarianten der Wirkung einer Gruppe $G$ auf einen Ring $R$ bilden immer ebenfalls einen Teilring von $R$, den sogenannten \tbf{Invariantenring} $R^G$.
	\end{proposition}
	\begin{definition}
		In $\bZ[X_1, \hdots, X_n]$ sind die \tbf{elementarsymmetrischen Polynome} die Polynome
		\begin{align*}
			s_i = \sum_{\abs{I} = i} \lr(\prod_{j \in I} X_j)
		\end{align*}
		Allgemeiner sind in einem kommutativen Ring $k[T]$ die koeffizienten des Polynoms $\pi_{i = 1}^n (T - \zeta_i)$ genau die elementarsymmetrischen Polynome in den $-\zeta_i$.
	\end{definition}
	\begin{theorem}
		Sei $k$ ein kommutativer Ring. Dann lässt sich jedes symmetrisch Polynom eindeutig als Polynom in den elementarsymmetrischen Polynomen darstellen.
	\end{theorem}
	\begin{proof}
		Wir notieren für $\alpha \in \bN^n$
		\begin{align*}
			X^\alpha = X_1^{\alpha_1} \hdots X_n^{\alpha_n}
		\end{align*}
		Wir betrachten die lexikographische Ordnung (quasi die "Wörterbuchordnung") $\prec$ auf $\bN^n$. Diese ist auf $\bN^n$ (mit $n \in \bN$, also endlich) eine Wohlordnung - jede nichtleere Teilmenge hat ein kleinstes Element. Gegeben
		\begin{align*}
			f = \sum_{\alpha \in \bN^n} c_\alpha X^\alpha \in k[X_1, \hdots, X_n] \setminus \lr{0}
		\end{align*}
		heiße das größte $\alpha$ mit $c_\alpha \neq 0$ sein \tbf{Leitindex}. Ist $f$ symmetrisch, So gilt für den Leitindex $\alpha_1 \geq \alpha_2 \geq \hdots \geq \alpha_n$. (Da jede Permutation des Leitkoeffizients als Koeffizient vorkommen muss, und diese Permutation die Lexikographisch größte ist).
		\newpar
		Angenommen, $k[s_1, \hdots, s_n] \neq k[X_1, \hdots, X_n]^{\cS_n}$. Sei dafür $f$ symmetrisch, aber kein Polynom symmetrischer Polynom, sodass $f$ den kleinstmöglichen Leitindex hat, also:
		\begin{align*}
			f = c_\alpha X^\alpha + \sum_{\beta \prec \alpha} c_\beta X^\beta
		\end{align*}
		Da $f$ symmetrisch ist, gilt $\alpha_1 \geq \alpha_2 \hdots \alpha_n$. Behauptung: Das Polynom
		\begin{align*}
			g := s_n^{\alpha_n} \cdot \prod_{i = 1}^{n-1} s_{i}^{\alpha_i - \alpha_{i + 1}}
		\end{align*}
		hat den selben Leitindex. (Später als Lemma refaktorisieren?). Somit hat das Polynom $h := f - c_\alpha g$ einen kleineren Leitindex. Außerdem ist $h$ kein Polynom in den elementarsymmetrischen Polynom, sonst wäre $f = h + c_\alpha$ ebenfalls ein solches. Dies widerspricht der Annahme, dass $f$ einen kleinstmöglichen Leitindex hat.
	\end{proof}
	\begin{definition}
		(Einfügen: Algebraische Unabhängigkeit)
	\end{definition}
	\begin{theorem}
		Die elementarsymmetrischen Polynome sind algebraisch unabhängig.
	\end{theorem}
	\begin{proposition}
		Das Polynom 
		\begin{align*}
			T^3 + pT + q \in k[T]
		\end{align*}
		in einem algebraisch abgeschlossenen Körper $k$ hat genau dann mehrfache Nullstellen, wenn 
		\begin{align*}
			4p^3+ 27q^2 = 0.
		\end{align*}
	\end{proposition}
	\begin{lemma}
		\begin{align*}
			(X-Y)^2(Y-Z)^2(Z-X)^2 &= s_1^2s_2^2 - 4s_1^3s_3 + 18s_1s_2s_3 - 4s_2^3 - 27s_3^2
		\end{align*}
	\end{lemma}
	\begin{lemma}
		Durch Betrachtung der Grade haben wir
		\begin{align*}
			&(X-Y)^2(Y-Z)^2(Z-X)^2\\ = &As_1^6 + Bs_1^4s_2 + Cs_1^3s_3 + Ds_1^2s_2^2 + Es_1s_2s_3 + Fs_2^3 + Gs_3^2
		\end{align*}
	\end{lemma}
	\begin{corollary}
		\begin{align*}
			T^3 + aT^2 + bT + c \in k[T]
		\end{align*} 
		in einem algebraisch abgeschlossenen Körper $k$ hat genau dann mehrfache Nullstellen, wenn
		\begin{align*}
			a^2b^2 - 4a^3c + 18abc - 4b^3 - 27c^3 = 0
		\end{align*}
	\end{corollary}
\chapter{Körper}
\section{Körpererweiterungen}
\begin{definition}
	Sei $K$ ein Körper. Wir nennen den von der leeren Menge erzeugten Teilkörper $T = (\emptyset)_K$ den \tbf{Primkörper von $K$}. Dieser ist automatisch der kleinste Unterkörper von $K$.
\end{definition}
\begin{definition}
	Eine \tbf{Körpererweiterung} ist ein paar $L \supset K$ bestehend aus einem Körper $L$ und einem Unterkörper $K$. Man schreibt dann statt $L \supset K$ meist $L / K$ und nennt $K$ den \tbf{Grundkörper} und $L$ den \tbf{Erweiterungskörper} oder \tbf{Oberkörper} der Körpererweiterung.
\end{definition}
\begin{definition}
	Sei $L / K$ eine Körpererweiterung und $\alpha \in L$. Wir nennen $\alpha$ \tbf{algebraisch über $K$}, wenn es ein vom Nullpolynom verschiedenes Polynom $0 \neq Q \in K[X]$ mit $Q(\alpha) = 0$ gibt. Ist $\alpha$ nicht algebraisch, heißt es \tbf{transzendent}.
\end{definition}
\begin{theorem}
	Sei $L / K$ eine Körpererweiterung, $\alpha \in L$.
	\begin{enumerate}
		\item Ist $\alpha$ transzendent, so ist der Einsetzungshomomorphismus $\phi_\alpha : K[X] \to L$ injektiv. Desweiteren induziert $\phi_\alpha$ einen Körperhomomorphismus $\tilde \phi_\alpha : \Frac(K[X]) \to L$ mit Bild $K(\alpha)$, also dem kleinsten von $K$ und $\alpha$ erzeugten Unterkörper von $L$
		\item Ist $\alpha$ algebraisch über $K$, so ist $\ker(\phi_\alpha) \neq \lr{0}$. So gibt es in $K[X] \setminus \lr{0}$ unter allen $P$ mit $P(\alpha) = 0$ genau ein normiertes Polynom von kleinstem Grad. Wir nennen dieses das \tbf{Minimalpolynom} $\Irr(\alpha,K)$.
	\end{enumerate}
\end{theorem}
\begin{proof}
	\begin{enumerate}
		\item (...)
		\item $K[X]$ ist ein Hauptidealring. Somit existiert mindestens ein $Q \in K[X] \setminus \lr{0}$ mit $\ker(\phi_\alpha) = \angles{Q}$. Wir wählen ein $Q$ mit kleinstmöglichem Grad und normieren es und erhalten das Minimalpolynom.
	\end{enumerate}
\end{proof}
\begin{lemma}
	Jedes Polynom $P \in K[X]$ mit $P(\alpha) = 0$ ist ein Vielfaches des Minimalpolynoms. 
\end{lemma}
\begin{proofsketch}
	"Ist klar."
\end{proofsketch}
\begin{corollary}
	Ist $Q \in K[X] ~\setminus 0$ irreduzibel und normiert mit $Q(\alpha) = 0$, so ist $Q$ bereits das Minimalpolynom.
\end{corollary}
\begin{lemma}
	Das Minimalpolynom ist $K[X]$-irreduzibel.
\end{lemma}
\begin{proof}
	Gemäß Isomorphiesatz für Quotientenringe gilt 
	\begin{align*}
		(K[X] ~/ \angles{Q}) = (K[X] ~/ \ker(\phi_\alpha)) \cong \Im(\phi_\alpha) = K[\alpha] \subset L
	\end{align*}
	Da $K[\alpha] \subset L$ ist $K[\alpha]$ ein kommutativer Integritätsring, also ist $K[X] ~/ Q$ ebenfalls ein kommutativer Integritätsring. Wäre $Q$ nun nicht irreduzibel in $K[X]$, so wären die Faktoren Nullteiler.
\end{proof}
\begin{lemma}
	Ist $d = \deg(\Irr(\alpha,K))$, so ist $\alpha^0, \alpha^1, \hdots, \alpha^{d-1}$ eine $K$-Basis von $K(\alpha)$.
\end{lemma}
\begin{example}
	\phantom{}
	\begin{enumerate}
		\item $\Irr(5,\bQ) = X - 5$
		\item $\Irr(0,\bQ) = X$
		\item $\Irr(i,\bR) = X^2 + 1$
		\item $\Irr(i,\bC) = X - i$
		\item $\Irr(\sqrt{5}, \bQ) = X^2 - 5$
		\item $\Irr(\sqrt[4]{5}, \bQ) = X^4 - 5$ (Irreduzibel gemäß Eisenstein)
		\item $\Irr(\sqrt[4]{5}, \bQ) = \Irr(\sqrt{2}, \bQ) = X^2 - 2$
	\end{enumerate}
\end{example}
\begin{definition}
	Sei $L / K$ eine Körpererweiterung. Wir definieren den \tbf{Grad der Körperwereiterung} als
	\begin{align*}
		[L : K] = \dim_K L,
	\end{align*}
	also den Grad von $L$ als $K$-Vektorraum.
\end{definition}
\begin{example}
	\phantom{}
	\begin{enumerate}
		\item $[\bC : \bR] = \dim_\bR \bC = 2$
		\item $[\bR : \bQ] = \dim_\bQ \bR = \infty$ (muss sein, da $\bQ$ abzählbar und $\bR$ überabzählbar)
	\end{enumerate}
\end{example}
\begin{proposition}
	\phantom{}
	\begin{enumerate}
		\item Ist $\alpha \in L$ algebraisch über $K$, so ist 
		\begin{align*}
			[K(\alpha) : K] = \deg(\Irr(\alpha,K)).
		\end{align*}
		\item Ist $\alpha \in L$ transzendent über $K$, so ist
		\begin{align*}
			[K(\alpha) : K] = \infty.
		\end{align*}
	\end{enumerate}
\end{proposition}
\begin{proof}
	\phantom{}
	\begin{enumerate}
		\item (?)
		\item Ist $[K(\alpha) : K] < \infty$, so können $\alpha^0, \alpha^1, \hdots$ nicht $K$-linear unabhängig sein. Somit gibt es eine nichttriviale Linearkombination dieser Elemente, welche $0$ ergibt. Diese ist genau ein Polynom mit $\alpha$ als Nullstelle, also ist $\alpha$ in diesem Fall nicht transzendent.
	\end{enumerate}
\end{proof}
\begin{lemma}
	Sei $L / K$, $\alpha \in L$. Die f2olgenden Aussagen sind äquivalent:
	\begin{enumerate}
		\item $\alpha$ ist algebraisch über $K$
		\item $[K(\alpha) : K] < \infty$
	\end{enumerate}
\end{lemma}
\begin{proposition}
	Bis auf einen Sonderfall entsteht jede Körpererweiterung mit Grad $2$ durch Adjunktion einer Quadratwurzel. Genauer: Sei $L / K$ eine Körpererweiterung und $\char(K) \neq 2$. Dann ist $[L : K] = 2$ genau dann, wenn es $\alpha \in L \setminus K$ gibt, sodass $\alpha^2 \in K$ und $L = K(\alpha)$
\end{proposition}
\begin{theorem}
	\theoremname{(Turmregel)}
	Seien $M \supset L \supset K$ Körper. Dann gilt:
	\begin{align*}
		[M : K] = [M : L][L : K]
	\end{align*}
\end{theorem}
\begin{proofsketch}
	Sei $m_1, \hdots, m_r$ eine Basis von $M$ als $L$-Vektorraum und $l_1, \hdots, l_k$ eine Basis von $L$ als $K$-Vektorraum. Behauptung: $(m_il_j)$ ist eine $K$-Bais von $M$.
\end{proofsketch}
\begin{corollary}
	Gegeben eine endliche Körpererweiterung ist jedes Element des Oberkörpers algebraisch über dem Unterkörper und sein Grad über dem Unterkörper teilt den Grad der Körpererweiterung.
\end{corollary}
\begin{corollary}
	Es gilt $\sqrt{2} \notin \bQ(\sqrt[3]{2})$.
\end{corollary}
\begin{proof}
	Das Minimalpolynom von $\sqrt{2}$ in $\bQ$ ist $X^2 - 2$ und das Minimalpolynom von $\sqrt[3]{2}$ ist $X^3 - 2$, also hat $\sqrt{2}$ den Grad $2$ und $\sqrt[3]{2}$ den Grad $3$. Wäre nun $\sqrt{2} \in \bQ(\sqrt[3]{2})$, so hätten wir $\bQ(\sqrt[3]{2}) \supset \bQ(\sqrt{2}) \supset \bQ$, also per Turmregel $[\bQ(\sqrt[3]{2}) : \bQ(\sqrt{2})] \cdot 2 = 3$.
\end{proof}
\begin{corollary}
	Seien $L / K$ eine Körpererweiterung und $\alpha_1, \hdots, \alpha_n \in L$ algebraisch über $K$. So ist $K(\alpha_1, \hdots, \alpha_n)$ endlich über $K$ und insbesondere sind alle Elemente dieser Körpererweiterung auch algebraisch über $K$.
\end{corollary}
\section{Konstruktionen mit Zirkel und Lineal}
\begin{definition}
	Sei $E \subset \bC$.
	\begin{enumerate}
		\item Eine reelle affine Gerade durch zwei verschiedene Punkte von $E$ heißt eine \tit{aus $E$ elementar konstruierbare Gerade}
		\item Ein Kreis durch einen Punkt von $E$ mit Mittelpunkt in einem anderen Punkt von $E$ heißt ein \tit{aus $E$ elementar konstruierbarer Kreis}
		\item Alle aus $E$ elementar konstruierbaren Geraden und Kreise nennen wir die \tit{aus $E$ konstruierbaren Figuren}
		\item Ein Punkt $z \in \bC$ heißt \tit{Elementar konstruierbar aus $E$}, wenn er im Schnitt von zwei verschiedenen, aus $E$ elementar konstruierbaren Figuren liegt.
	\end{enumerate}
\end{definition}
\begin{lemma}
	\phantom{}
	\begin{enumerate}
		\item Die Senkrechte zu einer Geraden $AB$ durch einen Punkt $P$ ist aus $AB$ und $P$ elementar konstruierbar.
		\item Die Parallele zu einer Geraden $AB$ durch einen Punkt $P$ ist aus $AB$ und $P$ elementar konstruierbar.
	\end{enumerate}
\end{lemma}
\begin{theorem}
	Die Folgenden Teilmengen $K$ und $Q$ von $\bC$ sind identisch:
	\begin{enumerate}
		\item Die kleinste Teilmenge $K \subset \bC$, die $0$ und $1$ enthält, sodass jede aus $K$  elementar konstruierbare komplexe Zahl wieder in $K$ liegt (die \tbf{konstruierbaren Zahlen})
		\item Die kleinste Teilmenge $Q \subset \bC$, die sowohl ein Teilkörper ist als auch stabil unter dem Bilden von Quadratwurzeln.
	\end{enumerate}
\end{theorem}
\begin{proposition}
	Wir können $K$ und $Q$ bilden, indem wir den Schnitt aller Mengen mit den gefragten Eigenschaften nehmen. Dieser hat wieder die gefragten Eigenschaften.
\end{proposition}
\begin{lemma}
	Sei $a \in \bC^\times$. Folgende Aussagen sind äquivalent:
	\begin{enumerate}
		\item $a \in K$,
		\item $\abs{a} \in K$ und $\frac{a}{\abs{a}} \in K$,
		\item $\Re(a) \in K$ und $\Im(A) \in K$.r
	\end{enumerate}
\end{lemma}
\begin{lemma}
	$K$ ist ein Körper.
\end{lemma}
\begin{lemma}
	$K$ ist unter der Bildung von Quadratwurzeln geschlossen.
\end{lemma}
\begin{proposition}
	Sei $L/K$ eine Körpererweiterung und $K$ von Charakteristik ungleich $2$. So ist gleichbedeutend:
	\begin{enumerate}
		\item $[L : K] = 2$
		\item $\exists \alpha \in L$, sodass $\alpha \notin K$, $\alpha^2 \in K$ und $L = K(\alpha)$.
	\end{enumerate}
\end{proposition}
\begin{theorem}
	Ist $z \in Q$, so ist $z$ algebraisch über $\bQ$ mit Grad $[\bQ(z) : \bQ] = 2^r$.
\end{theorem}
\begin{proof}
	$z \in Q$ gilt genau dann, wenn es eine Körperkette $\bQ = K_0 \subset K_1 \subset \hdots \subset K_r$ gibt, sodass $z \in K_r$. An jeder Stelle wird eine Quadratwurzel adjungiert, also ist für alle $i$ $[K_{i + 1} : K_i] = 2$.
\end{proof}
\begin{corollary}
	$\sqrt[3]{2}$ ist nicht mit Zirkel und Lineal konstruierbar, also kann man zu einem gegebenen Würfel keinen Würfel mit doppeltem Volumen konstruieren.
\end{corollary}
\begin{proof}
	 Das Minimalpolynom $\Irr(\sqrt[3]{2},\bQ)$ von $\sqrt[3]{2}$ über $\bQ$ ist $X^3 - 2$, also gilt $[\bQ(\sqrt[3]{2}) : \bQ] = 3 \neq 2^r : r \in \bN$.
\end{proof}
\begin{corollary}
	$\sqrt[7]{1} = e^\frac{2\pi i}{7}$ ist nicht konstruierbar, also ist das Regelmäßige $7$-Eck nicht konstruierbar.
\end{corollary}
\begin{proof}
	$\Irr(\sqrt[7]{1}, \bQ) = \Phi_7(X) = X^6 + X^5 + \hdots + 1$
\end{proof}
\begin{corollary}
	Es gibt keine Konstruktion zu einer Dreiteilung von Winkeln.
\end{corollary}
\begin{proof}
	$\sqrt[3]{1}$ ist konstruierbar, da $\Irr(\sqrt[3]{1},\bQ) = X^2 + X + 1$, also sind $120$°-Winkel konstruierbar. Allerdings ist $\zeta_9 := \sqrt[9]{1}$ nicht konstruierbar, da $\Irr(\sqrt[9]{1}, \bQ) = X^6 + X^3 + 1$, also kann man innerhalb dieses konstruierten $120$°-Winkel keine $40$°-Winkel konstruieren.
\end{proof}
\section{Endliche Körper}
\begin{theorem}
	Jeder endliche Schiefkörper ist bereits ein Körper.
\end{theorem}
\begin{theorem}
	Die Kardinalität eines endliches Körpers ist stets eine echte Primzahlpotenz, und zu jeder echten Primzahlpotenz gibt es einen bis auf (nicht eindeutigen) Isomorphismus eindeutig bestimmten endlichen Körper mit dieser Kardinalität. Die Kardinalität ist also eine bijektive Abbildung 
	\begin{align*}
		\lr{\tn{endliche Körper modulo Isomorphismus}} \iso \lr{\tn{echte Primzahlprotenzen}}
	\end{align*}
\end{theorem}
\begin{proof}[Beweis (Teil 1)]
	Ein endlicher Körper $\bF$ hat notwendig eine positive Charakteristik $p$. Wir haben bereits bewiesen, dass diese eine Primzahl ein muss. Wir erhalten also direkt eine Einbettung $\bF_p \to \bF$.
	\newpar
	Durch diese Einbettung wird $\bF$ ein endlichdimensionaler $\bF_p$-Vektorraum (?). Mit $r = \dim_{\bF_p} \bF = [\bF : \bF_p]$ gilt dann bereits $\abs{\bF} = p^r$. Somit ist die Kardinalität eines endlichen Körpers stets eine Primzahlpotenz.
\end{proof}
Es gilt noch zu zeigen, dass die Kardinalitätsabbildung auf den endlichen Körpern bijektiv ist. Hierfür beweisen wir als Hilfsmittel erst einen weiteren Satz und zwei Hilfslemmata.
\begin{lemma}
	\theoremname{(Adjunktion von Nullstellen):} Gegeben ein Körper $K$ und ein nichtkonstantes Polynom $P \in K[X] \setminus K$ gibt es einen Körperhomomorphismus $i : K \to L$ derart, dass unter der von $i$ auf den Polynomringen induzierte Abbildung $i_{[X]} : K[X] \to L[X]$ das Bild $i_{[X]}(P)$ von $P$ eine Nullstelle in $L$ hat.
\end{lemma}
\begin{proof}
	\begin{enumerate}
		\item Angenommen, $P$ ist irreduzibel in $K[X]$. Da $K[X]$ ein Polynomring in einer Veränderlichen über einem Körper ist, ist $K[X]$ ein Hauptidealring. Somit ist der Quotientenring $L := K[X]/\angles{P}$ ein Körper.
		\newpar
		Sei $\ol Q \in L$ die Nebenklasse eines Polynoms $Q \in K[X]$. Wir betrachten den eindeutigen Körperhomomorphismus
		\begin{align*}
			i : K &\to L = K[X]/\angles{P}
			a \mapsto a
		\end{align*}
		Dann gilt:
		\begin{align*}
			(i_{[X]}(P))(\ol X) 
			&= \sum_{i = 1}^n \ol a_i \ol X^i\\
			&= \ol{\lr(\sum_{i = 1}^n a_i X^i)}\\
			&= \ol P = \ol 0
		\end{align*}
		also ist $\ol X$ eine Nullstelle des Polynoms $i_{[X]}(P) \in L[X]$.
		\item Angenommen, $P$ ist nicht irreduzibel. Dann bilden wir stattdessen den Quotientenring über einen beliebigen irreduziblen Faktor von $P$. Dadurch adjungieren wir einen Nullstelle dieses Faktors, also auch eine Nullstelle von $P$ selbst.
	\end{enumerate}
\end{proof}
Diese Konstruktion wird in manchen Quellen auch als die \tbf{Kronecker-Konstruktion} bezeichnet.
\begin{proposition}
	Sei $P \in K[X]$ irreduzibel. Sei $L \supset K$ eine Körpererweiterung von $K$ und $\alpha \in L$ eine Nullstelle von $P$. Dann induziert das Einsetzen von $\alpha$ für $X$ einen Körperisomorphismus
	\begin{align*}
		K[X]/\angles{P} \iso K(\alpha)
	\end{align*}
\end{proposition}
\begin{corollary}
	\theoremname{(Zerfällung von Polynomen in Körpererweiterungen):} Gegeben ein Körper $K$ und ein von Null verschiedenes Polynom $P \in K[X]$ gibt es eine endliche Körpererweiterung $L$ von $K$ derart, dass $P$ als Element von $L[X]$ in Linearfaktoren zerfällt.
\end{corollary}
\begin{lemma}
	Seien $p$ eine Primzahl, $q = p^r$ mit $r \geq 1$, und $L$ ein Körper der Charakteristik $p$. Zerfällt das Polynom $P(X) = X^q - X$ über dem Körper $L$ vollständig in Linearfaktoren, so bilden die Nullstellen von $P(X)$ in $L$ einen Unterkörper der Kardinalität $q$.
\end{lemma}
\begin{proof}
	Die Abbildung $F : L \to L, a \mapsto a^q$ ist ein Körperhomomorphismus. Die Nullstellen unseres Polynoms sind genau die Elemente $a$ mit $a = a^q$, also die Fixpunkte von $F$.
	\newpar
	Als Fixpunkte eines Körperautomorphismus bilden sie einen Unterkörper $\bF$ von $L$. Es bleibt nur noch zu zeigen, dass es genau $q$ Elemente hat. Da $X^q - X$ von Grad $q$ ist und in Linearfaktoren zerfällt, hat es genau dann $q$ Nullstellen, wenn es nur einfache Nullstellen hat. Ist aber  $a$ eine Nullstelle, so gilt in $\bF_q[X]$ die Gleichheit:
	\begin{align*}
		X^q - X 
		&= (X - a)^q - (X - a)\\
		&= ((X - a)^{q-1} - 1)(X-a)
	\end{align*}
	und da $a$ keine Nullstelle von $((X - a)^{q-1} - 1)$ sein kann, handelt es sich um eine einfache Nullstelle. Der erste Schritt gilt, da sich durch die Charakteristik $p$ die zusätzlichen Terme in der Binomischen Erweiterung wegfallen:
	\begin{align*}
		(X - a)^q - (X - a) &= (X^q - X) + a^q - a + p^r\lr(\sum \hdots)\\
		&= X^q - X
	\end{align*}
\end{proof}
Wir können nun auch den zweiten Teil unseres Satzes über die Kardinalität endlicher Körper beweisen:
\begin{proof}[Beweis (Kardinalität endlicher Körper, Teil 2)]
	\phantom{}
	\begin{enumerate}
		\item Als erstes müssen wir noch zeigen, dass es zu jeder echten Potenz $q$ einer Primzahl auch tatsächlich einen Körper mit $q$ Elementen gibt.
		\newpar
		Wir wissen nun, dass wir zu $\bF_p$ eine Körpererweiterung von $L$ finden, durch die das Polynom $X^{p^r} - X$ über $\bF_p[X]$ vollständig in Linearfaktoren zerfällt, und dass die Nullstellen einen Unterkörper von $L$ der Kardinalität $p^r$ bilden.
		\item Als letztes ist noch zu zeigen, dass endliche Körper der selben Kardinalität isomorph sind.
		\newpar
		Sei dafür $\bF$ ein endlicher Körper mit $q = p^r$ Elementen. So gilt durch Anwendung des kleinen Satzes von Fermat für alle $a \in \bF^\times$ $a^{q-1} = 1$, also $a^q - a = 0$ für alle $a \in \bF$. 
		\newpar
		Insbesondere sind also die Minimalpolynome der Elemente von $\bF$ über $\bF_p$ genau die $\bF_p$-irreduzibeln Faktoren des Polynoms $X^q - X \in \bF_p[X]$. Die Erzeuger der Körpererweiterung $\bF$ sind damit genau die Nullstellen der $\bF_p$-irreduziblen Faktoren $P$ vom Grad $r$ unseres Polynoms $X^q - X$. Da die multiplikative Gruppe eines endlichen Körpers zyklisch ist, gibt es solche Erzeuger. Somit folgt für das Minimalpolynom $P$ dieser Erzeuger
		\begin{align*}
			\bF \cong \bF_p[X]/\angles{P}
		\end{align*}
		für einen und jeden $\bF_p$-irreduziblen Faktor $P$ vom Grad $r$ des Polynoms $X^q - X$.
	\end{enumerate}
\end{proof}
\begin{theorem}
	\theoremname{(Unterkörper endlicher Körper):} Gegeben ein endlicher Körper $F$ liefert die Kardinalität eine Bijektion
	\begin{align*}
		\lr{\tn{Unterkörper $K \subset \bF$}} \iso \lr{q \in \bN : \exists r \in \bN : q^r = \abs{F}}
	\end{align*}
\end{theorem}
Gegeben zwei endliche Körper lässt sich also insbesondere der eine genau dann in den anderen Einbetten, wenn die Kardinalität des einen eine Potenz der Kardinalität des anderen ist.
\begin{proof}
	Wir wissen nun, dass für den Grad $d : [F : K]$ $\abs{F} = \abs{K}^d$ gilt, also liefert die Kardinalität tatsächlich eine Abbildung zwischen den beschriebenen Mengen.
	\newpar
	Weiter muss gemäß kleinem Satz von Fermat der Unterkörper $K$, falls es ihn gibt, genau aus den $(\abs{K} - 1)$-ten Einheitswurzeln von $F$ und der Null bestehen. Diese Bedingung definiert den Unterkörper eindeutig, also gibt es genau einen Unterkörper jeder solcher Kardinalität, also ist die Abbildung injektiv.
	\newpar
	Für den Nachweis der Surjektivität überlegen wir uns zunächst, dass es in $\bF_{q^r}$ stets genau $(q-1)$ Elemente der Ordnung $q-1$ gibt. Dies folgt, da $\bF^\times_{q^r}$ eine zyklische Gruppe der Ordnung $q^r-1$ ist und da $q-1$ ein Teiler von $q^{r-1}$  ist (?).
	Also gibt es in $\bF_{q^r}$ genau $q$ Lösungen der Gleichung $X^q - X$. Wir wissen inzwischen, dass diese einen Unterkörper von $\bF_{q^r}$ mit $q$ Elementen bilden.
\end{proof}
\section{Zerfallungskörper}
\begin{definition}
	Sei $K$ ein Körper, $P \in K[X] \setminus K$. $L \supset K$ heißt \tbf{minimaler Zerfallungskörper von $P$}, oder einfach \tbf{Zerfallungskörper von $P$}, wenn:
	\begin{enumerate}
		\item $P$ in $L[X]$ in Linearfaktoren zerfällt, und
		\item $L = K(\alpha \in L \mid P(\alpha) = 0)$
	\end{enumerate}
\end{definition}
\begin{theorem}
	\theoremname{(Existenz und Eindeutigkeit von Zerfallungskörpern):}
	Seien $K$ ein Körper und $P \in K[X] \setminus 0$. So existieren Zerfallungskörper von $P$, und sind $M / K$ und $M'/K$ zwei Zerfallungskörper von $P$, so gibt es einen Körperisomorphismus $M \iso M'$, der eingeschränkt auf $K$ die Identität induziert.
\end{theorem}
\begin{proposition}
	Gegeben eine Körpererweiterung $j : K \inj M$ und eine primitive algebraische Erweiterung $K(\alpha)$ ihres Unterkörpers $K$ liefert das Auswerten an $\alpha$ eine Bijektion
	\begin{align*}
		\tn{Kring}^K(K(\alpha, M) &\iso \lr{\beta \in M \mid \Irr(\alpha, K)(\beta) = 0}\\
		\phi &\mapsto \phi(\alpha)
	\end{align*}
\end{proposition}
\begin{lemma}
	Sei ein kommutatives Dreieck von Körperhomomorphismen gegeben, sodass die linke Hälfte des folgenden Diagramms kommutiert:
	\begin{figure}[h!]
		% https://q.uiver.app/#q=WzAsNCxbMCwwLCJLIl0sWzIsMCwiTCJdLFsyLDIsIk0iXSxbNCwwLCJMKFxcYWxwaGEpIl0sWzAsMl0sWzEsMl0sWzAsMV0sWzMsMiwiIiwyLHsic3R5bGUiOnsiYm9keSI6eyJuYW1lIjoiZGFzaGVkIn19fV0sWzEsM11d
		\[\begin{tikzcd}
			K && L && {L(\alpha)} \\
			\\
			&& M
			\arrow[from=1-1, to=1-3]
			\arrow[from=1-1, to=3-3]
			\arrow[from=1-3, to=1-5]
			\arrow[from=1-3, to=3-3]
			\arrow[dashed, from=1-5, to=3-3]
		\end{tikzcd}\]
	\end{figure}
	Sei außerdem $\alpha$ algebraisch über $K$.
	\newpar
	Falls $\Irr(\alpha, K)$ über $M[X]$ vollständig in Linearfaktoren zrefällt, so gibt es einen Homomorphismus $\phi : L(\alpha) \to M$, sodass das Diagramm kommutiert.
\end{lemma}
\begin{proof}
	$\Irr(\alpha, L)$ teilt $\Irr(\alpha, K)$. Also zerfllt auf $\Irr(\alpha, L)$ in $M[X]$ und hat also Nullstellen $\beta \in M$.
\end{proof}
\begin{theorem}
	Seien $K \inj L$, $K \inj M$ Körpererweiterungen. Dann gilt:
	\begin{align*}
		\abs{\tn{Kring}^K(L,M)} \leq [L : K]
	\end{align*}
\end{theorem}
\begin{definition}
	Eine Körpererweiterung $L \supset K$ heißt \tbf{algebraisch}, wenn jedes $\alpha \in L$ algebraisch über $K$ ist.
\end{definition}
\begin{definition}
	Eine Körpererweiterung $L \supset K$ heißt \tbf{normal}, wenn sie algebraisch ist und für jedes $\alpha \in L$ das Minimalpolynom $\Irr(\alpha, K)$ über $L$ vollständig in Linearfaktoren zerfällt.
\end{definition}
\begin{theorem}
	Für eine endliche Körpererweiterung $N \supset K$ sind gleichbedeutend:
	\begin{enumerate}
		\item $N \supset K$ ist normal,
		\item $N$ ist der Zerfällungskörper eines Polynoms $P \in K[X]$.
		\item Gegeben $Q \in K[X]$ und $L/K$ Zerfällungskörper von $Q$ und $\phi, \psi \in \tn{Kring}^K(N, L)$ gilt $\phi(N) = \psi(N)$
	\end{enumerate}
\end{theorem}
\begin{proposition}
	Sei $N \supset K$ eine normale Körpererweiterung, $L \supset K$ eine beliebige Körpererweiterung. So gilt für alle $\phi, \psi \in \tn{Kring}^K(N,L)$ bereits:
	\begin{align*}
		\phi(N) = \psi(N)
	\end{align*}
\end{proposition}
\begin{proof}
	$\phi(N)$ ist der Teilkörper von $L$, der aus den in $L$ enthaltenen Nullstellen $L$ aller Minimalpolynome $\Irr(\alpha, K)$ für $\alpha \in N$ besteht.
\end{proof}
\section{Vielfachheit von Nullstellen}
\begin{definition}
	Ist $K$ ein Körper, $P \in K[X]$ ein Polynom, und $\lambda \in K$ eine Nullstelle von $P$. Dann nennen wir
	\begin{align*}
		\sup_{n \in \bN} (X - \lambda)^n \mid P
	\end{align*}
	die \tbf{Vielfachheit} oder \tbf{Ordnung} von $\lambda$.
\end{definition}
\begin{theorem}
	Ableitungsregeln für Polynome.
\end{theorem}
\begin{theorem}
	Sei $K$ ein Körper, $P \in K[X]$, $\alpha \in K$ mit $P(\alpha) = 0$. Dann ist $\alpha$ eine mehrfache Nullstelle, wenn $P'(\alpha) = 0$.
\end{theorem}
\begin{proof}
	\begin{align*}
		P(X) &= (X -\alpha)Q(X)\\
		\implies P'(X) &= Q(X) + (X - \alpha)Q'(X)\\
		\implies P'(\alpha) &= Q(\alpha)
	\end{align*}
\end{proof}
\begin{definition}
	Sei $K$ ein Körper, $f,g \in K[X]$ nicht beide Null. Betrachte das Hauptideal $\angles{f,g}_{K[X]} := \angles{d}_{K[X]}$. Dann gibt es genau ein normiertes $d$ mit diesen Eigenschaften. Wir nennen $d$ den \tbf{größtengradigen gemeinsamen normierten Teiler} von $f,g$:
	\begin{align*}
		d = \tn{ggnT}_K(f,g)
	\end{align*}
\end{definition}
\begin{proposition}
	Sei $L/K$ eine Körpererweiterung, $f,g \in K[X]$ Polynome mit $g \neq 0$. Dann gilt:
	\begin{enumerate}
		\item Polynomdivision mit Rest von $f$ durch $g$ liefert das selbe Resultat in $K[X]$ und $L[X]$.
		\item $g$ ist genau dann ein Teiler von $f$ in $L[X]$, wenn dasselbe gilt in $K[X]$.
		\item $\displaystyle \tn{ggnT}_K(f,g) = \tn{ggnT}_L(f,g)$
	\end{enumerate}
\end{proposition}
\begin{theorem}
	Sei $K$ ein Körper, $P \in K[X] \setminus 0$. Dann sind gleichbedeutend:
	\begin{enumerate}
		\item $P$ hat keine mehrfache Nullstellen in seinem Zerfällungskörper;
		\item $\tn{ggnT}_K(P,P') = 1$
	\end{enumerate}
\end{theorem}
\begin{proof}
	?
\end{proof}
\begin{theorem}
	Sei $K$ ein Körper. Gilt $\tn{char} K = 0$ oder ist $K$ endlich, so hat ein $K$-irreduzibles Polynom $P \in K[X]$ keine mehrfachen Nullstellen in seinem Zerfällungskörper $L$.
\end{theorem}
\begin{proof}
	Zeige $\ggnT(P, P') = 1$. Dies folgt jedoch direkt daraus, dass $P$ irreduzibel ist und $\deg P' < \deg P$ gilt (und gemäß $\tn{char} K = 0$ auch $P' \neq 0$ gilt).
\end{proof}
\begin{definition}
	Sei $K$ ein Körper und $P \in K[X]$. So heißt $P$ \tbf{separabel}, wenn es in seinem Zerfällungskörper $L$ keine mehrfachen Nullstellen hat.
\end{definition}
\begin{theorem}
	Sei $P \in K[X]$ irreduzibel. Dann ist gleichbedeutend:
	\begin{enumerate}
		\item $P$ ist nicht separabel,
		\item $P' = 0$.
		\item Es gilt $\tn{char} K = p > 0$ und es gibt $Q \in K[X]$ mit $P(X) = Q(X^p)$.
		\item Jede Nullstelle von $P$ in seinem Zerfällungskörper ist eine mehrfache Nullstelle.
	\end{enumerate}
\end{theorem}
\begin{definition}
	Ein Körper $K$ heißt \tbf{vollkommen} (eng. \tbf{perfect}) wenn $\tn{char} K = 0$ oder wenn $\tn{\char K} = p > 0$ und
	\begin{align*}
		\tn{Fr} : \alpha \mapsto \alpha^p
	\end{align*}
	surjektiv ist (also jedes Element eine $p$-te Wurzel hat).
\end{definition}
\begin{definition}
	Eine Körpererweiterung $L/K$ heißt \tbf{separabel}, falls sie algebraisch ist und für alle $\alpha \in L$ das Minimalpolynom von $\alpha$ über $K$ separabel ist.
\end{definition}
\begin{theorem}
	\theoremname{(Charakterisierung separabler Körpererweiterungen):} Für eine Körpererweiterung $L \supset K$ sind gleichbedeutend:
	\begin{enumerate}
		\item $L \supset K$ ist separabel,
		\item $L$ wird über $K$ von Elementen erzeugt, die separabel über $K$ sind.
	\end{enumerate}
	Ist $L \supset K$ endlich, so sind auch gleichbedeutend:
	\begin{enumerate}
		\item[3] Für jede Vergrößerung $N \supset L$ von $L$ zu einer normalen Erweiterung von $K$ beträgt die Anzahl der Körperhomomorphisen von $L$ nach $N$ genau $[L : K]$,
		\item[4] Es gibt mindestens eine Körpererweiterung $N \supset K$ von $K$, sodass es $[L : K]$ viele KKörperhomomorphismen von $L$ nach $N$ gibt.
	\end{enumerate}
\end{theorem}
\section{Satz vom primitiven Element}
\begin{lemma}
	Ein affiner Raum über einen unendlichen Körper kann nicht durch endlich viele echte affine Teilräume überdeckt werden.
\end{lemma}
\begin{lemma}
	Ein Vektorraum kann nicht durch endlich viele Untervektorräume von unendlicher Kodimension überdeckt werden.
\end{lemma}
\begin{corollary}
	Ein Körper kann nicht durch endlich viele echte Teilkörper überdeckt werden.
\end{corollary}
\begin{corollary}
	Gegeben Körpererweiterungen $L \supset K$ und $M \supset K$ desselben Grundkörpers $K$ und endlich viele paarweise verschiedene Homomorphismen $\sigma_1, \hdots, \sigma_r$ von $L$ nach $M$ gibt es stets ein Element $\alpha \in L$, sodass die Bilder $\sigma_i(\alpha)$ paarweise verschieden sind.
\end{corollary}
\begin{corollary}
	Eine Körpererweiterung $L \supset K$ ist genau dann endlich und primitiv, wenn es zwischen $L$ und $K$ nur endlich viele Zwischenkörper gibt.
\end{corollary}
\begin{proof}
	\phantom{}
	\begin{enumerate}
		\item[$\Longleftarrow$:] Gibt es nur endlich viele Zwischenkörper, so können diese nicht ganz $L$ überdecken. Es gibt also ein $\alpha \in L$, dessen erzeugter Zwischenkörper $K(\alpha)$ eine echte Körpererweiterung von $K$ ist, aber kein echter Teilkörper von $L$, also muss $K(\alpha)$ bereits $L$ sein. 
		
		$\alpha$ muss außerdem algebraisch über $K$ sein, denn sonst gäbe es unendlich viele paarweise verschiedene Zwischenkörper der Form $K(\alpha^n)$.
		
		\item[$\Longrightarrow$:] Angenommen, $L \supset K$ ist endlich und primitiv, also $L = K(\alpha)$ mit $\alpha$ algebraisch. So können wir eine Abbildung $\phi$ definieren, die jeden echten Zwischenkörper $M$ auf $\Irr(\alpha, M)$ abbildet, welches dann in $L(K)$ ein echter Teiler von $\Irr(\alpha, K)$ ist.
		
		Wir zeigen nun, dass die Abbildung injektiv ist. 
		
		Sei $M' \subset M$ der Körper, welcher über $K$ von den Koeffizienten von $\Irr(\alpha, M)$ erzeugt wird. 
		
		Dann gilt $[L : M'] = \deg(\Irr(\alpha, M')) = \deg(\Irr(\alpha, M)) = [L : M]$, also $L = M(\alpha) = M'(\alpha)$, also $M = M'$. Also existiert für jedes Minimalpolynom ein eindeutiges $M$, also ist die Abbildung injektiv.
		
		Dies folgt daraus, dass $M$ bereits von den Koeffizienten von $\Irr(\alpha, M)$ überzeugt wird, was wiederum daraus folgt, dass
		
		Nun kann $\Irr(\alpha, K)$ nur endlich viele Teiler haben, also folgt aus der Injektivität bereits, dass es nur endlich viele echte Zwischenkörper gibt.
	\end{enumerate}
\end{proof}
\begin{theorem}
	\theoremname{Satz vom primitiven Element:}
	Sei $L \supset K$ endlich und separabel. So gibt es ein $\alpha \in L$ mit $L = K(\alpha)$.
\end{theorem}

\chapter{Galoistheorie}
\begin{definition}
	Sei $L \supset K$ eine Körpererweiterung. So bildet die Menge der bijektiven Körperhomomorphismen $L \to L$, welche auf $K$ die Identität sind, eine Gruppe, welche wir die Galoisgruppe $\tn{Gal}(L/K)$ nennen.
\end{definition}
\begin{example}
	$\tn{Gal}(\bC/\bR)$ besteht aus der Identität und der komplexen Konjugation, also gilt $\tn{Gal}(\bC/\bR) \iso \bZ/2\bZ$
\end{example}
\begin{proposition}
	$\Gal(\bQ(\sqrt[3]{2})/\bQ)$ besteht nur aus der Identität.
\end{proposition}
\begin{proof}
	Die Elemente von $\Gal(\bQ(\sqrt[3]{2})/\bQ)$ sind per Definition Körperhomomorphismen $\phi : \bQ(\sqrt[3]{2}) \to \bQ(\sqrt[3]{2})$, welche $\bQ$ invariant halten. Da $\sqrt[3]{2}$ in $\bQ(\sqrt[3]{2})$ die einzige Lösung der Gleichung $x^3 = 2$ ist, muss also gelten:
	\begin{align*}
		\phi(\sqrt[3]{2}) \cdot \phi(\sqrt[3]{2}) \cdot \phi(\sqrt[3]{2}) = \phi(2) = 2
		\implies \phi(\sqrt[3]{2}) = \sqrt[3]{2}
	\end{align*}
	Somit hält $\phi$ sowohl $\bQ$ als auch $\sqrt[3]{2}$ punktweise konstant, also muss es den ganzen Körper konstant halten. Es handelt sich somit um die Identität.
\end{proof}
\begin{lemma}
	Es gilt
	\begin{align*}
		\abs{\Gal(L/K)} \leq [L : K]
	\end{align*}
\end{lemma}
\begin{theorem}
	Sei $q$ eine echte Primzahlpotenz und $r \geq 1$. So ist die Galoisgruppe $\Gal(\bF_{q^r} / \bF_q)$ eine zyklische Gruppe, erzeugt vom Frobeniushomomorphismus
	\begin{align*}
		F : \bF_{q^r} &\iso \bF_{q^r}\\
		a &\mapsto a^q
	\end{align*}
\end{theorem}
\begin{theorem}
	\theoremname{(Hauptsatz der Galoistheorie):} Sei $L$ ein Körper und $G \subset \tn{Aut}(L)$ eine endliche Untergruppe der Automorphismengruppe von $G$. Sei $L^G$ der Körper, der aus den Fixpunkten von $G$ besteht. So ist
	\begin{align*}
		\tn{Gal}(L/L^G) = G
	\end{align*}
\end{theorem}
\begin{definition}
	Eine Körpererweiterung $L/K$ heißt eine \tbf{Galoiserweiterung}, wenn sie normal und separabel ist.
\end{definition}
\begin{theorem}
	Seien $L$ ein Körper, $G \supset \tn{Aut}(G)$ endlich und $L^G$ der Fixpunktkörper von $G$. So gilt:
	\begin{enumerate}
		\item Jedes Element $\alpha \in L$ ist algebraisch über $L^G$ und sein Minimalpolynom über $L^G$ ist
		\begin{align*}
			\Irr(\alpha, L^G) = \prod_{\beta \in G\alpha} (X -\beta)
		\end{align*}
		\item Die Körpererweiterung $L/L^G$ ist eine endliche Galoiserweiterung vom Grad
		\begin{align*}
			[L : L^G] = \abs{G}
		\end{align*}
		mit Galoisgruppe
		\begin{align*}
			\tn{Gal}(L / L^G) = G
		\end{align*}
	\end{enumerate}
\end{theorem}
\begin{theorem}
	Sei $L/K$ eine endliche Körpererweiterung, $G = \tn{Gal}(L/K)$. Dann sind gleichbedeutend:
	\begin{enumerate}
		\item $L/K$ ist Galois
		\item $\abs{G} = [L : K]$
		\item $K = L^G$
	\end{enumerate}
\end{theorem}
\begin{definition}
	Eine Wirkung einer Gruppe auf einer Menge heißt \tbf{treu} (en. \tbf{faithful}), wenn nur das neutrale Element jedes Element der Menge festhält.
\end{definition}
\begin{definition}
	Eine Wirkung einer Gruppe auf einer Menge heißt \tbf{transitiv}, wenn die Menge nicht leer ist und je zwei ihrer Elemente durch ein geeignetes Gruppenelement ineinander überführt werden können.
\end{definition}
\begin{theorem}
	\theoremname{(Operation der Galoisgruppe auf Nullstellen):}
	Sei $L/K$ eine Körpererweiterung und $L$ der Zerfallungskörper von $P \in K[X] \setminus 0$ mit $P$ irreduzibel, so operiert $\Gal(L/K)$ auf der Nullstellenmenge von $P$ treu und transitiv.
\end{theorem}
\begin{definition}
	Sei $K$ ein Körper, $P \in K[X] \setminus 0$. So definieren wir die "Galoisgruppe von $P$" als die Galoisgruppe des Zerfallungskörpers von $P$.
\end{definition}
\begin{example}
	\begin{enumerate}
		
		\item Betrachte $P(X) = X^3 -2 \in \bQ[X]$, dann gilt $\bQ \subset \bQ(\sqrt[3]{2}) \supset \bQ(\sqrt[3]{2}, \sqrt[3]{1})$, wobei die erste Erweiterung Grad $3$ hat und die zweite Grad $2$, also hat die entstehende Galoisgruppe $6$ Elemente, also ist es bereits die volle Gruppe $\cS_3$. Dies ist der typische Fall - ein irreduzibles Polynom vom Grad $n$ hat in der Regel $\cS_n$ als Galoisgruppe.
		\item Betrachte $P(X) = X^3 + X^2 - 2X - 1 \in \bQ[X]$. Dieses Polynom entstand folgendermaßen:
		\begin{enumerate}
			\item $\zeta = e^{2\pi i / 7}$
			\item $\alpha = \zeta + \ol \zeta$
			\item $\beta = \zeta^2 + \ol \zeta^2$
			\item $\gamma = \zeta^3 + \ol \zeta^3$
			\item $P(X) = (X-\alpha)(X-\beta)(X-\gamma)$
		\end{enumerate}
		Es gilt also:
		\begin{enumerate}
			\item $\alpha^2 = \beta + 2$
			\item $\beta^2 = \gamma + 2$
			\item $\gamma^2 = \alpha + 2$
		\end{enumerate}
		Und Aufgrund dieser spezifischen Relation ist die Galoisgruppe letztendlich zyklisch.
	\end{enumerate}
\end{example}
\begin{theorem}
	\theoremname{(Galoiskorrespondenz):}
\end{theorem}
\begin{definition}
	\theoremname{(Biquadratische Erweiterungen):}
\end{definition}
\begin{theorem}
	\theoremname{(Fundamentalsatz der Algebra)}
\end{theorem}
\begin{theorem}
	\theoremname{(Galoisgruppen der Kreisteilungskörper):}
\end{theorem}
\begin{definition}
	\theoremname{(Eulersche $\phi$-Funktion):}
\end{definition}
\begin{theorem}
	\theoremname{(Konstruierbarkeit regelmäßiger $n$-Ecke):}
	Ein regelmäßiges $n$-Eck ist genau dann mit Zirkel und Lineal konstruierbar, wenn $\phi(n)$ eine Zweierpotenz ist.
\end{theorem}
\begin{lemma}
	\theoremname{(Rechenregeln für die Eulersche $\phi$-Funktion):}
\end{lemma}
\begin{definition}
	Eine Galoiserweiterung mit zyklischer Galoisgruppe heißt \tbf{zyklische Erweiterung}.
\end{definition}
\begin{definition}
	Eine Galoiserweiterung mit abelscher Galoisgruppe heißt \tbf{abelsche Erweiterung}.
\end{definition}
\begin{theorem}
	\theoremname{(Über zyklische Erweiterungen):}
	Seien $K$ ein Körper und $n \geq 1$ natürlich, sodass $K$ alle $n$-ten Einheitswurzeln enthält und sodass seine Charakteristik $p$ nicht $n$ teilt. So gilt:
	\begin{enumerate}
		\item Alle zyklischen Erweiterungen von $K$ vom Grad $n$ entstehen durch die Adjunktion einer $n$-ten Wurzel.
		\item Adjungieren wir zu $K$ eine $n$-te Wurzel, so erhalten wir eine zyklische Erweiterung, deren Grad $n$ teilt.
	\end{enumerate} 
\end{theorem}
\begin{proof}
	\begin{enumerate}
		\item 
		\item
		Die Gruppe $\mu_n$ der Einheitswurzeln in $K$ ist eine zyklische Gruppe isomorph zu $(\bZ/n\bZ,+)$.
		\newpar
		$(X^n-1)' = nX^{n-1}$ hat als einzige Nullstelle $0$, da $p \nmid n$. Somit hat $(X^n-1)$ nur einfache Nullstellen.
		\newpar
		Sei nun $L = K(\alpha)$ mit $\alpha^n \in K$. Dann ist $L$ der Zerfällungskörper von $\Irr(\alpha,K)$, also normal. Es gilt desweiteren:
		\begin{align*}
			\Irr(\alpha,K) \mid (X^n - \alpha^n)
		\end{align*}
		Das Polynom $(X^n - \alpha^n)$ hat als Nullstellen $\zeta_i \cdot \alpha$, wobei $\zeta_i$ $n$-te Einheitswurzeln sind. Diese Nullstellen sind paarweise verschieden, also ist $L/K$ auch separabel, also Galois.
		\newpar
		Sei $\sigma \in \Gal(K(\alpha)/K)$. Dann muss $\sigma(\alpha) = \zeta\alpha$ für $\zeta \in \mu_n$. So können wir eine injektion
		\begin{align*}
			\Gal &\to \mu_n\\
			\sigma &\mapsto \frac{\sigma(\alpha)}{\alpha}
		\end{align*}
		definieren. Diese ist sogar ein Gruppenhomomorphismus, da
		\begin{align*}
			\frac{(\sigma \tau)(\alpha)}{\alpha}
			&= \frac{\sigma(\tau(\alpha))}{\tau(\alpha)} \cdot \frac{\tau(\alpha)}{\alpha}\\
			&= \frac{\sigma(\zeta\alpha)}{\zeta\alpha} \cdot \frac{\tau(\alpha)}{\alpha}\\
			&= \frac{\zeta\sigma(\alpha)}{\zeta\alpha} \cdot \frac{\tau(\alpha)}{\alpha}\\
			&= \frac{\sigma(\alpha)}{\alpha} \cdot \frac{\tau(\alpha)}{\alpha}\\
		\end{align*}
	\end{enumerate}
\end{proof}
\begin{corollary}
	Seien $p$ eine Primzahl und $K$ ein Körper einer Charakteristik ungleich $p$, der alle $p$-ten Einheitswurzeln enthält. 
	Dann ist eine echte Erweiterung unseres Körpers genau dann Galois vom Grad $p$, wenn sie durch Adjunktion einer $p$-ten Wurzel entsteht.
\end{corollary}
($\hdots$)
\begin{definition}
	Eine Gruppe $G$ heißt \tbf{auflösbar}, wenn es eine Folge von Untergruppen
	\begin{align*}
		G = G_n \supset G_{n-1} \supset \hdots \supset G_0 = 1
	\end{align*}
	gibt, sodass $G_{i-1}$ ein Normalteiler von $G_i$ ist und $G_i/G_{i-1}$ abelsch ist.
\end{definition}
\begin{proposition}
	Ist eine endliche Gruppe $G$ auflösbar, so gibt es sogar eine solche Folge, in der $G_i/G_{i-1}$ sogar zyklisch ist.
\end{proposition}
\begin{theorem}
	Sei $\tn{char} K = 0$, $P \in K[X]$. Dann sind gleichbedeutend:
	\begin{enumerate}
		\item Die Gleichung $P(X) = 0$ lässt sich über $K$ durch Radikale auflösen,
		\item Die Galoisgruppe des Zerfällungskörpers von $P$ über $K$ ist auflösbar.
	\end{enumerate}
\end{theorem}
Wir werden für diesen Satz erst etwas Vorarbeit leisten müssen:
\begin{theorem}
	Sei $\char K = 0$.
	\begin{enumerate}
		\item Jede Radikalerweiterung $R / K$ lässt sich so in eine endliche Galoiserweiterung $L / K$ einbetten, sodass $\Gal(L/K)$ auflösbar ist.
		\item Jede endliche Galoiserweiterung $L/K$ mit auflösbarer Galoisgruppe lässt sich in eine Radikalerweiterung $R/K$ einbetten.
	\end{enumerate}
\end{theorem}
\chapter{Algebraischer Abschluss}
\begin{proposition}
	\theoremname{Auswahlaxiom:} Sei $f : X \to Y$ surjektiv. So gibt es eine Abbildung $g : Y \to X$, sodass $f \circ g = \id_Y$.
\end{proposition}
\begin{theorem}
	\theoremname{Zornsches Lemma:} Sei $(X, \leq)$ eine teilgeordnete Menge, in der jede total geordnete Teilmenge eine obere Schranke hat. So hat $X$ mindestens ein maximales Element.
\end{theorem}
\begin{corollary}
	\begin{enumerate}
		\item Jeder Körper $K$ besitzt eine algebraische Körpererweiterung $L/K$, sodass $L$ algebraisch abgeschlossen ist.
		\item Der algebraische Abschluss ist bis auf einen (nicht eindeutigen) isomorphismus eindeutig.
	\end{enumerate}
\end{corollary}
\begin{proof}
	\begin{enumerate}
		\item Sei $K$ ein Körper. Sei $\Omega$ eine Menge, sodass es für $L/K$ algebraisch keine Surjektion $L \surj \Omega$ gibt (also eine Menge mit größerer Kardinalität als alle $L$, die algebraisch über $K$ ist), z.B. existiert $L \inj K[X] \times \bN$, also wähle
		\begin{align*}
			\Omega = \cP(K[X] \times \bN).
		\end{align*}
		Betrachte nun
		\begin{align*}
			X = \lr{(M,\phi,s) \bigmid 
				\begin{array}{c} 
					M \subset \Omega\\ 
					s \tn{ Körperstruktur auf $M$}\\
					\phi : K \to M \tn{ Körperhomomorphismus, sd. $M/K$ algebraisch.}
			\end{array}}
		\end{align*}
		Durch das Zornsche Lemma finden wir $M_{max}, s_{max}, \phi_{max}$, da jedes $M$ durch $\Omega$ begrenzt ist.
	\end{enumerate}
\end{proof}
\appendix
\chapter{Axiomatik der Natürlichen Zahlen}
\begin{theorem}
	Es gibt Paare $(N, s)$ mit $N$ Menge und $s: N \inj N$ eine injektive Abbildung, sodass $N \setminus s(N)$ aus genau einem Element besteht (wir nennen dieses Element $0$) und sodass jede $s$-stabile Teilmenge $M \subset N$ bereits die ganze Menge $N$ ist.
\end{theorem}
$s$ ist hier bekannt als die Nachfolgerfunktion ("successor function"), oft auch geschrieben als $\textnormal{suc}$ oder $\textnormal{succ}$. Letztendlich wird dies im Standardmodell der natürlichen Zahlen der Abbildung $s : n \mapsto n+1$ entsprechen.
\begin{proof}
	Die Existenz einer Menge $A$ und $s: A \inj A$, sodass $A \setminus s(A) \neq \emptyset$, ist das Unendlichkeitsaxiom der Mengenlehre. Sei $N$ die kleinste $s$-stabile Teilmenge von $A$ mit $0 \in N$, also der Schnitt über alle solchen Mengen.
\end{proof}
\begin{theorem}
	Gegeben $(X,f,x)$ mit einer Menge $X$, $f : X \to X$, $x \in X$ gibt es genau ein $\psi: N \to X$ mit $\psi(0) = x$ und $\psi \circ s = f \circ \psi$
\end{theorem}
Im Standardmodell fordern wir genau $\psi(n+1) = f(\psi(n))$ und $\psi(0) = x$, also gilt $\psi(n) = f^n(x)$.
\begin{proof}
	Wir betrachten alle $G \subset N \times X$ mit $(0,x) \in G$ und $(n,y) \in G \implies (s(n),f(y)) \in G$. Sei $G'$ der Schnitt aller solcher Teilmengen. Dieses $G'$ ist dann genau der Graph von $\psi$, also die Menge von Tupeln $(n,\psi(n)) = (n,f^n(x))$.
\end{proof}
\begin{theorem}
	Ist $(N',s')$ ein weiteres Paar wie oben, so ist die Eindeutige Abbildung $\psi: N \to N'$ mit $\psi s = s'\psi$ und $\psi(0) = 0'$ eine Bijektion.
\end{theorem}
Also sind alle Modelle der natürlichen Zahlen isomorph.
\section{Die Ganzen Zahlen}
Wir wollen die Konstruktion der ganzen Zahlen aus den natürlichen Zahlen direkt in verallgemeinerter Form einführen.
\begin{theorem}
	Sei $(M, \circ)$ ein kommutatives Monoid und $N \subset M$ ein Untermonoid. So definieren wir die Struktur $N^{-1}M$ folgendermaßen:
	\begin{align*}
		N^{-1}M := M \times N / \equiv
	\end{align*}
	mit $(m,a) \equiv (n,b)$ genau dann wenn ein $x \in N$ existiert, sodass $m \circ b \circ x = n \circ a \circ x$. Falls die Monoidoperation injektiv ist, reicht bereits $m \circ b = n \circ a$.
	\newpar
	Wir erklären nun eine Verknüpfung durch
	\begin{align*}
		[(m,a)] \circ [(n,b)] = [(m \circ n, a \circ b)]
	\end{align*}
	diese ist wohldefiniert und macht $N^{-1}M$ zu einem kommutativen Monoid.
\end{theorem}
\begin{example}
	\begin{enumerate}
		\item $(\bQ, \cdot) = (\bZ \setminus \lr{0}, \cdot)^{-1} (\bZ, \cdot)$
		\item $(\bZ, +) = (\bN, +)^{-1}(\bN, +)$
	\end{enumerate}
\end{example}
\begin{proposition}
	Für jedes Monoid $M$ ist $M^{-1}M$ eine abelsche Gruppe.
\end{proposition}
\begin{theorem}
	Sei $(R, +, \geq)$ eine angeordnete abelsche Gruppe, also:
	\begin{enumerate}
		\item $\geq$ ist eine lineare Ordnung
		\item $a \geq b \implies a + c \geq b + c$
	\end{enumerate}
	Sei auf $R_{\geq 0}$ eine kommutative Monoidstruktur $\cdot$ gegeben, sodass für alle $a,b,c \in R_{> 0}$ das Distributivgesetz gilt. So gibt es genau eine Fortsetzung von $\cdot$ auf ganz $R$, sodass $(R, +, \cdot)$ einen kommutativen Ring bildet. (Insbesondere folgt direkt "$-$ mal $-$ ist $+$")
\end{theorem}
\begin{proof}[Beweis (Eindeutigkeit)]
	Wir brauchen:
	\begin{align*}
		a \cdot b =
		\begin{cases}
			0 & a = 0 \vee b = 0\\
			-((-a) \cdot b) & a < 0, b > 0\\
			-(a \cdot (-b)) & a > 0, b < 0\\
			(-a) \cdot (-b) & a < 0, b < 0
		\end{cases}
	\end{align*}
	Denn 
	\begin{align*}
		ab + (-a)b = (a + (-a)b = 0b = 0,
	\end{align*}
	also $ab = -((-a)b)$, und
	\begin{align*}
		0 &= (a + (-a))(b + (-b))\\
		  &= ab +(-a)b + a(-b) + (-a)(-b)\\
		  &= ab + -(-a)(-b) + -(-a)(-b) + (-a)(-b)\\
		  &= ab + -(-a)(-b),
	\end{align*}
	also $ab = -(-((-a)(-b))) = (-a)(-b)$
\end{proof}
\section{Die Reellen Zahlen}
\begin{align*}
	\bR := \lr{\alpha \subset \bQ : \begin{array}{c}
			\textnormal{$\alpha$ hat untere Schranke}\\
			\textnormal{$x \in \alpha$ und $y > x \implies y \in \alpha$}\\
			\textnormal{$\alpha$ hat kein kleinstes Element}
	\end{array}}
\end{align*}
\begin{theorem}
	Sei $y \subset \bR$, sodass $y$ eine untere Schranke hat. Dann gilt 
	\begin{align*}
		\inf y = \bigcup_{\alpha \in y} \alpha \in \bR
	\end{align*}
\end{theorem}
\end{document}