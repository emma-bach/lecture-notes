...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			\item Fall $1$: Gilt $s_M(q) \neq p = q$, wählen wir die Gerade durch $p$ und $s_M(q)$, und gilt $s_M(q) = p \neq q$ wählen wir analog die Gerade durch $p$ und $q$.
			\item Fall $2$: Seien $p \neq q \in M$. Sei zuerst einmal $M$ eine Gerade. Dann existiert offensichtlich ein eindeutiger senkrechter verallgemeinerter Kreis durch $p$ und $q$ - ist einer der Punkte $\infty$, erhalten wir die zu $M$ senkrechte Gerade durch $p$, und ist keiner der Punkte $\infty$, erhalten wir den eindeutigen Kreis durch den Mittelpunkt von $p$ und $q$, der $p$ und $q$ enthält.
			
			Ist $M$ ein Kreis, können wir $M$ durch eine Möbiustransformation $\phi$ auf eine Gerade abbilden und dann das Urbild des eindeutigen Kreises durch $\phi(p)$ und $\phi(q)$ nehmen.
			
			\item Fall $3$: Sei $p \neq q$ und OBdA $p \notin M$, also $s_M(p) \neq p$. Aus der Schule ist bekannt, dass durch je drei nicht kolineare Punkte ein eindeutiger Kreis geht, und durch drei kolineare Punkte geht offensichtlich eine eindeutige erweiterte Gerade. Somit geht ein eindeutiger erweiterter Kreis durch $\lr{p,s_M(p),q}$.
		...
		Wir wollen zuerst zeigen, dass jede verallgemeinerte Sphäre im $\bR^3$ analog zum Fall $\bR^2$ gegeben ist durch eine Gleichung der Form
		\begin{align*}
			x \in S \Leftrightarrow
			p\scalar{x}{x} - 2\scalar{x}{v} + q = 0
		\end{align*}
		mit $v \in \bR^3$, $p,q \in \bR$ $\scalar{v}{v} > pq$.
		\begin{itemize}
			\item Eine Ebene im $\bR^3$ ist in Hesse-Normalform gegeben als eine Nullstellenmenge
			\begin{align*}
				\scalar{x}{v} - d = 0
			\end{align*}
			mit $v \neq 0$. Formen wir um zu
			\begin{align*}
				-2\scalar{x}{v} + 2d = 0
			\end{align*}
			erhalten wir mit $q = 2d$ und $p = 0$ eine Gleichung in der Gewünschten Form, wobei $\scalar{v}{v} > pq$ aus der positiven Definitheit des Skalarprodukts folgt.
			\item Analog ist eine Sphäre mit Mittelpunkt $v$ und Radius $r > 0$ die Nullstellenmenge
			\begin{align*}
				\norm{x-v} - r = 0.
			\end{align*}
			Da beide Terme nichtnegativ sind, entspricht dies der Nullstellenmenge
			\begin{align*}
				\norm{x-v}^2 - r^2 =  \scalar{x-v}{x-v} - r^2 = 0.
			\end{align*}
			Wir formen um zu:
			\begin{align*}
				\scalar{x-v}{x-v} - r^2 
				&=
				\scalar{x}{v} - 2\scalar{x}{v} + \scalar{v}{v} - r^2
			\end{align*}
			und erhalten mit $p = 1$, $q = \scalar{v}{v} - r^2$ eine Gleichung in der gewünschten Form.
			\item Wir wollen nun zeigen, dass jede solche Nullstellenmenge eine verallgemeinerte Sphäre ist. Sei also $$p\scalar{x}{x} - 2\scalar{x}{v} + q = 0$$. Im Fall $p = 0$ erhalten wir trivial wie zuvor eine Ebene in Hesse-Normalform.
			
			Sei also $p \neq 0$. Dann teilen wir durch $p$ und erhalten
			\begin{align*}
				\scalar{x}{x} - \frac{2}{p}\scalar{x}{v} + \frac{p}{q} = 0\\
				\implies
				\scalar{x}{x} - 2\scalar{x}{\frac{v}{p}} + \frac{p}{q} = 0
			\end{align*}
		\end{itemize}
		Aus $\scalar{v}{v} > pq$ folgt $\scalar{\frac{v}{p}}{\frac{v}{p}} > \frac{q}{p}$, also existiert ein $r > 0$ mit $\frac{q}{p} = \scalar{\frac{v}{p}}{\frac{v}{p}} - r^2$. Setzen wir dies in die gegebene Gleichung ein, ergibt sich nach etwas Umformen 
		$$\norm{x - \frac{v}{p}} - r = 0,$$
		also eine Sphäre.
		\item 
		Wir können nun mit dem tatsächlichen Beweis anfangen. Wir betrachten nur das Bild an der Einheitsphäre, da das Ergebnis für Geraden trivial ist und da Spiegelungen an anderen Sphären durch Verkettungen mit Translation und Skalierungen folgen.
		
		Für $x \neq 0$ ist das Bild von $x$ unter Spiegelung an der Einheitssphäre gegeben durch $s(x) := \frac{x}{\scalar{x}{x}}$. Dann folgt:
		\begin{enumerate}
			\item
			\begin{align*}
				&p\scalar{x}{x} - 2\scalar{x}{v} + q = 0\\
				\Leftrightarrow
				&\frac{p}{\scalar{x}{x}}\scalar{x}{x} - \frac{2}{\scalar{x}{x}}\scalar{x}{v} + \frac{q}{\scalar{x}{x}} = 0\\
				\Leftrightarrow
				&p\scalar{x}{x}\scalar{\frac{x}{\scalar{x}{x}}}{\frac{x}{\scalar{x}{x}}} - 2 \scalar{\frac{x}{\scalar{x}{x}}}{v} + \frac{q}{\scalar{x}{x}} = 0
			\end{align*}
			Diese Gleichung hat die gewünschte Form, also ist das Bild jeder Sphäre eine verallgemeinerte Sphäre.
			\item Analog folgt:
			\begin{align*}
				&p\scalar{\frac{x}{\scalar{x}x}}{\frac{x}{\scalar{x}x}} - 2 \scalar{\frac{x}{\scalar{x}x}}{v} + q = 0\\
				\Leftrightarrow
				&\frac{q}{\scalar{x}{x}^2}\scalar{x}{x} - \frac{2}{\scalar{x}{x}}\scalar{x}{v} + p = 0\\
				\Leftrightarrow
				&\frac{q}{\scalar{x}{x}}\scalar{x}{x} - 2\scalar{x}{v} + p\scalar{x}{x} = 0,
			\end{align*}	
			also ist auch das Bild jeder verallgemeinerten Sphäre einer verallgemeinerte Sphäre.
		\end{enumerate}
		...
		\end{align*}
		Wir wollen zeigen, dass für jede Möbiustransformation $\phi \in \tn{Möb}_H(\hat E)$ und jeden Punkt $p \in \hat E \setminus \hat g$ außerhalb der Geraden Punkte auf der Geraden existieren, durch deren Bilder wir auch $\phi(p)$ eindeutig bestimmen können. 
		
		Wir konstruieren hierfür die Mittelsenkrechte $h$ zu $\hat g$ durch $p$ und den analogen senkrechten Kreis $K$ durch $p$, dessen Radius der Abstand von $p$ zu $\hat g$ ist. Der Mittelpunkt $m$ von $K$ ist dann auch der Schnittpunkt von $h$ mit $\hat g$. Seien $s_1$, $s_2$ die Schnittpunkte von $K$ mit $\hat g$. 
		
		Nun sind die Bilder $\phi(h)$ und $\phi(K)$ dieser Figuren unter einer Möbiustransformation $\phi$ der eindeutige auf $\hat g$ senkrechte verallgemeinerte Kreis durch $\phi(m)$ und $\phi(\infty)$ und der eindeutige auf $\hat g$ senkrechte Kreis durch $\phi(s_1)$ und $\phi(s_2)$ (Die Eindeutigkeit wurde in 7.2 bewiesen). Wir können nun $\phi(x)$ zurückgewinnen als denjenigen Schnittpunkt von $\phi(h)$ und $\phi(K)$, der auf der gleichen Seite von $\hat g$ liegt wie $x$.
		\item[7.1 - \tbf{2}]
	...
	\begin{enumerate}
		...
		\item[9.4 - \tbf{3}]
		\tit{Sei $K$ ein Körper. Die Gruppe $\GL(2,K)$ operiert auf $K^2$ und überführt Teilräume in Teilräume der selben Dimension und operiert folglich auch auf $\bP^1K$. Man zeige, dass es für je zwei Tripel $(q_1, q_2, q_3)$ und $p_1, p_2, p_3)$ aus drei paarweise verschiedenen Punkten $G \in \GL(2,K)$ gibt mit $Gq_i = p_i$. Man zeige, daß jede weitere mögliche Wahl $G'$ ein skalares Vielfaches von $G$ ist.}
		
		Unsere Punkte $p_i, q_i$ entsprechen eindimensionalen Teilräumen in $K^2$.
		
		Wir brauchen also eine invertierbare Matrix, welche drei verschiedene Ursprungsgeraden auf sich selbst abbildet. Vektoren aus je zwei der Geraden bilden eine Basis, also gibt uns die Vorschrift "bilde $q_1$ auf $p_1$ und $q_2$ auf $p_2$ ab" schonmal eine Bijektion $\phi$, welche zwei der Geraden korrekt abbildet. Sei nun $(a,b) \in \phi(q_3) \setminus 0$ und $(c,d) \in p_3 \setminus 0$. Wir können die Bilder von $q_1$ und $q_2$ beliebig skalieren, also insbesondere auch so, dass $(a,b) = (c,d)$.
		\item[10.1 - \tbf{3}]
		Die invertierbaren Abbildungen, die die Gerade durch $(1,0)$ festhalten, sind genau die invertierbaren Abbildungen, die $e_1 = (1,0)$ auf ein Vielfaches von sich selbst abbilden und $e_2 = (0,1)$ auf einen beliebigen Vektor. Dies sind genau die invertierbaren oberen Dreiecksmatrizen.
		\item[8.2 - \tbf{2}]
