...
\end{itemize}
Linear Regressions are more powerful than they seem - for example, $f(x) = (x+1)^2$ can be written as \[f(a,b) = a +2b + 1\] (such that $b = x$ and $a = x^2$). Formally, we can take any set of arbitrary \tbf{basis functions} $\phi_j(x_i)$ and arrive at a linear model
\begin{align*}
...
\begin{align*}
 y_i - f(x_i;w)
\end{align*}
For the loss function of a linear model, we just some the losses of the individual predictions. Typical loss choices are L2 loss $(y-\yhat)^2$ and L1 loss $|y - \yhat|$
\newpar
