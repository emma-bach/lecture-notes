...
\begin{multicols}{2}
	...
	\tbf{Wärmeströmungen der Ozeane:}\\
	$Q = Q_{\tn{SW}} + Q_{\tn{LW}} + Q_{\tn{Kond}} + Q_{\tn{Dampf}} + Q_{\tn{Strö}}$
\end{multicols}
...
$\Delta F \approx \alpha \cdot \ln \lr(\frac{C_0 + \Delta C}{C_0})$ mit $\alpha \approx 5,35 \frac{\tn{W}}{\m^2}$\\ 
bei Erhöhung des \ce{CO2}-Volumentanteils von $C_0$ um $\Delta C$.\\
...
\tbf{Erwärmung der Ozeane durch Strahlung}\\
$\Delta Q = F_S \cdot A \cdot \Delta T = m \cdot C \cdot \Delta T = \rho \cdot V \cdot C \cdot \Delta T = \rho \cdot A \cdot h \cdot C \cdot \Delta T$
\tbf{Wärmeenergie des Golfstroms:} $Q = C_p \cdot \rho \cdot V \cdot T$
\tbf{Wärmetransport des Golfstroms:} $\Phi_H = \dot Q = C_p \cdot \rho \cdot V \cdot \Delta T \cdot v$
\end{document}