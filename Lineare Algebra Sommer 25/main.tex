...
\begin{definition}
	Eine \tbf{$n \times m$ - Matrix} ist formell eine Abbildung aus der Produktmenge $\{1, \hdots, n\} \times \{1, \hdots m\}$ in eine Menge $M$, in der Regel einen Körper. Wir schreiben die Menge der $n \times m$-Matrizen über einem Körper $K$ als 
	\begin{align*}
		\text{Mat}(n \times m, K)
	\end{align*}
	Ist $A \in \text{Mat}(n \times m, K)$ schreiben wir den Koeffizienten $A(i,j)$ in der Regel als $A_{ij}$ oder $a_{ij}$.
\end{definition}
\begin{theorem}
	Ist die Lösungsmenge eines Linearen Gleichungssystems nichtleer, so erhalten wir alle Lösungen, indem wir zu einer Lösung des Systems beliebige Lösungen des zugehörigen Homogenen Systems addieren.
\end{theorem}
\chapter{Vektorräume}
\begin{definition}
	Ein Vektorraum $(V, K, +, \cdot)$ ist ein Tupel aus einer Menge $V$, einem Körper $K$, einer Abbildung $+ : V \times V \to V$ (genannt \tbf{Vektoraddition}), und einer Abbildung $\cdot : K \times V \to V$ (genannt \tbf{Skalarmultipliation}), sodass folgende Axiome gelten:
	\begin{itemize}
		\item $(V, +)$ ist eine abelsche (also kommutative) Gruppe
		\item $\cdot$ erfüllt folgende Rechenregeln:
		\begin{enumerate}
			\item Distributivität über die Addition:
			\begin{align*}
				\forall \lambda \in K, \forall v,w \in V:\\
...
\end{align*}
\begin{section}{Der Satz von Wünsch-dir-was}
\begin{theorem}
	Let $V$ and $W$ be vector spaces. Let $B_V$ and $V_W$ be bases. Then 
\end{theorem}
\begin{proof}
	Sei $((\vb_i)_{i \in I})$ eine Basis von $V$. Dann induzieren wir eine eindeutige lineare Abbildung durch
	\[g(f_1(\vb_i)) = f_2(\vb_i).\]
\end{proof}
\end{section}
\clearpage
\section{Universelle Eigenschaft des Quotientenraums}
Wenn $W \subseteq \text{ker}(l)$ existiert genau ein $\eta$, sodass das folgende Diagramm kommutiert:
\begin{figure}[h!]
	% https://q.uiver.app/#q=WzAsNCxbMCwwLCJWIl0sWzAsMiwiViJdLFsyLDAsIlYvVyJdLFsyLDIsIlQiXSxbMCwxLCJpZCIsMl0sWzEsMywibCIsMl0sWzAsMiwicSJdLFsyLDMsIlxcZXhpc3RzICEgXFxldGEiXV0=
	\[\begin{tikzcd}
		V && {V/W} \\
		\\
		V && W
		...
		\arrow["{\exists ! \eta}", from=1-3, to=3-3]
  ...
  \end{beobachtung}
   ...
   \end{theorem}
	...
	\begin{proof}\hphantom{hi}\newpar
		...
		\end{itemize}
		\ul{Schritt 3: Surjektivität}
		\begin{itemize}
			\item Sei eine Familien von linearen Abbildungen $(f_i)_{i \in I \in \prod(V^*)}$ gegeben. Wir brauchen eine lineare Abbildung 
			\[\varphi : \bigoplus V_i \to k\]
			Eine mögliche Wahl ist:
			\[(v_i)_{i \in I} \mapsto \sum_{i \in I} f_i(p_i((v_i)_{i \in I}))\]
			Dass diese Abbildung die richtige ist folgt aus der Kommutativität des Diagramms (This proof is left as an exercise for the reader.) - Zu zeigen ist:
			\[\eta(\varphi) = (f_i)_{i \in I}\]
			Dazu verwendet man:
			\[\eta(\varphi) = (f_i) \Leftrightarrow \forall j \in I : p_j(\eta(\varphi)) = p_j((f_i)_{i \in I})\]
		\end{itemize}
	\end{proof}
\end{section}
\begin{section}{Existenz des Tensorprodukts}
	\ul{Situation:} Gegeben Körper $k$, zwei $k$-Vektorrume $U, V$ wollen wir einen Vektorraum $T$ und eine bilineare Abbildung $\tau : U \times V \to T$ finden, sodass das folgende Diagramm kommutiert:
	 \begin{figure}[h!]
		\centering
		\begin{tikzcd}[row sep = huge, column sep = huge]
			U \times V
	...
	\end{figure}
	\newpar
	\ul{Konstruktion 1:} Betrachte den durch die Menge $U \times V$ frei erzeugten Vektorraum, also
	\[T' = k^{(U \times V)}\]
	Dieser ist identifiziert mit der Menge der Abbildungen $U \times V \to k$, die fast überall $0$ sind. Betrachte dazu
	\begin{align*}
		\tau' : U \times V &\to T'\\
			(\vu, \vv) &\mapsto e_{\vu, \vv}
	\end{align*}
	Wobei $e_{\vu, \vv}$ die Abbildung ist, welche das Paar $(\vu, \vv)$ auf $1_k$ abbildet und alle anderen Paare auf $0_k$.
	\newpar
	\ul{Eigenschaft der Konstruktion:}
	Sei $W$ irgendein Vektorraum und sei $s : U \times V \to W$ irgendeine bilineare Abbildung. Dann gibt es genau eine lineare Abbildung $\eta' : T' \to W$, sodass das Tensorproduktdiagramm kommutiert.
	\begin{proof}
		Diese Abbildung ist induziert durch $\eta'(e_{(\vu, \vv)}) = s(\vu, \vv)$. Professor Kebekus nennt dies den "Satz von Wünsch-dir-was".
	\end{proof}
	\noindent Ups, diese Konstruktion ist falsch! $\tau'$ ist nicht im geringsten bilinear!
	\newpar
	\ul{Überlegung:} Wenn $\tau'$ tatsächlich bilinear wäre, dann wäre
	\[\tau'(\lambda \vu, \vv) = \lambda \tau'(\vu,\vv)\]
	also:
	\[e_{(\lambda \vu, \vv)} - \lambda \cdot e_{(\vu, \vv)}\]
	etc.
	\newpar
	\ul{Reparatur:}
	\newpar
	Sei
	\[M_1 = \left\{e_{(\vu_1 + \lambda \vu_2, \vv)} - e_{\vu_1, \vv} - \lambda e_{\vu_2 \vv} \mid \vu_1, \vu_2 \in U, \lambda \in k, \vv \in V\right\}\]
	und
	\[M_2 = \left\{e_{(\vu, \vv_1 - \lambda \vv_2)} - e_{\vu, \vv_1} - \lambda e_{\vu, \vv_2} \mid \vu_1, \vu_2 \in U, \lambda \in k, \vv \in V\right\}\]
	Betrachte nun
	\[T = T' / \langle M_1, M_2\rangle\]
	\iffalse
	und das Diagramm
	\begin{figure}[h!]
		% https://q.uiver.app/#q=WzAsMyxbMCwwLCJVIFxcdGltZXMgViJdLFsyLDAsIlQnIl0sWzQsMCwiVCJdLFswLDEsIlxcdGF1JyJdLFsxLDIsIlxcdGV4dHtxdW90fSJdLFswLDIsIlxcdGF1IiwyLHsiY3VydmUiOjV9XV0=
		\[\begin{tikzcd}
			{U \times V} && {T'} && T
			\arrow["{\tau'}", from=1-1, to=1-3]
			\arrow["\tau"', curve={height=30pt}, from=1-1, to=1-5]
			\arrow["{\text{quot}}", from=1-3, to=1-5]
		\end{tikzcd}\]
	\end{figure}
	\fi
	und $\tau = \tau' \circ \text{quot}$. Die Quotientenabbildung teilt alles raus, was die bilinearität stört, also ist $\tau$ bilinear.
	\newpar
	Sei nun wieder ein Vektorraum $W$ und eine bilineare Abbildung $s : U \times V \to W$ gegeben. Die universelle Eigenschaft des Quotientenraums sagt, es existiert genau ein $\eta$, sodass das Quotientenraumdiagramm kommutiert, falls $\langle M_1, M_2 \rangle \subseteq \text{ker}(\eta')$. Das gilt, weil $s$ bilinear ist.
	\newpar
	Wir werden nun versuchen, das Tensorprodukt besser zu verstehen.
	\newpar
	\begin{definition}
		Seien $U, V$ Vektorräume. Wir bezeichnen das Tensorprodukt mit $U \otimes V$. Die bilineare Abbildung $\tau$ bezeichnen wir mit $\otimes : U \times V \to U \otimes V$. Die Elemente von $U \otimes V$ werden als \textbf{Tensoren} bezeichnet. Elemente im Bild von $\oplus$ nennen wir \textbf{reine Tensoren}.
	\end{definition}
	\begin{lemma}
		Der Tensor $\ve_1 \tensor \ve_1 + \ve_2 \tensor \ve_2$ ist nicht rein.
	\end{lemma}
	\begin{proof}
		Angenommen, wir hätten
		\begin{align*}
			\ve_1 \tensor \ve_1 + \ve_2 \tensor \ve_2 = \vv \tensor \vw
		\end{align*}
	\end{proof}
	\newpar
	Reine Tensoren bilden im Allgemeinen keinen Untervektorraum.
	\newpar
	Für einen reinen Tensor $t$ ist die Darstellung $t = \vu \otimes \vv$ in der Regel nicht eindeutig. Das folgt zum Beispiel bereits aus der Bilinearität von $\otimes$:
	\begin{align*}
		(2\vu) \otimes \vv = \vu \otimes (2\vv) = 2(\vu \otimes \vv)
	\end{align*}
	Es gibt aber auch viele Beispiele, in denen sich die möglichen Vektoren auf keine Weise ähnlich sind. Die Frage, ob ein gegebener Tensor rein ist, ist im Allgemeinen schwierig.
	\begin{theorem}
		Sei $R \subseteq U \otimes V$ die Menge der reinen Tensoren. Dann gilt
		\[\langle R \rangle = U \otimes V.\]
		Also ist jedes Tensorprodukt eine Linearkombination reiner Tensoren.
	\end{theorem}
	\begin{proof}
		\iffalse
		\begin{figure}[h!]
			% https://q.uiver.app/#q=
			\[\begin{tikzcd}
				{U \times V} &&& {U \otimes V} \\
				\\
				{U \times V} &&& {\langle R \rangle} \\
				\\
				{U \times V} &&& {U \otimes V}
				\arrow["{\otimes : U \times V \to U \times U\otimes V}", from=1-1, to=1-4]
				\arrow["id"', from=1-1, to=3-1]
				\arrow["{\exists ! \eta}", from=1-4, to=3-4]
				\arrow["id"', curve={height=-30pt}, from=1-4, to=5-4]
				\arrow["{\otimes : U \times V \to \langle R\rangle}"', from=3-1, to=3-4]
				\arrow["id"', from=3-1, to=5-1]
				\arrow["Inklusion"', hook, from=3-4, to=5-4]
				\arrow["\otimes"', from=5-1, to=5-4]
			\end{tikzcd}\]
		\end{figure}
		\fi
	\end{proof}
	\begin{theorem}
		Seien $(u_i)_{i \in I}$ und $(v_i)_{i \in J}$ Basen von $U$ und $V$. Dann ist 
		\[(u_i \tensor v_j)_{(i,j) \in I \times J}\]
		eine Basis des Tensorprodukts.
	\end{theorem}
	\begin{beispiel}
		\begin{align*}
			U^* \tensor V &\to \Hom(U,V)\\
			\phi \tensor \vv &\mapsto (\vu \mapsto \phi(\vu) \vv)
		\end{align*}
		Dies ist eine lineare Abbildung zwischen Räumen der gleichen Dimension, also ein Isomorphismus! Der Raum $\Hom(U,V)$ ist also ein Tensorprodukt!
	\end{beispiel}
	\noindent Eine Konsequenz ist nun, dass Tensorprodukte endlichdimensionaler Räume als Matrizen betrachtet werden können! Genauer:
	\begin{definition}
		Sei $\dim U < \infty$ und $\dim V < \infty$. Seien $\vu_{1}, \hdots, \vu_{n}$ und $\vv_1, \hdots, \vv_m$ angeordnete Basen von $U$ und $V$. Wir betrachten die folgende \textit{lexikographisch angeordnete Produktbasis}:
		\begin{align*}
			\begin{array}{ccc}
				\vu_1 \tensor \vv_1, & \hdots & \vu_1 \tensor \vv_m,\\
				\vu_2 \tensor \vv_1, & \hdots & \vu_2 \tensor \vv_m,\\
				\vdots & & \vdots\\
				\vu_n \tensor \vv_1, & \hdots & \vu_n \tensor \vv_m,\\
			\end{array}
		\end{align*}
	\end{definition}
	\section{Tensorprodukte von Abbildungen und Matrizen}
	\begin{definition}
		Sei $f_1 : U_1 \to V_1$ und $f_2 : U_2 \to V_2$. Wir definieren folgende Abbildung:
		\begin{align*}
			U_1 \times U_2 \overset{f_1 \times f_2}{\longrightarrow} V_1 \times V_2 \overset{\tensor}{\longrightarrow}{V_1 \tensor V_2}
		\end{align*}
		Diese Abbildung ist bilinear! Also gibt es genau eine lineare Abbildung, sodass das folgende Diagramm kommutiert:
		% https://q.uiver.app/#q=
		\[\begin{tikzcd}
			{U_1 \times U_2} && {U_1 \otimes U_2} \\
			\\
			{U_1 \times U_2} && {V_1 \otimes V_2}
			\arrow["{\tau_U}", from=1-1, to=1-3]
			\arrow[equals, from=1-1, to=3-1]
			\arrow["{\exists!\eta}", from=1-3, to=3-3]
			\arrow["{\tau_V \circ (f_1 \times f_2)}"', from=3-1, to=3-3]
		\end{tikzcd}\]
		Betrachte:
		\begin{align*}
			f_1 : k^{b_1} &\to k^{a_1}\\
		...
		\begin{align*}
			...
			\vw &\mapsto A_2 \vw
		\end{align*}
		So erhalten wir das Tensorprodukt von Abbildungen:
		\begin{align*}
			f_1 \tensor f_2 : (k^{b_1}) \tensor (k^{b_2}) \to (k^{a_1}) \tensor (k^{a_2})
		\end{align*}
		Wir betrachten in den Räumen $k^i$ die Standardbasen und in den Tensorprodukten die lexikographisch angeordneten Produktbasen $B_b$, $B_a$. Die darstellende Matrix
		\[\Mat{B_a}{B_b}{f_1 \tensor f_2} \in \text{Mat}((a_1, a_2) \times (b_1, b_2))\]
		Wird $A_1 \tensor A_2$ geschrieben und als \textbf{Tensorprodukt} oder \tbf{Kroneckerprodukt} von Matrizen bezeichnet.
	\end{definition}
	\begin{proposition}
		Seien $f_1 : U_1 \to V_1$, $f_2 : U_2 \to V_2$ linear, alle Räume endlichdimensional. Seien angeordnete Basen $B_{U_1}, B_{U_2}, B_{V_1}, B_{V_2}$ gegeben. Seien $B_{U_1 \times U_2}$ und $B_{V_1 \times V_2}$ die lexikographisch angeordneten Produktbasen. Dann gilt:
		\begin{align*}
			\Mat{B_{U_1 \times U_2}}{B_{V_1 \times V_2}}{(f_1 \tensor f_2)} = \Mat{B_{U_1}}{B_{V_1}}{f_1} \tensor \Mat{B_{U_2}}{B_{V_2}}{f_2}
		\end{align*}
	\end{proposition}
	\section{Rechenregeln für Tensorprodukte}
	\begin{theorem}
		Sei $V$ ein Vektorraum und $(U_i)_{i \in I}$ eine Familie von Vektorräumen. Dann existiert folgender kanonischer isomorphismus:
		\[\left(\bigoplus_{i \in I} U_i\right) \tensor V \simeq \bigoplus_{i \in I} \left(U_i \tensor V\right)\]
	\end{theorem}
	\begin{theorem}
		Seien $A,B,C$ Vektorräume. Dann existiert folgender kanonischer Isomorphismus:
		\[(A \tensor B) \tensor C \simeq A \tensor (B \tensor C)\]
		Das Tensorprodukt ist also assoziativ.
	\end{theorem}
	\begin{definition}
		Haben wir ein Tensorprodukt von mehr als zwei Räumen bezeichnen wir die Elemente der Menge
		\[R = \left\{\bigotimes_{i \in I} \vv_i \mid \vv_i \in V_i\right\} \subseteq \bigotimes_{i \in I} V_i\]
		als reine Tensoren. Genau wie im Fall mit zwei Räumen bilden diese Elemente ein Erzeugendensystem des Raums $\bigotimes_{i \in I} V_i$.
	\end{definition}
	\begin{theorem}
		Seien $(v_2^1),, \hdots, (v_j^n)$ Erzeugendensysteme von $V_1, \hdots, V_n$. Dann bilden die zugehörigen reinen Tensoren ein Erzeugendensystem des Tensorproduktraums $\bigotimes V_i$. Handelt es sich um eine Menge von Basen, bilden die reinen Tensoren eine Basis des Tensorproduktraums.
	\end{theorem}
	\begin{theorem}
		Die Tensorproduktabbildung 
		\begin{align*}
			\tau : V_1 \times \hdots \times V_n &\to V_1 \otimes \hdots \otimes V_n\\
					(\vv_1, \hdots, \vv_n) &\mapsto \vv_1 \tensor \hdots \tensor \vv_n 
		\end{align*}
		ist \textbf{multilinear}.
	\end{theorem}
	\begin{theorem}
		Das mehrfache Tensorprodukt erfüllt die analoge universelle Eigenschaft zum doppelten Tensorprodukt, für multilineare Abbildungen statt bilinearen Abbildungen.
	\end{theorem}
	Das schönste Beispiel einer Multilinearen Abbildung ist die Determinante:
	\begin{align*}
		\text{det} : (\bR^n)^n &\to \bR\\
					(\vv_1, \hdots, \vv_n) &\mapsto \text{det}(\vv_1, \hdots, \vv_n)
	\end{align*}
	Gemäß der universellen Eigenschaft des Tensorprodukts kommutiert das folgende Diagramm:
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{(\mathbb{R}^n)^n} && {\bigotimes\mathbb{R}^n} \\
		\\
		{(\mathbb{R}^n)^n} && {\mathbb{R}}
		\arrow["\tau", from=1-1, to=1-3]
		\arrow[equals, from=1-1, to=3-1]
		\arrow["{\exists! \eta}", from=1-3, to=3-3]
		\arrow["{\text{det}}"', from=3-1, to=3-3]
	\end{tikzcd}\]
	Die Determinante lässt sich also auch als lineare Abbildung aus dem Tensorprodukt nach $\bR$ definieren.
	\begin{definition}
		Sei $V$ ein $k$-Vektorraum, sei $n \in \bN$. Wir definieren:
		\begin{align*}
			V^{\tensor n} = 
			\begin{cases}
				k & n = 0\\
				\overset{\underbrace{V \tensor \hdots \tensor V}}{n \text{ mal}} & n > 0\\
			\end{cases}
		\end{align*}
	\end{definition}
	\begin{proposition}
		Gegeben $a, b > 0$, dann gibt es genau eine bilineare Abbildung
		\[m_{ab} : V^{\tensor a} \times V^{\tensor b} \to V^{\tensor(a + b)}\]
		sodas für alle reinen Tensoren gilt:
		\begin{align*}
			m_{ab} (\vv_1 \tensor \hdots \tensor \vv_a, \vw_1 \hdots \vw_b)
			= \vv_1 \tensor \hdots \tensor \vv_a \tensor \vw_1 \hdots \vw_b
		\end{align*}
	\end{proposition}
	\begin{proposition}
		Gegeben $a = 0$, $b > 0$, dann gibt es genau eine bilineare Abbildung
		\[m_{0b} : V^0 \tensor V^b \to V^b\]
		sodass für alle reinen Tensoren gilt:
		\[m_{ab} (\lambda, \vw_1, \hdots, \vw_b) = \lambda \vw_1 \tensor \hdots \tensor \vw_b\]
		Analog für $a > 0$, $b = 0$ und $a = b = 0$.
	\end{proposition}
	\begin{definition}
		Ein $k$-Vektorraum $W$ zusammen mit einer bilinearen Abbildung 
		\[m : W \times W \to W\] 
		wird als \textbf{$k$-Algebra} bezeichnet.
	\end{definition}
	\begin{definition}
		Wir nennen eine Algebra:
		\begin{itemize}
			\item kommutativ, falls
			\[\forall \vv, \vw \in W : m(\vv, \vw) = m(\vw, \vv)\]
			\item assoziativ, falls
			\[\forall \vv, \vw, \vu \in W : m(\vv, m(\vw, \vu)) = m(m(\vv, \vw), \vu)\]
			\item "mit Eins", falls $\ve \in W$ existiert, sodass
			\[\forall \vw \in W m(\ve, \vw) = m(\vw, \ve) = \vw\]
		\end{itemize}
	\end{definition}
	\begin{proposition}
		Sei $V$ ein $k$-Vektorraum. Das Tensorprodukt $V \tensor V$ bildet mit der Abbildung $m$ eine assoziative $k$-Algebra mit Eins, bekannt als die \textbf{Tensoralgebra}.
	\end{proposition}
\end{section}
\end{chapter}
\chapter{Die Äußere Algebra}
\begin{definition}
	Seien $V,W$ $k$-Vektorräume und $n \in \bN$ eine natürliche Zahl. Eine multilineare Abbildung
	\[s : V^n \to W\]
	heißt \textit{alternierend}, falls für alle Tupel $(\vv_1, \hdots, \vv_n)$ mit mindestens zwei identischen Einträgen $\vv_i = \vv_j$ gilt, dass
	\[s(\vv_1, \hdots, \vv_n) = 0\]
\end{definition}
\begin{lemma}
	Sei $s$ eine alternierende multilineare Abbildung und $\sigma = (ij)$ eine Transposition, so ist
	\[s(\vv_1, \hdots, \vv_n) = -s(\vv_{\sigma(1)}, \hdots, \vv_{\sigma(n)})\]
	Also ändert sich das Vorzeichen, wenn man zwei der Argumente vertauscht.
\end{lemma}
\begin{proof} (Im Fall $n = 3$):
	\begin{align*}
		s(\vv_1, \vv_2, \vv_3) &= s(\vv_1, \vv_2, \vv_3) + s(\vv_1, \vv_2, \vv_1)\\
							   &= s(\vv_1, \vv_2, \vv_1 + \vv_3)
	\end{align*}
	\begin{align*}
		s(\vv_3, \vv_2, \vv_1) &= s(\vv_3, \vv_2, \vv_1) + s(\vv_3, \vv_2, \vv_3)\\
		&= s(\vv_3, \vv_2, \vv_1 + \vv_3)
	\end{align*}
	\begin{align*}
		s(\vv_1, \vv_2, \vv_1 + \vv_3) + s(\vv_3, \vv_2, \vv_1 + \vv_3) &= 0\\
		\implies s(\vv_1, \vv_2, \vv_3) + s(\vv_3, \vv_2, \vv_1) &= 0\\
		\implies s(\vv_1, \vv_2, \vv_3) &= -s(\vv_3, \vv_2, \vv_1)
	\end{align*}
\end{proof}
\begin{lemma}
	Für jede Permutation $\sigma \in S_n$ und alle Tupel $(\vv_1, \hdots, \vv_n) \in V^n$ gilt:
	\[s(\vv_{\sigma(1)}, \hdots, \vv_{\sigma(n)}) = \text{sgn}(\sigma) \cdot s(\vv_1, \hdots, \vv_n)\]
\end{lemma}
\begin{definition}
	Sei $V$ ein $k$-Vektorraum, $n \in \bN$. Ein $n$-faches äußeres Produkt (auch "Dachprodukt" oder "wedge product") ist ein Vektorraum $T$ zusammen mit eine alternierenden multilinearen Abbildung $\tau : V^n \to T$, sodass für jede alternierende multilineare Abbildung $s: V^n \to W$ eine eindeutige lineare Abbildung $\eta : T \to W$ existiert, sodass
	\[s = \eta \circ \tau\]
	Also kommutiert das folgende Diagramm:
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{V^n} &&&& T \\
		\\
		{V^n} &&&& W
		\arrow["{\tau \text{ alternierend multilinear}}"', from=1-1, to=1-5]
		\arrow["{id}", from=1-1, to=3-1]
		\arrow["{\exists! \eta \text{ linear}}", from=1-5, to=3-5]
		\arrow["{s \text{ alternierend multilinear}}", from=3-1, to=3-5]
	\end{tikzcd}\]
\end{definition}
\begin{lemma}
	Äußere Produkt existieren und sind eindeutig bis auf kanonischen Isomorphismus.
\end{lemma}
\begin{definition}
	Wir schreiben $T$ als
	\[\bigwedge^n V\]
	und $\tau(\vv_1, \hdots, \vv_n) \in \bigwedge^n V$ als:
	\[\vv_1 \wedge \hdots \wedge \vv_n\]
\end{definition}
\begin{definition}
	Ein Element $t \in \bigwedge^n V$ heißt \textit{reines Dach}, wenn $\vv_1, \hdots, \vv_n \in V$ existieren, sodass $t = \vv_1 \wedge \hdots \wedge \vv_n$.
\end{definition}
\begin{proposition}
	Jedes Element des Dachproduktraums ist eine endliche Summe reiner Dächer. 
\end{proposition}
\begin{proposition}
	Die Verdachung von Erzeugendensystemen der Basisräume bildet ein Erzeugendensystems des Dachproduktraums.
\end{proposition}
\begin{theorem}
	Die Verdachung von Basen der Basisrume bildet \ul{keine} Basis des Dachproduktraums!
\end{theorem}
\begin{proof}
	Sei $V = \bR^3$ mit Standardbasis. Dann ist
	\[\{\ve_i \wedge \ve_j \mid i,j \in \{1,2,3\}\}\]
	ein Erzeugendensystem von $\bigwedge^2 V$. Allerdings folgt aus der Definition über alternierende Abbildungen, dass $\ve_i \wedge \ve_i = 0$ und $\ve_i \wedge \ve_j = -\ve_j \wedge \ve_i$. Also bildet das System keine Basis.
\end{proof}
\begin{proposition}
	Sei $(\vb_i)$ eine Basis eines Vektorraums $V$. Die Menge
	\[\{\vb_{i_1} \wedge \hdots \wedge \vb_{i_n} \mid i_{k} < i_{k+1}\}\]
	bildet eine Basis von $\bigwedge^2 V$.
\end{proposition}
\noindent Es folgt, dass der Dachproduktraum eine deutlich niedrigere Dimension als der Tensorproduktraum hat!
\begin{lemma}
	Sei $\dim V = a$. Dann ist
	\[\dim \bigwedge^n V = \binom{a}{n}\]
\end{lemma}
\begin{corollary}
	Sei $\dim V = a$.
	\begin{itemize}
		\item Es gilt $\text{dim }\bigwedge^a V = 1$.
		\item Sei $n > a$. Dann ist $\bigwedge^n V = \{0\}$.
	\end{itemize} 
\end{corollary}
\section{Die Determinante}
\begin{theorem}
	Sei $V$ ein $n$-dimensionaler $k$-Vektorraum. Die Determinante kann alternativ definiert werden als die Abbildung:
	\begin{align*}
		d : V &\to k\\
			f &\mapsto \tr(\bigwedge^n f)
	\end{align*}
\end{theorem}
Wir betrachten den Fall $d_m : f \mapsto \tr(\bigwedge^m f)$. Sei $f$ beliebig. Es gilt:
\begin{itemize}
	\item $d_0(f) = 1$
	\item $d_1(f) = \tr(f)$
	\item $\hdots$
	\item $d_n(f) = \det(f)$
\end{itemize}
Dies sieht den Koeffizienten des charakteristischen Polynoms $\chi_f$ verdächtig ähnlich...
\section{Invariantentheorie}
Wie viele Funktionen $GL_n \to GL_n$ gibt es, welche unter ähnlichen Matrizen invariant sind?
Wir wissen, dass es mindestens $n$ "Basisinvarianten" gibt:
\begin{itemize}
	\item $\inv_0 = \tr(\bigwedge^0 f) = 1$
	\item $\inv_1 = \tr(\bigwedge^1 f) = \tr(f)$
	\item $\inv_2 = \tr(\bigwedge^2 f)$
	\item $\hdots$
	\item $\inv_n = \tr(\bigwedge^n f) = \det(f)$
\end{itemize}
\begin{theorem}
	Alle Polynome in der Menge $(\inv_i)_{i \in (0,n)}$ sind ebenfalls Invarianten. Genauer: Betrachte die Abbildung
	\begin{align*}
		\tau : k[x_0, \hdots, x_n] &\to \text{Invarianten}\\
		f(x_0, \hdots, x_n) &\mapsto f(\inv_0, \hdots, \inv_n)
	\end{align*}
\end{theorem}
\begin{theorem}{\textbf{\emph{Hauptsatz der Invariantentheorie:}}}
	Die Abbildung $\tau$ ist ein Isomorphismus zwischen dem Ring $k[x_0, \hdots, x_n]$ und dem Ring der Invarianten. Dementsprechend geben diese Polynome den gesamten Raum der Invarianten.
\end{theorem}
%
%
%
...
\[\begin{tikzcd}
	...
	\arrow["{\mathcal{Y}}"', dotted, tail reversed, from=1-1, to=3-1]
	\arrow["{\mathcal{Y}}", dotted, tail reversed, from=1-3, to=3-3]
